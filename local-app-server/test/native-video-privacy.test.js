...
test('video privacy is shared by app playback and the edge device', async () => {
  ...
  try {
    ...
    const camera = await request(baseURL, '/api/cameras', {
      ...
      body: JSON.stringify({
        family_id: familyID, device_id: deviceID, name: '客厅主视', room: '客厅',
    ...
    const playback = await request(baseURL, '/api/v1/video/sessions', {
      method: 'POST', headers: owner.headers,
      body: JSON.stringify({
        resource_type: 'stream', camera_id: String(camera.body.id), profile: 'mobile', privacy_mode: 'original',
      }),
    });
    ...
    const posePacket = {
      schema_version: 'eacp-pose-relay-v1', camera_id: 24, frame_id: 'frame-100',
      captured_at: new Date().toISOString(), state: 'observed', source: 'observed',
      image_width: 640, image_height: 360,
      poses: [{
        track_id: 'person-1', confidence: 0.91, bbox: [120, 20, 400, 350],
        keypoints: [{ name: 'nose', x: 220, y: 60, confidence: 0.95, visible: true }],
      }],
      display_only: true, formal_evidence_eligible: false,
    };
    ...
    assert.ok(Buffer.from(sceneChunk.value).includes(jpeg));
