...
function createDbFromCloudRows(rowsByTable, fallbackDb) {
    ...
    for (const asset of rowsByTable.media_assets || []) {
        db.assets.push({
            ...
            retention_status: asset.retention_status || "active",
            retention_reason: asset.retention_reason || "",
            retain_until: iso(asset.retain_until),
            deletion_attempts: Number(asset.deletion_attempts || 0),
            deletion_error: asset.deletion_error || "",
...
function mediaLifecycleOrphanPersistenceRow(orphan) {
    return {
        ...
        protection_reason: String(orphan.protection_reason || "").slice(0, 500),
        metadata: orphan.metadata && typeof orphan.metadata === "object" && !Array.isArray(orphan.metadata)
            ? orphan.metadata
            : {},
