...
- 公共 API 客户端已升级为按账号隔离的持久化分层缓存：家庭与关怀资料使用分钟级缓存，设备与摄像头使用秒级缓存，事件短缓存；过期数据先显示并在后台静默更新，实时快照与视频流不缓存，任何写操作自动失效缓存。
- 首页、守护、事件、陪伴、我的已接入共享页面状态仓库。二次进入先恢复最近一次真实渲染，再重连视频和后台刷新；首次没有缓存时直接展示中性的页面结构，不再出现整页转圈或逐模块“正在读取/同步中”。短期关怀图片票据随快照保留并在新图加载完成后替换，过期票据回退为卡片真实标题和正文；视频流、带 access token 的地址、表单输入和临时弹窗不进入快照。登出或账号切换会整体清理缓存。
...
阶段 18 当前状态：基础真机运行、生产代码清理和主页面状态复用已完成并部署腾讯云；本地与线上五个主页面均已移除整页遮罩、转圈和阻塞式加载文案，守护页不会重建双摄卡片造成状态回退，首页与陪伴页返回后关怀原图继续可见。下一步由真机复测主导航和视频流重连体感，确认稳定后进入 APNs 能力配置；旧 `app-shell / detection` 产品路径迁移继续保持独立任务。
