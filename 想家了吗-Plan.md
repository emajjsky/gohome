...
#### X. 阶段 23：EACP 事件自适应连续姿态感知（P0/P1/P2/P3a/P3a.1 已完成，P3b-P6 待实施）
...
4. **P3a 连续关键点**：接入 KLT 关键点短时传播，增加前后向误差、骨骼几何、置信度衰减和约 600ms 过期门；管理页分别绘制模型锚点和跟踪补帧。**已完成并部署。** P3a.1 展示解耦也已部署并完成浏览器实机验收：管理页保持一条连续 MJPEG 底图，只轮询无 JPEG 的姿态元数据并叠加骨架；同帧 JPEG 仅用于研发精确帧复核。
5. **P3b 多人轨迹**：接入 OC-SORT 人框轨迹和遮挡后身份恢复，确保同一路多人、短时遮挡和重新出现时不串轨；其结果仍不能绕过模型锚点成为正式安全证据。
6. **P4 风险升频与事件整合**：由运动突变、人体中心快速下降、人体框高度变化和姿态因子触发短时升频；床/沙发内正常躺卧只保持 active，不得被原始单帧跌倒分长期锁在 risk；边缘临时候选先进入 App，云端三图复核继续异步更新同一 SafetyIncident。
7. **P5 数据驱动增强**：仅在真实回归确认具体误报/漏报缺口后按需补样本并评估轻量 TCN/Continual ST-GCN，输出下降、倒地和恢复趋势，与 PoseFactorGraph 并行；公开数据集是备选，不直接部署 SkateFormer、CoTracker 或 TAPIR。
8. **P6 加速后端评估**：只有 CPU 路线未达到双摄延迟和温度目标时，才对 RTMO、Hailo YOLO 或兼容姿态模型做同帧 A/B；业务规则不得绑定 AI HAT+。
...
阶段 23 当前状态：P0/P1/P2/P3a/P3a.1 已完成并部署真实树莓派。P1 关闭算法页时双摄 idle 实测约 0.99-1.15 FPS/路，双路视频约 7.97-7.98 FPS。P2 在同一历史真人帧上的姿态阶段中位数由约 0.461 秒降至 0.068 秒，减少 85.3%；包含外层 YOLO 的完整模型链由约 0.711 秒降至 0.318 秒，减少 55.3%。P3a 实机走动已在两路摄像头产生大量真实 `observed / tracked / expired`。P3a.1 浏览器验收期间，camera 25 在 `tracked / observed / expired` 间切换 10 次，MJPEG `src` 始终只有 1 个且未变化；骨架过期只清覆盖层，双摄切换后新视频稳定为 640x360，无空白层和前端错误。统一视觉感知页只消费后台 EACP 结果，不因打开页面重复执行完整 RTMPose。当前真实场景两路同时有人时纯 CPU 仍约 216% 且温度约 81.8 摄氏度，因此下一步 P3b/P4 必须同时评估多人轨迹和多路推理预算；在未确认前不擅自改成静态单摄或全局单槽位。
