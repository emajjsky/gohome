...
P4 当前状态：家庭私密时间流、评论、收藏、人物/时间标签、最多 9 张图片或 1 个 60 秒内视频已完成；媒体在原生端压缩后通过临时签名直传私有 COS，PostgreSQL 只保存结构化元数据，视频通过短时签名 URL 流式播放。真实 COS 上传、Range 播放、元数据回读和删除清理已通过，iPhone 端照片/视频选择、压缩时延、前后台恢复和真机播放仍需现场验收。事件证据迁移到 `event-evidence/*` 不属于本 P4 分支。
