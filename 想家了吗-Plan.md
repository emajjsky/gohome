...
3. **分层计算**：Hailo 负责高频人物姿态，CPU YOLO 每 3 秒补充宠物和家具；Hailo 失败自动回退 CPU YOLO + RTMPose，30 秒后重试。
...
6. **I/O 预算**：风险观察证据最多 4 FPS，日常观察保持 5 秒一次，正式事件立即写入；清理必须保留事件引用媒体、开放状态和待上传任务。
7. **兼容验收**：覆盖双摄、盒子算法页 12 FPS 同步预览、公网 8 FPS 中继、互斥可辨认的骨架/模糊隐私、宠物/家具/火灾辅助链、盒子日志、云端复核、App 事件和 CPU 回退。

当前证据：独立 300 帧 `125.60 FPS` 是不含双摄调度、CPU 解码、场景补充、规则、存储和展示的裸网络基准，不作为产品 FPS。候选预筛选后，CPU DFL/关键点解码与 NMS 从约 16-18 ms 降至无人约 0.23-0.38 ms、有人约 0.67-0.70 ms；Hailo 姿态全流程中位约 13.2 ms、P95 约 14.5 ms，常规完整产品分析约 14-17 ms。提高调度节拍后实机 CPU 约 57%、温度约 60.6 摄氏度、Hailo 失败为 0。每 3 秒一次的宠物/家具 CPU 辅助链仍有约 120-137 ms 短时尖峰，KLT 与独立视频流用于承接显示连续性。误触发排查已移除沙发多人卧躺的 box-only 假跌倒，并将历史高频证据写入造成的数据库/快照膨胀清理到约 963 MB/840 MB，SQLite `quick_check` 通过。
