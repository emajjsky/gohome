...
3. [x] 主线改为所有隐私模式始终显示手机实际解码 FPS；骨架模式收到姿态流后追加真实 `POSE Hz`，有效的空姿态包不再清零速率；当前完整 iOS 自动化 `152/152` 和发布元数据校验通过。
4. [ ] 下一 TestFlight 构建实机确认骨架 FPS/POSE Hz；继续验收长时间停留、切页返回、蜂窝网络、家庭与手机双位置、自动业务推送、跌倒事件和单次通知。
...
2. [x] 历史安全背景增加当前场景一致性校验；同步包校验摄像头 ID，摄像头配置变化时串行停旧线程、清缓存、启新线程。
3. [x] 通过人工注入另一路背景的回归测试，确认输出拒绝跨路像素；修复已部署盒子并清空旧内存背景。
4. [x] 新增摄像头级宠物候选轨迹、两次命中确认、猫狗多帧加权投票、单帧错类阻止和 2.2 秒短时保持。
5. [x] 宠物稳定器通过单帧、确认、错类、过期和双摄隔离回归；验证脚本排除生产部署。
6. [ ] 真机确认两路骨架模式不再串画面；让家庭猫狗分别在近、中、远距离和部分遮挡下通过两路摄像头，保留错误样本后再评估专项模型。
