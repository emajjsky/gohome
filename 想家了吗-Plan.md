...
状态：摄像头 31 校准完成；管理流身份根因已本地修复并通过完整边缘回归；摄像头 32 和双摄切换等待现场验收。
...
8. [ ] 部署 Pi，连续切换两路并制造一路不可用，确认零旧像素、零错标签和正确加载/错误状态。
9. [ ] 摄像头 32 真实画面无人时完成校准，核对持久文件、哈希与 `ready=true`；随后切换纯骨架并完成管理首页、算法页和 TestFlight 三端真人验收。
10. [ ] 单次重启服务，确认两路持久基线复验恢复；完成进入、遮挡、离开、宠物移动和 30 分钟双摄稳定性测试。
