...
12. [ ] 执行双摄有人、遮挡、快速移动、离开、宠物和电视内容实测，目标活跃人物 Pose 每路约 `10-15 Hz`；记录 Hailo 队列、合并/替换、阶段 P50/P95、CPU、温度、Tracker 锚点 Hz 和真实成品 FPS。空场电视假候选已实机反证：131 个原始 Pose 样本下 Scheduler 始终 idle，旧 refresh 为 0，正式验证由约 12 次/分钟降至 3 次并按 `10/20/40 秒` 递增。下一步只剩真人进入延迟、遮挡、快速移动、离开、宠物和长时温控。
