...
19. [ ] 等摄像头 31 真实空房后由现有基线自动完成 `3/3` 复验，确认文件哈希不变且复验调度活动流清零；不得重复校准或绕过人物门禁。
20. [ ] 两路均 `ready` 后切换纯骨架，完成管理首页、算法页、生产云和 TestFlight 的同帧真人验收；随后单次重启服务复验持久基线恢复，并完成进入、遮挡、离开、宠物移动和 30 分钟双摄稳定性测试。
