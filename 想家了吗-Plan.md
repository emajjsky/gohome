...
状态：管理流会话隔离与连接终止已通过 20 次真实切换；摄像头 31 已恢复持久资产发现和安全复验，当前因真实人物在场保持 `0/3`，摄像头 32 等待真实空房。
...
18. [ ] 摄像头 32 真实画面无人时完成校准，核对持久文件、文件/像素哈希与 `ready=true`；随后切换纯骨架并完成管理首页、算法页和 TestFlight 三端真人验收。
19. [ ] 两路画面真正无人后单次重启服务，确认两路持久基线发现与三次自动复验；完成进入、遮挡、离开、宠物移动和 30 分钟双摄稳定性测试。
