...
4. **调度预算**：Hailo 模式 idle/active/risk 间隔分别为 `0.5 / 0.10 / 0.067` 秒；CPU 模式保持原热保护预算。运动门只立即唤醒一次 Hailo 探测，正式人物才续期 active，KLT 风险提示才进入 risk，避免无人路因光照或画面噪声长期占用高频预算。双摄按最新帧公平调度，算法页开关和页面开闭不影响正式 worker。
...
7. **兼容验收**：覆盖双摄、8 FPS 中继、骨架/模糊隐私、宠物/家具/火灾辅助链、盒子日志、云端复核、App 事件和 CPU 回退。

当前证据：独立 300 帧基准约 125.60 FPS 且无 Hailo/DMA 错误；真实客厅三人画面稳定推理中位数 32.56 ms、P95 38.13 ms，生产记录约 28-32 ms；近 2000 次正式推理失败为 0。误触发排查已移除沙发多人卧躺的 box-only 假跌倒，并将历史高频证据写入造成的数据库/快照膨胀清理到约 963 MB/840 MB，SQLite `quick_check` 通过。
