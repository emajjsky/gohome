...
#### X. 阶段 23：EACP 事件自适应连续姿态感知（P0/P1/P2/P3a 已完成，P3b-P6 待实施）
...
4. **P3a 连续关键点**：接入 KLT 关键点短时传播，增加前后向误差、骨骼几何、置信度衰减和约 600ms 过期门；管理页分别绘制模型锚点和跟踪补帧。**已完成并部署。**
5. **P3b 多人轨迹**：接入 OC-SORT 人框轨迹和遮挡后身份恢复，确保同一路多人、短时遮挡和重新出现时不串轨；其结果仍不能绕过模型锚点成为正式安全证据。
6. **P4 风险升频与事件整合**：由运动突变、人体中心快速下降、人体框高度变化和姿态因子触发短时升频；边缘临时候选先进入 App，云端三图复核继续异步更新同一 SafetyIncident。
7. **P5 数据驱动增强**：申请数据集到位后训练轻量 TCN/Continual ST-GCN，输出下降、倒地和恢复趋势，与 PoseFactorGraph 并行；不直接部署 SkateFormer、CoTracker 或 TAPIR。
...
阶段 23 当前状态：P0/P1/P2/P3a 已完成并部署真实树莓派。P1 关闭算法页时双摄 idle 实测约 0.99-1.15 FPS/路，双路视频约 7.97-7.98 FPS。P2 在同一历史真人帧上的姿态阶段中位数由约 0.461 秒降至 0.068 秒，减少 85.3%；包含外层 YOLO 的完整模型链由约 0.711 秒降至 0.318 秒，减少 55.3%。P3a 实机走动已在两路摄像头产生大量真实 `observed / tracked / expired`：camera 24 为 `4741 / 15002 / 2726`，camera 25 为 `10386 / 34862 / 5157`；最近有效跟踪分别保留 14 点和 9 点，前后向误差约 0.1193 和 1.1218，人物离开后 600ms 内清除，无旧骨架残留。统一视觉感知页现改为读取后台同帧 EACP 数据，不再因打开页面重复执行完整 RTMPose；实线为模型锚点，虚线为 KLT 补帧，并显示年龄、有效点和 FB 误差。下一步进入 P3b OC-SORT 多人轨迹，再完成 P4 风险升频；真实人物 risk 3-5 FPS、300ms 风险触发和 1.5-3 秒候选延迟仍需后续安全动作验收。
