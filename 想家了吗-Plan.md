...
目标：EACP 以“最新帧视频 + 自适应模型锚点 + 有界关键点跟踪 + 时间规则 + 云端复核”形成一条正式链路。早期 CPU 路线用于证明人物出现后约 2 FPS、风险 3-5 FPS 的调度与证据边界；Hailo-8 接入后，产品目标提升为有人约 15 FPS、风险约 20 FPS，盒子算法管理页使用约 12 FPS 的同步姿态预览，公网云中继暂时保持 8 FPS。
...
- CPU 回退模式维持活跃约 2 FPS、风险 3-5 FPS；Hailo 模式目标为活跃约 15 FPS、风险约 20 FPS，盒子算法管理页约 12 FPS 且上限 15 FPS，并明确锚点、跟踪和 coasting 来源。公网云中继仍以 8 FPS 独立验收。
...
隐私修正：`原画 / 人物模糊 / 骨架保护` 三种模式已分离；骨架保护使用人物遮罩后绘制骨架，不再复用“模糊再叠加骨架”的视觉。配置改为本地先应用和持久化，云端 401 时保留本地效果并暴露待同步状态，恢复绑定后再收敛到云端家庭配置。
...
#### 阶段 27：活动区间 outbox 与有效观察覆盖率（代码完成，实机上传待恢复绑定）
...
6. [ ] 恢复设备绑定后验收真实区间上传、真实断网补传、云端七日概览和 App 活动变化提醒；本阶段不改 EACP、跌倒/火灾阈值或 Hailo HEF。
