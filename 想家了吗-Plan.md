...
- 首页、守护、事件、陪伴、我的已接入共享页面状态仓库。二次进入先恢复最近一次真实渲染，再重连视频和后台刷新；首次没有缓存时只显示一个页面级加载状态，不再逐模块闪烁“正在读取/同步中”。登出或账号切换会清理页面与 API 缓存，视频票据、表单输入和临时弹窗不进入快照。
...
阶段 18 当前状态：基础真机运行、生产代码清理和主页面状态复用已完成并部署腾讯云；本地浏览器二次进入五个主页面约 36-101ms 即显示真实状态，守护页不会重建双摄卡片造成“同步中”回退。下一步由真机复测主导航和视频流重连体感，确认稳定后进入 APNs 能力配置；旧 `app-shell / detection` 产品路径迁移继续保持独立任务。
