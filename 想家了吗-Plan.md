...
阶段 23 当前状态：P0/P1/P2/P3a/P3a.1/P3a.2 已完成并部署真实树莓派。P1 关闭算法页时双摄 idle 实测约 0.99-1.15 FPS/路，双路视频约 7.97-7.98 FPS。P2 在同一历史真人帧上的姿态阶段中位数由约 0.461 秒降至 0.068 秒，减少 85.3%；包含外层 YOLO 的完整模型链由约 0.711 秒降至 0.318 秒，减少 55.3%。P3a 实机走动已在两路摄像头产生大量真实 `observed / tracked / coasting / expired`；新增 `coasting` 只承接展示层，不改变正式证据门。P3a.1 浏览器验收期间，camera 25 在 `tracked / observed / expired` 间切换 10 次，MJPEG `src` 始终只有 1 个且未变化；骨架过期只清覆盖层，双摄切换后新视频稳定为 640x360，无空白层和前端错误。P3a.2 已针对两路同时有人时曾达到约 216% CPU、81.8 摄氏度和历史热降频的问题增加全局推理预算与温度保护；部署后短时观察温度约 71.6-74.7 摄氏度、资源状态 normal，两路 active 锚点约 1.8/0.78 FPS，双路视频和云同步均保持运行。下一步进入 P3b/P4，不能把 P3a.2 的热保护数字替代快速跌倒端到端验收。
