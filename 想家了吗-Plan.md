...
状态：摄像头 31 校准完成；管理流首轮部署发现旧 HTTP 请求未终止，正在完成连接生命周期根因修复；摄像头 32 等待真实空房。
...
11. [ ] 提交、部署和 20 次切换，确认订阅回落到稳定基线、每次首帧有界到达、零旧像素和零错标签。
12. [ ] 摄像头 32 真实画面无人时完成校准，核对持久文件、哈希与 `ready=true`；随后切换纯骨架并完成管理首页、算法页和 TestFlight 三端真人验收。
13. [ ] 单次重启服务，确认两路持久基线复验恢复；完成进入、遮挡、离开、宠物移动和 30 分钟双摄稳定性测试。
