...
12. [ ] 执行双摄有人、遮挡、快速移动、离开、宠物和电视内容实测，目标活跃人物 Pose 每路约 `10-15 Hz`；记录 Hailo 队列、合并/替换、阶段 P50/P95、CPU、温度、Tracker 锚点 Hz 和真实成品 FPS。空场电视假候选已实机反证：131 个原始 Pose 样本下 Scheduler 始终 idle，旧 refresh 为 0，正式验证由约 12 次/分钟降至 3 次并按 `10/20/40 秒` 递增。真人首轮结构化记录已确认两路可进入、遮挡后恢复、离开清空且不误报，但采样发生在离开后，不能证明活跃频率和进入延迟；健康状态已补最近验证、确认、拒绝和错误的时间与年龄，下一轮必须在同步采样运行期间重复一次短动作，再继续宠物和长时温控。
13. [x] 宠物时序状态改为线程安全的唯一状态源；健康接口按摄像头公开确认类别、猫狗证据命中数、时序平均置信度和最近命中年龄，不暴露图像或框坐标。完整边缘回归 `59/59` 通过。
14. [ ] 部署宠物诊断后，以家中真实猫经过两路视角的同一时间窗为标注，确认错误来自 Hailo Object 原始类别还是时序分类；没有版本化标注集、混淆矩阵和阈值报告前，不把类别准确率写为完成。
