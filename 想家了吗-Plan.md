...
6. [x] 新增 COS 全分页清单、`edge-evidence/` 与 `memory-media/` 孤儿对账、本地媒体孤儿对账；COS 与本地分别建立权威 key 集，禁止同名 key 跨存储后端互相误保护。
...
状态：管理流会话隔离与连接终止已通过 20 次真实切换；摄像头 31 持久文件存在但重启后状态语义错误，摄像头 32 等待真实空房。

1. [x] 分别直连两路 RTSP 抓取真实当前帧，确认摄像头 31 为电视柜/沙发视角、摄像头 32 为冰箱/电视墙视角，配置源和内容指纹彼此独立。
2. [x] 摄像头 31 在无人画面完成 8 帧空房确认和持久提交，状态 `ready`、`baseline_revision=1`，基线文件 SHA-256 为 `cc8ff649c3611f51ae063ba1c089758140415c48c56d12d4566750ffcab5f969`。
...
14. [ ] 修复摄像头 31 持久 `.npz` 在服务重启后被管理页显示为“未校准”的状态语义；应显示“已存基线，待空房复验”，不得要求无意义重校准。
15. [ ] 摄像头 32 真实画面无人时完成校准，核对持久文件、哈希与 `ready=true`；随后切换纯骨架并完成管理首页、算法页和 TestFlight 三端真人验收。
16. [ ] 单次重启服务，确认两路持久基线发现与自动复验；完成进入、遮挡、离开、宠物移动和 30 分钟双摄稳定性测试。
