...
4. 原生完成首页、守护和事件，确保切 Tab 不重新读取整页，视频只播放当前选中一路。
5. 完成回家消息、系统分享、消息动作记录、精选目录和我的设置。
6. 配置 APNs、定位、隐私清单、签名、真机回归和 TestFlight 上传。
7. 原生能力验收后停止家属端 Web 公开入口；盒子管理、云端运维和协议帮助 Web 保留。
...
- 当前口径：边缘事件先入库，再异步调用 Qwen；复核可用时不提前通知，模型不可用时只允许正式边缘高危规则走一次失效保护通知。
...
阶段 4 状态：已完成并部署腾讯云。当前已升级为最多三张代表证据图；早期持续提醒设计已由阶段 29 的 confirmed-once 策略替代。

阶段 5：家庭级长期未见和持续提醒（历史方案，通知部分已由阶段 29 替代）
...
- 历史实现：首次事件通知后按分钟时间桶生成提醒；该生成器已在阶段 29 删除，当前只允许 confirmed 首次通知一次。
- 已完成：真实双摄像头覆盖率均为 `0.8194`，家庭状态为 observing，最近见人后未误触发 long_absence；部署后历史 incident reminder 为 0。

阶段 5 状态：家庭级长期未见已完成；分钟提醒属于已废弃历史实现，当前通知口径以阶段 29 为准。
...
- 已增加 MJPEG 内部重连、短暂黑帧保持和页面指数退避重连；持续真实黑屏仍按原阈值检测。
...
- 历史实现曾为 confirmed/rejected/uncertain 分别生成结果消息；阶段 29 已收口为仅 confirmed 首次告警，rejected/uncertain 只更新事件。
- 已实现同家庭、同事件类型、45 秒窗口内的跨摄像头关联；多个摄像头保留各自事件证据，但 App 列表和唯一 confirmed 通知只使用主事件。
- 已实现复核聚合优先级：任一路 confirmed 即确认；没有确认但存在 uncertain、failed 或 unavailable 时需要人工确认；仍有任务处理中时保持 verifying；全部 rejected 才排除。
- 已实现用户确认联动：处理任一关联事件会确认整个 incident 并归档关联开放消息。
...
阶段 16 状态：三图证据与复核已完成。分钟提醒验收项已废弃；当前应核对 confirmed 单次通知、App 确认归档和可信恢复三者的一致性，宠物现场校准等待真实猫狗条件。
...
目标：EACP 以“最新帧视频 + 自适应模型锚点 + 有界关键点跟踪 + 时间规则 + 云端复核”形成一条正式链路。早期 CPU 路线用于证明人物出现后约 2 FPS、风险 3-5 FPS 的调度与证据边界；Hailo-8 接入后，正式模型锚点目标为有人约 15 FPS、风险约 20 FPS。当前盒子算法管理页使用约 12 FPS 的同步姿态预览，公网云中继暂时保持 8 FPS；后续 30 FPS 展示升级必须改造视频传输和客户端同步渲染，不能只提高 MJPEG/JPEG 限频。
...
9. **P6 Hailo 加速后端（已完成代码与实机基线）**：采用 YOLOv8s Pose HEF 一次输出人物框与 17 关键点，复用 EACP 跟踪、姿态因子图、规则和云端复核。业务规则不绑定 AI HAT+，设备失败时回退 CPU YOLO + RTMPose；Hailo 真人跌倒端到端复验仍待恢复正式云端绑定后完成。

验收门：

- 双路视频持续不低于 7.5 FPS；算法页开关不改变正式守护频率。
- 人物进入 1.5 秒内开始姿态观察，风险升频触发目标不超过 300ms，边缘临时候选约 1.5-3 秒形成。
- CPU 回退模式维持活跃约 2 FPS、风险 3-5 FPS；Hailo 模式正式锚点目标为活跃约 15 FPS、风险约 20 FPS，并明确 `observed / tracked / coasting` 来源。当前 MJPEG 算法页约 12 FPS 且上限 15 FPS、公网 JPEG 中继 8 FPS；30 FPS 产品展示另按 H.264/WebRTC 和时间戳骨架通道验收。
...
#### 阶段 25：Hailo-8 统一姿态加速（代码与实机基线完成，闭环复验待完成）

1. **硬件与驱动**：确认 Hailo-8 26 TOPS、`/dev/hailo0`、HailoRT 4.23 和 YOLOv8s Pose HEF；固定由系统级 `gohome-edge-agent.service` 单进程持有设备。驱动补丁纳入仓库并执行 300 帧无 DMA/call-trace 压测。
2. **统一推理后端**：实现持久化 Hailo VDevice、bindings 和输出缓冲，一次推理同时解码人物框与 17 关键点；复用现有姿态质量、分类、因子图和跌倒状态机，不复制业务规则。
3. **分层计算**：Hailo Pose 负责高频人物姿态，Hailo Object 约每 1 秒补充猫狗和家具；两个 HEF 共享单一 VDevice 和 ROUND_ROBIN 调度。任一 Hailo 路径失败时回退 CPU YOLO/RTMPose，30 秒后重试，不复制业务规则。
4. **调度预算**：Hailo 模式 idle/active/risk 间隔分别为 `0.5 / 0.067 / 0.05` 秒，对应约 `2 / 15 / 20 FPS` 的单路预算；CPU 模式保持原热保护预算。运动门只立即唤醒一次 Hailo 探测，正式人物才续期 active，KLT 风险提示才进入 risk，避免无人路因光照或画面噪声长期占用高频预算。双摄按最新帧公平调度，算法页开关和页面开闭不影响正式 worker。
5. **语义收口**：Hailo 模式关闭冗余 box-only 跌倒路径；床/沙发正常卧躺不进入姿态或框跌倒候选，静态因子图高分也不单独升到 risk。真实跌倒仍可由同轨迹垂直下降+运动、正式快速跌倒因子、RuleEngine 和云端多图复核确认，包括跌到沙发上的动作。
6. **I/O 预算**：普通分析只在内存运行；首次可信人物、姿态变化和离开即时写结构化活动段，持续有人每 10 分钟写轻量心跳，持续无人不写；风险证据最多每 0.5 秒一张，正式事件立即写入完整证据链。清理必须保留事件引用媒体、开放状态和待上传任务。
7. **兼容验收**：覆盖双摄、盒子算法页 12 FPS 同步预览、公网 8 FPS 中继、互斥可辨认的骨架/模糊隐私、宠物/家具/火灾辅助链、盒子日志、云端复核、App 事件和 CPU 回退。

当前证据：独立 300 帧 `125.60 FPS` 是不含双摄调度、视频解码、业务后处理、规则、存储和展示的裸网络基准，不作为产品 FPS。统一双模型后，Hailo Pose 中位约 13.28 ms、P95 约 32.06 ms，Hailo Object 中位约 34.42 ms、P95 约 35.73 ms，完整 Pipeline 中位约 16.36 ms、P95 约 51.95 ms，两个后端失败均为 0；双摄稳定运行 CPU 约 40%、温度约 56°C。事件驱动持久化的 203 次分析没有普通图片写入，规避 199 次冗余落盘。SQLite 清理并受控压缩后约 444MB，`integrity_check=ok`。

隐私修正：`原画 / 人物模糊 / 骨架保护` 三种模式已分离；骨架保护删除人物像素，以按摄像头隔离的无人物背景重建场景后只绘制骨架，禁止矩形填黑或“模糊再叠加骨架”。首次无背景时使用隐私安全的场景修复，空场景出现后自动转为真实背景。配置改为本地先应用和持久化，云端不可用时保留本地效果并暴露待同步状态，恢复连接后再收敛到云端家庭配置。

剩余验收顺序：

1. 恢复盒子与腾讯云的正式家庭绑定和两路摄像头配置，确认设备令牌、隐私配置、中继和日志重新同步；不得直接让空云端配置覆盖盒子现有两路摄像头。
2. 连续观察 30-60 分钟，要求 Hailo 失败为 0、无新增 PCIe/DMA/call-trace、温度无持续热降频、证据写入受限、两路视频在线。
3. 使用 EACP 验收会话依次完成走动、快速坐下、下蹲、沙发卧躺和低风险模拟跌倒。前三类不得形成事故；模拟跌倒必须形成同轨迹边缘事件、最多三帧证据、云端多模态结论和 App 处置时间线。
4. 完成上述现场验收后再冻结阈值并提交；未完成前只能表述为“Hailo 加速后端和正常场景实机通过”，不能表述为“Hailo 版跌倒闭环全部验收通过”。

#### 阶段 26：Hailo 双模型与数据生命周期收口（代码与实机完成，云端复验待绑定）

1. 已将猫狗、沙发、床、椅子、餐桌和电视补充识别从周期性 CPU YOLO 迁移到 Hailo Object，并与 Hailo Pose 共用一个设备上下文；CPU 仅保留故障回退。
2. 已删除正式 worker 的周期 `live_frame` 入队逻辑。实时画面只走内存中继，普通分析不保存图片，COS 只接收正式事件关键帧和用户媒体。
3. 已完成活动轨迹的事件驱动持久化：出现、姿态变化、离开即时记录，持续有人 10 分钟心跳，无人不写；不会因降低落盘频率漏掉人物活动段。
4. 已设置本地 6 小时普通记录保留、70%/85% 磁盘水位和分批清理；已补齐清理事务对已关闭活动段、姿态段和已完成上传任务的历史快照引用解除。
5. 已完成数据库备份、`WAL checkpoint`、`VACUUM` 和完整性检查，数据库由约 1.69GB 降到约 444MB，空闲页归零；历史已删除摄像头留下的旧事件外键另列数据迁移任务，不能删除历史事件换取表面检查通过。
6. 下一步只做两项：App 局域网重新配对恢复令牌和云端配置；随后执行 30-60 分钟稳定性观察及走动、快坐、下蹲、沙发躺卧、模拟跌倒的端到端现场验收。

#### 阶段 27：活动区间 outbox 与有效观察覆盖率（代码与真实上传已验证，断网和七日产品验收待完成）

1. [x] 新增每路 `activity_export_cursors`，人物出现建立游标，姿态变化/10 分钟心跳/离开关闭区间，单段最长 6 小时。
2. [x] 新增 `activity_interval_upload`，复用 `upload_jobs` 退避、重试和幂等键；发送前把本地 camera ID 映射为云端 camera ID，payload 明确 `contains_media=false`。
3. [x] 离线、禁用、删除和 worker 重启时按最后可信观察收口；长观察间隙不跨断电时间连接成伪活动。
4. [x] 新增线程安全内存 `ObservationCoverageTracker`，配置同步改读 worker resolver；历史最后见人从 presence session 与旧 snapshot 取较新值，普通覆盖率不写 SQLite/JPEG。
5. [x] 通过活动区间、断网间隙、运行状态收口、远端摄像头映射、观察覆盖率、事件驱动持久化和配置同步定向回归；模拟 HTTP 401 已确认任务保持 `failed + next_attempt_at` 且 payload 不丢失。
6. [x] 设备绑定恢复后已完成真实活动区间上传，盒子 outbox 与云端 `activity_intervals` 幂等记录一致；本阶段未修改 EACP、跌倒/火灾阈值或 Hailo HEF。
7. [x] 实机动作验收发现原始姿态标签在站立/坐下间亚秒抖动会把活动轨迹切成大量短段。活动导出层已增加默认 5 秒稳定窗：出现/离开即时记录，短暂 `unknown` 不切段，只有稳定姿态变化才闭合区间；对应 80 条本轮伪区间已在双端备份后精确清理。
8. [ ] 继续验收真实断网补传、云端七日概览和 App 活动变化提醒，并增加跨日、多人和摄像头离线样本。

#### 阶段 37：双路视频传输与云端同步稳定性（核心修复已部署，真机观看验收待完成）

1. [x] 两路实时上传改为有界并发、持久连接、乱序拒绝和最新帧语义，避免延迟队列长期增长。
2. [x] 盒子和云端增加 10 秒/60 秒滚动 FPS、延迟、丢弃、乱序、活跃连接和事件循环指标，指标不落盘。
3. [x] 修正管理页同步状态：瞬时失败先重试，连续失败才标故障，成功恢复后清除故障；隐私轮询降至 5 秒且后台暂停。
4. [x] 将无变化的每分钟调度从全量 PostgreSQL 保存改为单行增量保存，调度日志保留 500 条。
5. [x] 活动规律评估改为每家庭 10 分钟、每轮最多一个家庭；修复时区格式器热点，4000 区间基准约 20ms。
6. [x] 云端连续调度耗时降至 7-8ms，新版本观察窗 Nginx 499 为 0；Node RSS 从约 1GB 降至 130-180MB。
7. [ ] 真机打开守护页完成 10-20 分钟前后台、切页返回和骨架模式验收，确认端到端延迟不累积。
8. [x] 完成源流诊断：两路均标称 15 FPS，但 camera 29 直连实际约 11.2-12.0 FPS、camera 30 约 14.1-15.1 FPS；TCP/UDP 差异可忽略，Hailo Pose 中位约 14ms 且失败为 0。盒子已增加实际 FPS、帧龄、抖动、读取延迟和重连指标。
9. [ ] 评估 H.264/HEVC 硬件解码 WebRTC 通道；只有在设备、网络和真机长期验收通过后替换 MJPEG，MJPEG 保留为兼容回退。
10. [x] iOS JPEG 解码与预显示准备移出主线程，定向构建和 14 项测试通过；待真机覆盖安装后纳入第 7 项长期观看验收。
11. [ ] 人物出现时记录姿态包频率、`observed/tracked` 比例、track ID 连续性和骨架贴合；不以 30 Hz Canvas 绘制率冒充模型或视频源 FPS。

#### 阶段 38：手机位置与家庭位置分离（客户端临时边界已完成，坐标闭环待实现）

1. [x] 社区服务改用长者家庭档案的城市/区县；家庭位置缺失时不回退手机位置，非紧急附近服务不可用，120 保持可用。
2. [ ] 云端家庭增加固定经纬度、地址、坐标来源和更新时间；盒子配网或家庭资料页负责确认，不允许静默覆盖。
3. [ ] iOS 使用 CoreLocation 获取手机实时位置，只用于“我与家的距离”；权限拒绝、定位过期和无网络均有明确降级状态。
4. [ ] App 以手机坐标和家庭坐标计算真实距离；社区查询始终使用家庭坐标，并通过测试锁定两类位置不可串用。

#### 阶段 28：稳定视频与 30 Hz 骨架呈现升级（代码、云端、盒子和真机包已部署，视觉验收待完成）

1. 已确认当前摄像头源流是 HEVC `15 FPS`，因此视频目标修正为按真实源帧率稳定、低延迟、无长期积压播放；不得把 Hailo 裸模型 FPS、客户端绘制刷新率或插值帧率写成 30 FPS 视频源。
2. 骨架已拆为独立的 `eacp-pose-relay-v1` 数据通道，包含 `camera_id / frame_id / captured_at / track_id / source / state`，最多 4 人、每人最多 24 个关键点。盒子默认以最高 20 包/秒发送状态变化并去重，云端通过 SSE 分发。
3. 骨架隐私播放只接受 `safe-scene-pose-v1`：安全场景是不含人物像素的完整 JPEG，云端绝不回退原画或历史截图；姿态包过期 1.5 秒后发送一次 `expired` 清场，安全场景超过 30 秒视为不可用。
4. iOS 使用同一播放票据创建安全场景流和姿态流，`TimelineView + Canvas` 以 30 Hz 绘制，在约 67ms 有界展示延迟内插值前后两个姿态包，并使用与视频一致的 aspect-fill 坐标变换。姿态流失败不拖断视频，切摄像头、前后台、隐私同步和视频 watchdog 保持原有生命周期。
5. 安全证据边界不变：所有姿态包均标记 `display_only=true / formal_evidence_eligible=false`；`tracked / coasting` 只负责连续显示和风险唤醒，正式跌倒、恢复、多模态验证和通知仍要求新鲜 `observed` Hailo 锚点及后续规则闭环。
6. 2026-07-28 已按“腾讯云服务端 -> 树莓派 -> iOS 真机”顺序部署。双摄 Hailo 姿态累计 645 次成功、0 失败，中位约 13.57ms；Pipeline 中位约 17.92ms、P95 约 67.29ms；两路姿态中继各约 295 包且上传错误为 0，安全场景上传错误为空，盒子进程约 42% CPU、RSS 约 245MB。
7. 当前仍需在 App 完成走动、快速转身、坐下、蹲下、离场和重新入场的视觉验收，并记录骨架贴合、清场时间、首帧时间、长期延迟和温度。MediaMTX/go2rtc/WebRTC 暂不引入；只有现有 HEVC/MJPEG 链路出现可复现的视频积压且收益能被指标证明时再评估。

#### 阶段 29：多模态通知决策收口（已完成并部署腾讯云）

1. 多模态严格保留人数、姿态、表面、紧急性、置信度、理由和建议事件类型等指标，所有结果均进入 SafetyIncident 与事件时间线。
2. `verifying / uncertain / rejected` 统一为 `record_only`，不创建 App 消息或通知；只有首次 `confirmed` 使用 incident 级幂等键通知一次。
3. 删除每分钟事故提醒生成器和后台历史事件补发循环。事件通知只允许在事件首次创建或首次 confirmed 时产生；关怀调度只负责每日关怀，不扫描任何旧事件补发消息。
4. 旧 `incident-reminder` 以及旧的 rejected/uncertain 结果消息在数据库归一化时归档，历史事件和证据不删除。
5. 服务端回归已覆盖 confirmed 单次投递、rejected 只入事件、uncertain 只入事件、长时间未见单次通知、调度不重推、已删除摄像头的历史事件以及数据库导出恢复。腾讯云连续调度周期已成功且 `incident_reminders_created=0 / event_alerts_created=0`。

#### 阶段 30：骨架替身隐私渲染（同帧背景重建已实机通过，实例分割按需评估）

1. 已删除骨架模式的矩形中性遮罩，新增按摄像头和输出分辨率隔离的有界背景重建器；背景状态最多 6 份，支持重置与内存指标。
2. 连续 3 帧稳定空场景才更新真实背景，单帧检测漏失不得污染背景；人物区域以背景替换后再绘制骨架。首次没有干净背景时使用场景修复兜底，人物检测存在但姿态缺失时仍隐藏人物且不伪造关键点。
3. 已覆盖背景重建、矩形填充拒绝、无姿态人物保护、首次兜底、状态隔离、LRU 上限和重置回归；盒子仅部署新增模块与隐私渲染文件，不改 EACP、跌倒规则或原始安全推理输入。
4. 已将隐私渲染输入收口为 `ContinualPoseTracker.latest_synchronized_frame()`；像素、人物框、关键点和 `frame_id` 原子对应。没有同步包时只输出可信背景或中性安全帧，不回退异步原画。人物模糊模式持续预热干净背景，按空场景锚点 ID 去重。
5. 2026-07-28 首轮真人验收暴露异步错帧：人物仍可见且骨架落在旧位置，已立即把云端、盒子和 App 全局策略降回 `person_blur`。同帧修复后再次抓取 200 帧，camera 29 横向行走过程中人物纹理不可见、背景清晰、骨架位置同步、无黑框和旧位置残留。随后已通过 App 正式播放票据和公网云中继复验，云端、盒子、App 两路策略均切回 `skeleton`，动态人物只显示随动骨架。
6. 姿态锚点交接曾出现一帧旧空背景闪回，已增加最近安全场景缓存：只缓存已经去除人物且尚未绘制骨架的场景，丢锚时复用该场景，禁止回退原画、旧人物或旧骨架。EACP idle 阶段只有 Hailo 后端 `ready`、人数为 0 且人物列表为空时，才允许刷新可信空场景；检测到人后先保持隐私兜底，再升频进入 active 姿态分析。
7. 修复重启后的 21:40 真人序列保留了 11 张人物证据帧和对应同帧姿态元数据，人物从画面边缘经过左侧、中央再离开。使用生产 `privacy-frame-renderer-v5` 复放后，11 帧均只保留随动骨架，人物衣物、面部和肢体像素不可见；前后明确无人，未触发跌倒候选，Hailo 正式推理失败为 0。
8. 盒子现有 `yolov5n_seg_h8.hef` 只作为复杂动态背景或遮挡场景的像素级轮廓增强备选；接入必须复用共享 VDevice，并用真实缺口和 Pose/Object/Seg 延迟、视频 FPS、CPU、温度及失败数证明收益，不能为了“模型更多”直接增加第三条常驻推理负载。

#### 阶段 31：正常动作误报与 confirmed-only 生产复验（正例、APNs 与恢复门禁已通过，稳定观察进行中）

1. [x] 用事件 `2161/298` 的完整证据定位误报：因子图候选错误覆盖规则层 `insufficient_descent`，并在 `0.134s` 内由两帧 fast 路径确认。
2. [x] 重新划分职责：因子图输出因素；RuleEngine 决定事件。立即云端复核必须满足 review-ready 完整门禁，非立即 fast 路径必须满足物理/持续时序证据、至少两帧和 `0.75s` 最短持续时间。
3. [x] 将现场低运动、下降不足、站/弯/躺/蹲抖动固化为回归样本；要求不生成正式事件，transition 保持 `confirmed=false / reason=insufficient_descent`。
4. [x] 原生云端服务统一为视觉事件 confirmed-only：证据缺失、模型不可用、不确定、失败和驳回只保留事件；首次 confirmed 使用事故级唯一消息和目标级幂等投递；删除分钟重复提醒。
5. [x] 完成腾讯云、盒子、PostgreSQL/SQLite 双端备份与部署；事件 `298/2161` 标记误报，两条错误消息归档，证据和投递审计保留。调度器已改从 NativeRepository 查询活动区间，并统一活动报告模块版本；视觉候选禁止走通用事件补发。生产连续三轮成功，错误补建的 `event-alert-296/297/298` 已归档且审计保留。
6. [x] 完成正常动作负例：23:51 的走动、转身、坐下、下蹲、离场和重新入场形成有效连续姿态覆盖；51 条风险上下文评估中规则候选和 EventCandidate 均为 0，边缘事件、云端事件、消息和投递新增均为 0。下蹲单帧曾被姿态头误判为 lying，但下降/时序门禁正确拒绝。
7. [x] 低风险模拟跌倒完成主要链路：事件 `2162/299` 的快速下降约 `0.893s`，躺倒姿态出现后约 `0.223s` 建立边缘事件；三帧证据自动补齐后云端以 `0.96` 置信度 confirmed，数据库保持一个模型任务、一条消息和一条投递。App 重新登记 token 后复用原投递 `34616` 获得 APNs 200，用户已确认 iPhone 实际展示，消息和投递数量仍各为 1。
8. [x] 修复正例暴露的两个结构问题：边缘 outbox 先完成证据再提交事件，云端自动归并迟到证据并恢复复核；恢复要求同轨迹明显抬升至少 `8%` 且稳定 `1.5s`，禁止两帧坐姿抖动直接结案。历史错误恢复记录保留为审计反证。
9. [ ] 连续观察 30-60 分钟，记录 Hailo Pose/Object 失败数、正式事件数、推送数、CPU、温度、节流、视频/骨架延迟和本地/COS 增量；全部通过后再冻结阶段 25/28/31 的产品交付口径。

#### 阶段 32：稳定躺倒证据延迟取帧（代码、回归、部署与历史真实序列回放已完成，部署后自然正例待验收）

1. [x] 用事件 `2173/312` 与 `2174/314` 完成恢复门禁正反例：低位坐起仅抬升 `1.70%-4.99%` 被拒绝；抬升 `19.46%` 但只稳定 `0.438s` 且边缘裁切被拒绝；另一同轨迹抬升 `18.49%` 并稳定 `1.598s` 后正确恢复。
2. [x] 定位事件 `2174` 多模态驳回根因：边缘垂直下降 `0.2177`、因子分 `0.9632` 均成立，但原 current 仍是双手撑地的过渡姿态，真正稳定躺倒帧晚约 `1.26s`，没有进入模型输入。
3. [x] 新增持久化 `event_evidence_finalize` outbox。跌倒事件立即落库，settle 后按同轨迹选择最早稳定 lying 快照，再一次性公开固定三角色媒体任务和事件任务；重启、重复执行和上传均幂等。非跌倒事件不改变。
4. [x] 使用生产 SQLite online backup 回放事件 `2174`，策略 `role-aware-post-settle-v3` 选中快照 `751000`（`20:23:40.472`），输出严格保持三张媒体与一个事件任务；生产历史未修改。
5. [x] 完成定向与全链回归并部署树莓派。备份为 `/home/gohome/gohome/backups/stage32-evidence-settle-20260729-2042.tar.gz`，部署文件 checksum 对齐，服务、双摄、Hailo、视频与姿态中继正常。
6. [x] 完成一轮正常动作反例：走动、经过双摄、在沙发附近坐下/俯身；单帧 lying 的下降只有 `0.0725 < 0.12`，边缘/云端事件、消息和通知均新增 `0`。
7. [ ] 等待自然正例验证新 finalizer 的现场链路：必须看到 same-track settled lying、三角色证据、多模态结果和 confirmed-only 单次通知；历史数据库副本回放不能替代该项。
8. [ ] 分层剖析上传延迟。先记录 finalizer settle、三媒体传输、云端 PostgreSQL/COS 持久化、事件接收和模型耗时，再评估即时唤醒、批量提交或有界并发；不得先加线程再补幂等与容量证明。
9. [ ] 完成 30-60 分钟稳定性观察，并处理历史卡在 `uploading` 的孤儿任务；清理前必须确认这些任务不对应开放事故或待补传证据，保留审计和数据库备份。
10. [ ] 将腾讯云生产权威源码归档回工作区并做结构化差异合并。当前生产 `server.js` 新于本地，禁止直接从工作区覆盖生产。
11. [x] 完成 30 分钟、7 采样点稳定性观察：服务 PID 不变、双摄活动、Hailo Pose/Object 失败均为 0，温度最高约 `59.5C` 且无节流；短暂读取超时可自行恢复。24 小时老化仍作为产品交付前独立任务。
12. [x] 复盘事件 `2175/315` 与 `2176/316`：延迟取证和云端沙发躺卧驳回正确、消息/通知均为 0；同时确认边缘存在行走者与沙发躺卧者串轨，以及 `track_identity_missing` 被因子图覆盖的问题。
13. [x] 本地完成双层身份门禁：只接受通过点数、前后向误差、几何尺度和 stale 检查的 KLT 身份桥；未经验证的突发大位移建立新轨；多人缺失身份成为 RuleEngine 硬门禁。生产反例已固化为回归。
14. [x] 双层身份门禁部署及算法现场证据通过：`21:37` 多人沙发负例零事件；`2177/317` 通过同轨快速下降、same-track settled lying、三角色证据、Qwen `0.98` confirmed、单条消息与单条投递记录。恢复能力使用既有真实正例 `2173/312` 的同轨抬升 `18.49%`、稳定 `1.598s` 证明。本项不再错误包含“APNs 真机展示”或“第二次跌倒正确去重”：用户未看到 `21:41` 通知，`21:54` 新动作被压住的根因是 App 处置未回流盒子。
15. [ ] 加固 APNs 网络异常恢复。事件 `2177` 的原投递自动三次失败后，经数据库备份、同记录恢复和服务重载获得 APNs 200，消息/投递仍各一条。需要在生产权威源码归档合并后增加可观测的连接错误、持久退避和受控重试，避免依赖人工重载，同时继续保证同一事故不新建投递。

#### 阶段 33：上传任务崩溃恢复与媒体幂等（边缘租约本地完成，部署受控）

1. [x] 定位历史 `uploading` 永久卡死根因，并逐项核对云端状态；确认事件 `2067` 的对应媒体已经存在，禁止直接 requeue 造成重复资产。
2. [x] 本地增加上传领取令牌、领取时间和过期租约；只有带明确过期租约的新任务可被重启进程回收，历史无租约任务保持隔离。
3. [x] 完成/失败写回校验领取令牌，旧 worker 的迟到结果不能覆盖新领取者；新增重启、过期领取、旧令牌拒绝和历史任务隔离回归。
4. [ ] 先把腾讯云生产权威 `server.js / postgres-store.js` 归档并与工作区做人工审查合并，禁止本地旧文件覆盖生产。
5. [ ] 云端媒体上传增加稳定幂等键：`device_id + edge_event_id + snapshot_path + evidence_frame_role`；重复请求必须返回原资产，不能重复写 COS 或 PostgreSQL。
6. [ ] 对生产 SQLite 做 online backup，为四个历史孤儿任务写入退役原因和云端核对依据，再受控退役；事件、媒体和投递审计不删除。
7. [ ] 增加每类上传任务的边缘请求耗时、云端 PostgreSQL/COS 持久化耗时和模型排队耗时；完成数据剖析后再决定即时唤醒、批量或有界并发。
8. [ ] 媒体幂等和崩溃恢复全链路通过前，不部署租约、不增加上传线程、不宣称孤儿任务问题已生产修复。

#### 阶段 34：App 处置双向同步（代码、云端、盒子和实机对账已完成）

1. [x] 纠正验收事实：投递 `34623` 的 APNs `200` 不等于真机送达；用户最新可见通知为 `21:17`，没有看到 `21:41` 事故通知。
2. [x] 定位 `21:54` 新跌倒被压住的根因：云端事件 `317` 已由 App 处理，盒子事件 `2177` 仍未确认，旧安全事故状态没有释放；该结果不能记作正确去重。
3. [x] 云端新增设备级 `event_state_commands`，使用确定性命令 ID、配置指纹、最近 24 条首次对账和设备 metadata 回执；新事件显式保存设备 ID。
4. [x] 盒子新增持久化命令幂等、逐命令回执和目标安全状态清理；只清 RuleEngine/PoseFactorGraph 事故状态，不重置视频、骨架、隐私和活动轨迹。
5. [x] 增加并通过云端命令生成/回执停止下发、盒子首次应用/重复忽略、事件落库和安全内存释放回归。
6. [x] 按云端先、盒子后的顺序部署并完成生产对账：`2177` 本地已确认，云端收到 24 条首次回执，后续配置命令数为 0；双摄、Hailo 和服务均稳定。
7. [ ] 使用独立测试通知或下一次自然 confirmed 事件验收 APNs 终端展示。不得再次重投 `2177`，不得用 APNs 200 代替用户真机可见。

#### 阶段 35：APNs 持久重试与终端回执（代码、云端和真机包已部署，独立通知待确认）

1. [x] 为单条 NotificationDelivery 增加请求超时和最多 8 次持久有界退避；无效 token 立即撤销，APNs 已接受后停止重试，不新建重复消息或投递。
2. [x] 推送 payload 增加 `delivery_id`，云端新增家庭鉴权的幂等回执接口；前台收到写 `delivered_at`，用户点击写 `clicked_at`，原始 APNs ID 和尝试次数继续保留。
3. [x] 原生 App 统一由 PushNotificationCoordinator 上报回执，并在“提醒与内容偏好”展示授权、横幅、通知中心、锁屏、声音和 token 连接状态。
4. [x] 增加正式队列的独立测试通知入口；服务端 6 项 APNs 回归和 iOS 7 项推送回归通过，腾讯云与 iPhone 包已部署，登录态和安装 ID 保留。
5. [ ] 用户只触发一次独立测试，核对一条消息、一条投递、APNs 响应和 App 回执，并确认系统通知是否可见；未得到用户可见确认前不得宣布推送闭环通过。

#### 阶段 36：真实 FPS、低积压播放与 TestFlight 通道统一（实施完成，真机验收待完成）

1. [x] 读取摄像头 CGI 能力接口并确认主、子码流只提供 `15/10/5/1`，同时记录 camera 29 约 12 FPS、camera 30 约 15 FPS 的实际源流，不再凭 RTSP 元数据推断上限。
2. [x] 盒子管理端增加右上角真实源流 FPS；App 守护页增加真实解码显示 FPS，两个指标互不冒充。
3. [x] iOS 网络层与 ViewModel 均采用最新帧有界缓冲，JPEG 后台解码，切页、切摄像头和前后台切换取消旧流；完整 121 单测和 22 UI 测试零失败。
4. [x] APNs 改为 1 秒扫描、持久 HTTP/2 会话、即时优先级 10、短退避和分段延迟指标；生产独立探针 765ms 获得 APNs 200。
5. [x] 上传 TestFlight `1.0.0 (3)`；App Store Connect 已接收并进入 processing。登录后确认 production token 替换 sandbox-only 状态仍待真机完成；不得再默认直装覆盖手机。
6. [ ] 真机并排观看管理端与 App 10-20 分钟，记录两端 FPS、可见时钟延迟、切页返回、前后台恢复和断流重连；延迟持续增长或视频卡死均判失败。
7. [ ] 从 App 只发送一次独立测试通知，核对一条消息、一条投递、服务端分段耗时和真机可见/回执；APNs 200 不等于该项通过。
#### 阶段 41：有界存储、证据归档、通知状态机与性能收口

1. [x] 盒子增加 2048 MB 运行预算和最老优先循环回收；普通分析 6 小时、已同步证据 24 小时、轻量事件 30 天、完成上传任务 7 天。
2. [x] 事件增加 `cloud_sync_status / cloud_synced_at`；未上传或不完整证据永久排除在自动回收之外，事件上传成功后才允许释放本地重媒体关联。
3. [x] 存储健康状态统计 SQLite 主文件、WAL/SHM、截图目录和可复用页；高压时执行有界多批回收，不在实时服务中自动 `VACUUM`。
4. [x] 盒子事件证据直接进入私有 COS，App 继续通过鉴权资产接口读取；云端本地磁盘只保留显式降级路径。
5. [x] 删除普通分析中重复保存的完整姿态 JSON，只保留紧凑摘要，完整姿态证据继续由快照记录持有。
6. [x] 预览、实时中继和姿态流统一使用 30 FPS 能力上限及真实新帧语义；默认预览档位为 15/24/30/24。现有 JPEG/HTTPS 中继每路使用 4 个有界 worker、持久连接、新帧丢弃和序号拒绝旧帧；不再继续加 worker，长期转 H.264/H.265/WebRTC。
7. [x] iOS 姿态插值优先使用盒子 `captured_at` 时间轴，仅用接收时刻估算传输偏移；时间戳无效或倒退时才回退到接收时间。
8. [x] 通知投递改为不可重置的幂等状态机；排队不再写送达，APNs 接受只写 sent，App 回执写 delivered，点击写 opened，并在 `/health` 暴露队列质量指标。
9. [x] 云端完整 `npm test` 通过；Node 原生服务 87 项中 86 通过、1 项因未配置本地真实 PostgreSQL 地址按设计跳过。盒子存储环、活动 outbox、活动轨迹去抖、上传租约、不可重试 HTTP 拒绝、共享解码、隐私、连续感知、跌倒、去重和配置同步回归通过；iOS 保持既有 122 单测和 22 UI 测试结果。
10. [x] 生产 `agent.db` 已备份、停服回收、离线 `VACUUM` 并重启；数据库从约 1.10 GB 降至 `289,488,896` 字节，`quick_check=ok`，事件 `completed=83 / local_only=2101 / pending=0`，未完成上传任务为 0。
11. [ ] 实机连续 20 分钟核对源流 FPS、中继提交/接受 FPS、App 显示 FPS、骨架抖动、延迟增长、CPU、温度和断线恢复；不得以 30 FPS 请求值替代真实交付率。
12. [ ] 使用 TestFlight build 3 登记 production token，只触发一次独立通知，核对一条消息、一条投递、APNs sent、App delivered/opened 与用户可见结果。

#### 阶段 42：生产数据对账、日志去抖与实时链路硬化

1. [x] 完成历史事件的 `completed / local_only / pending` 重新分类，未上传证据持续受保护，没有完整 `event_upload` 的历史记录不冒充云端成功。
2. [x] 上传队列对不可重试 HTTP 4xx 进行终止审计，消除旧恢复任务 949 次重试；网络、限流和服务端短故障继续有界退避。
3. [x] 云端调度器跳过无 ID 并发占位记录，PostgreSQL 增加空 ID 门禁，生产脏行已清理，配置同步连续失败归零。
4. [x] Nginx 关闭视频/姿态/安全场景成功上传的 access log，保留错误与业务审计；45 秒日志增长 5,245 字节。
5. [x] 活动日志增加 15 秒离场宽限，短时漏检不切断活动段；134 条积压已清零，新窗口 `activity_intervals_enqueued=0`。
6. [x] 45 秒生产采样：源流两路约 15 FPS、无重连/读取失败；云端接受约 12.6/14.0 FPS，传输 P95 约 98/93 ms，Node CPU 约 44.3%，事件循环 P95 1 ms。
7. [ ] 真机 App 连续播放 20 分钟，覆盖切页返回、前后台、骨架贴合、延迟不累积和断流恢复；同时准备 H.264/H.265/WebRTC 持久链路，MJPEG 只作降级。
8. [ ] 使用 TestFlight Release 登记 production APNs token，只发一次独立通知，核对 `sent -> delivered/opened -> 用户可见`。

#### 阶段 43：TestFlight Build 4 通知中心修复与自动通知验收

1. [x] 修复 App 前台通知只显示横幅但不进入通知中心的问题；统一展示选项为 `banner / list / badge / sound`，不改变通知去重、事件路由或服务端投递状态机。
2. [x] 增加前台通知保留到通知中心的回归；Build 4 发布门禁为 123 项单元测试和 22 项 UI 测试全部通过，Release 元数据与隐私清单检查通过。
3. [x] 上传并从 TestFlight 安装 `GoHomeShell 1.0.0 (4)`。Apple 远程签名使用 Distribution 证书和 `aps-environment=production`，Delivery UUID 为 `477a252a-4426-4b94-8b9e-55bd21a660f5`；production token 已重新登记。
4. [x] 云端主动发送独立通知，单次 APNs 尝试后由 Build 4 回传 `delivered + opened`，消息和投递均未重复，也未关联事故事件。
5. [x] 人工确认 Build 4 前台横幅在下拉通知中心仍然保留；用户点击后沿用同一投递的 opened 回执，没有产生第二条通知。后台/锁屏展示及事件深链留给真实业务通知验收。
6. [ ] 使用新的真实 confirmed 事件或已配置的定时关怀验收自动通知。运维测试只验证传输通道，不能替代产品自动触发；没有业务事件时系统不得凭空制造消息。

#### 阶段 44：受控云端主动推送验收（代码、部署和终端回执已完成）

1. [x] 新增 `GOHOME_OPS_TOKEN` 保护的 `/api/v1/internal/notifications/test`，不使用用户 session、不绕过家庭权限、不写入或重投事故事件。
2. [x] 运维测试只选择目标家庭最近活跃的 production APNs 安装；队列支持显式单 token，调度支持显式单 delivery，禁止回退为家庭广播。
3. [x] 每次请求生成独立测试消息和投递，记录 `generated_by=ops-notification-test`、安装 ID、环境和请求 ID；同一次请求仅尝试一次，真实业务的幂等键和重试规则保持不变。
4. [x] Node 原生服务 88 项回归中 87 通过、1 项因本机未配置 PostgreSQL 按设计跳过；未授权请求返回 403，无 production token 返回 409。
5. [x] 生产部署前备份服务文件和 PostgreSQL 到 `/opt/gohome/backups/stage44-ops-push-20260730-162535`。投递 `34636` 在 `16:26:44` 一次 APNs 尝试成功，Build 4 于 `16:26:48` 回传 delivered/opened，事件 ID 为空。
6. [x] 用户人工确认该前台通知在下拉通知中心可见并已点击；测试通知闭环完成。随后只用下一次真实 confirmed 事件或正常到期关怀验证自动业务通知。

#### 阶段 45：骨架独立传输与家庭固定位置（代码、云端和盒子已完成，Build 5 与现场验收待完成）

1. [x] 拆分三类帧率：原画/人像模糊显示成功解码的真实 `FPS`；骨架显示真实姿态包 `POSE Hz`；`30 Hz` 只是客户端 Canvas 插值绘制频率，三者不得互相冒充。
2. [x] 骨架模式停止逐帧上传合成骨架 JPEG，只保留低频无人安全场景和独立 EACP 关键点流；骨架跟随不再受 JPEG 上传频率限制，也不再把安全背景的 `0.x-1.x FPS` 显示为识别帧率。
3. [x] 骨架背景只允许使用已确认无人场景或中性启动场景；禁止使用带人物模糊像素的画面。骨架模式与人像模糊保持两套独立显示语义。
4. [x] 绑定盒子成功后，若家庭尚无固定位置，App 自动请求手机定位并展示确认面板；用户点击“设为家庭位置”后才保存。已有家庭位置在重新绑定时不得覆盖。
5. [x] 家庭固定经纬度持久化到云端家庭资料；首页使用“手机当前坐标 + 家庭固定坐标”计算回家距离，社区服务查询只使用家庭坐标，不回退到手机所在地。
6. [x] 自动回归已通过：iOS `123 passed / 1 skipped / 0 failed`，UI `22 passed / 0 failed`；云端家庭位置契约、隐私传输契约和盒子 Python 编译通过，Hailo Pose 中位约 `13.77 ms`、P95 约 `22.79 ms`。
7. [ ] 从 TestFlight 安装 Build 5，验收绑定后定位确认、首页距离和社区固定坐标；同时执行 10-20 分钟双摄像头切换、前后台、人物进出画面、骨架跟随和无延迟累积验收。
8. [ ] 原画和模糊要达到硬件解码与厂商 App 接近的长时流畅度，仍需将主链路升级为持久 H.264/H.265/WebRTC 或等价低延迟传输；MJPEG 只作降级，未验收前不承诺 30 FPS。

#### 阶段 46：快速跌倒漏报与摄像头瞬时掉线误导修复

1. [x] 对账盒子原始候选、姿态帧、规则状态和云端记录，确认 `22:10` 动作为快速倒地后约两秒起身；本地在 `1.634s` 后因短暂蹲姿和 track ID 切换清空候选，未形成事件，因此云端复核和 APNs 均未获得执行机会。
2. [x] 将动态低位跌倒确认窗口统一为 `1.5s`，保留帧数、低位、旋转、速度和空间连续性约束；不是无条件降低跌倒阈值。
3. [x] 单人场景允许受约束的跨 track ID 空间重关联，多人场景禁止继承；拆分站立基线和短时过渡运动，骨架明确为躺、蹲、弯腰时不再由人物框兜底写成站立。
4. [x] 摄像头离线改为至少连续 3 次失败且持续 15 秒才形成事件；单次 RTSP 超时只记重连诊断，恢复出帧后关闭未解决离线事件并上传 `camera_reconnected`。
5. [x] 管理端历史离线事件明确显示“曾发生连接中断，当前已恢复”，避免把历史事件误读为当前设备状态。
6. [x] 本地回归通过：快速跌倒在约 `1.6s` 形成事件；正常坐下、蹲下、多人换轨、沙发躺卧和边缘裁切仍不告警；摄像头瞬时失败不生成离线事件。
7. [x] 选择性部署 5 个生产文件并验证服务、Hailo、双路源流、历史离线事件恢复和状态上传，没有覆盖其他未验收改动；同时补齐云端 `camera_reconnected` 状态契约并归档旧离线提醒。
8. [ ] 用户完成一次真实快速跌倒验收：本地候选进入云端复核，`confirmed` 只通知一次，`rejected/uncertain` 只入事件不通知。
