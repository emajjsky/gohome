...
12. [ ] 执行双摄有人、遮挡、快速移动、离开、宠物和电视内容实测，目标活跃人物 Pose 每路约 `10-15 Hz`；记录 Hailo 队列、合并/替换、阶段 P50/P95、CPU、温度、Tracker 锚点 Hz 和真实成品 FPS。空场实测已定位单帧原始 Pose 反复延长 active 的根因；已拆分“候选正式验证”和“Tracker 快速重锚”，部署后 Scheduler 稳定保持 idle。持续电视假候选的正式拒绝进一步改为 `10/20/40/60 秒` 上限退避，待 Pi 再验证验证频率和真人进入延迟。
