...
这份 Plan 定义 `回家` 从当前 V1 家庭试点候选版走向可交付产品的实施路径。
...
当前已有的 `edge-agent`、管理台、云端 API、正式 iOS App 和 YOLO/Hailo 检测，是 V1 试点候选版的技术基线。Mac 只作为开发对照环境；当前主验证对象是树莓派盒子、生产云和 Build 15 真机链路，目标是完成三端可追踪、可恢复、可验收的交付闭环。
...
7. [x] 正式 GoHome App 首轮确认纯骨架可见，用户经过两路摄像头并离开后主观流畅度基本可接受；结构化证据显示人物合成完成且离开后目标清零。
8. [ ] 在同一 App 会话连续切换 `original -> person_blur -> skeleton`，每路确认画面、模式、FPS、帧龄、同步状态和无残影，再进行遮挡与断流验收。
9. [x] 正式 App 已完成原画、模糊、纯骨架连续切换，用户确认三种模式均流畅；盒子两路模式状态保持 `ready`，源流约 `15.2 FPS`，无读取/重连/发布失败。
10. [ ] 在纯骨架模式执行遮挡、快速移动、离开后立即再次进入，以及摄像头断流和自动恢复；同时记录编码输入 FPS 长尾，不能只以主观流畅关闭性能门槛。

## 15.65 断流恢复闭环

状态：代码修复完成，等待 Pi 实机验证。

1. [x] 将空分割锚点与有人但 Pose 不同步拆成两类证据；空分割可以累计复验，有人证据必须同帧同步。
2. [x] 增加隐私复验回归，验证流代变化后空房恢复、旧 Pose 不复用和有人证据继续阻断。
3. [x] App 保留有限快速重试，失败后按 `5/10/20/30s` 退避持续重建 WHEP 会话。
4. [x] 增加 App 持续恢复测试；守护定向测试 `13/13` 通过。
5. [ ] 精确部署盒子运行文件，拔出一路摄像头并确认另一路不中断。
6. [ ] 插回同一路后确认盒子复验恢复、H.264 发布恢复、App 自动重协商恢复，不退出 App、不删除摄像头、不重新绑定。
7. [ ] 验证连续断流、前后台切换、切路和模式切换没有重复 WHEP 会话或旧画面。

Build 12 已上传 App Store Connect，等待 TestFlight 处理后执行上述真实断流恢复验收。

## 15.66 在线状态与隐私成品状态分离

状态：现场根因已定位，等待机位确认。

1. [x] 证明摄像头 `31` 源流在线、解码约 `15 FPS`，但隐私发布器因 `scene_review_required` 暂停。
2. [x] 证明摄像头 `32` 同期可以发布，排除盒子全局服务、Hailo、云端和 App 账号整体故障。
3. [ ] 确认电视柜摄像头是否在插拔时发生真实角度变化。
4. [ ] 无人时恢复原角度或重新确认该路空房基线；不删除摄像头、不重新绑定。
5. [ ] 将管理端、云端会话和 App 状态统一为源流、隐私复核、成品发布、WHEP 可读四级状态。
6. [ ] 增加“在线但实时不可用”的自动回归，确保用户看到明确原因而不是永久连接中。

## 15.67 可转动摄像头的多机位隐私基线（2026-08-06）

状态：本地实现和专项回归完成，等待部署到 Pi 后做真实云台转动、断电重连和 App/管理端验收。

1. [x] 将单一背景扩展为每路最多 4 份用户明确确认的机位基线；每份基线按内容身份持久化，旧版无后缀文件继续兼容。
2. [x] 重连或画面变化时先匹配已知机位；命中后切换活动基线并重新进行有限帧复验，不能把流代变化误当成永久校准失效。
3. [x] 未知机位只允许原画/人物模糊继续使用，纯骨架返回结构化“新机位待空房确认”；不自动学习有人画面，不覆盖旧基线。
...
## 15.68 摄像头源切换失败边界（2026-08-07）

状态：GH-022 代码根因已修复，自动回归通过，待树莓派现场验收。

1. [x] 旧共享 reader 停止返回明确成功/失败；失败时不移除所有权、不清缓存、不发送已完成配置变更、不创建第二个 reader。
2. [x] 配置切换期间新的抓帧和订阅请求不得旁路新建采集；必须返回“旧 reader 仍在退出”的结构化错误，等待下一次配置对账。
3. [x] 云端 `setup_required` 记录会禁用旧本地源；完整 URL、账号和密码同步后，按原摄像头 ID 恢复托管采集。
4. [x] 专项回归通过：共享流、配置同步、断流恢复、直播中继、Python 编译和差异检查。
5. [x] 提交 `1094411` 已精确部署 Pi 两个运行文件；旧文件已备份，服务重启后 `active`、`NRestarts=0`，两路发布器和隐私成品均恢复。
6. [ ] Pi 实机修改 DHCP 地址或 RTSP 地址，确认旧地址无新连接尝试、新源首帧前不报在线，管理端、云端和 App 状态一致。
7. [ ] Pi 实机拔出/插回一路摄像头，确认另一路不中断，插回后不重绑、不删摄像头、自动恢复隐私成品流和 App 播放。

## 15.69 2026-08-10 参赛提交与交付边界

本节作为当前执行顺序的收口基线，优先级高于早期仍停留在 Build 7、旧 MJPEG 或“定位未接入”口径的历史计划段落。

### 已完成并可作为当前材料依据

- 正式 GoHome iOS 工程唯一入口为 `ios-shell/GoHomeShell.xcodeproj`，Bundle ID `com.gohome.family`，`1.0.0 (15)` 已归档并上传 App Store Connect。
- 首页、守护、记忆、社区和我的主流程，以及设备/摄像头/事件/消息/媒体核心契约已纳入当前工程；首页截图使用 Build 15 证据，不再使用旧首页截图代表当前产品。
- 双摄实时媒体已收敛为盒子一次隐私合成、H.264 发布、MediaMTX/WHEP/WebRTC 播放；旧正式逐帧 JPEG/MJPEG App 链路不再作为主路径。
- iOS 单元测试 `194/194`、UI 测试 `28/28`、Node 服务门禁、Python 编译和差异检查通过。

### 仍必须保留为待验收或后续规划

- 真实家庭连续运行的误报率、漏报率、P95 告警时延、通知到达率和家属响应时间。
- 双摄三种隐私模式的 30 分钟长时、前后台、Wi-Fi/蜂窝、断网/恢复、摄像头地址变化和已知/未知云台角度验收。
- 猫狗独立标注集与混淆矩阵、火灾/烟雾困难负样本，以及按家庭隔离的 train/validation/test 数据集。
- APNs 生产能力、正式短信验证码、量产二维码、恢复出厂 SOP、跨品牌 RTSP/ONVIF 兼容清单和规模化运维指标。

禁止为了参赛材料把以上待验收项写成“准确率已达标”“系统已完成救援闭环”或“所有摄像头兼容”。

## 15.70 2026-08-11 Build 16 四帧事件验收收口

### 已完成的实现与发布动作

1. 统一 `before / transition / evidence / current` 四个证据角色，要求四张不同时间和资产身份的真实帧；不足四帧时不复制、不调用多模态、不通知。
2. 将百炼复核收敛为一次请求中的有序视频序列，使用严格 `gohome-vision-verification-v2` 四级结果：`confirmed`、`suspected`、`rejected`、`uncertain`。
3. 将事件动作收敛为单一状态机：前两级进入 App 待处理且同一事件只通知一次；后两级进入系统记录，不通知、不要求确认、不提供误报操作。
4. 盒子管理端和 iOS App 复用同一事件证据关系和四帧角色，不复制图片、不跨摄像头拼接、不由客户端二次绘制。
5. 代码已部署盒子和生产云；唯一原生工程已归档为 `1.0.0 (16)`，App Store Connect 已处理并加入内部组“比赛内测”。

### 必须完成的真机门禁

- 用真实动作产生一条完整四帧事件，核对四帧顺序、四个角色、摄像头归属和时间线。
- 用正常坐下、弯腰或宠物活动产生安全复核，确认只进入系统记录，不产生通知和待处理按钮。
- 用受控快速跌倒产生疑似或确认危险，确认 App 只收到一次通知，事件进入待处理并能完成确认安全/标记误报。
- 重复刷新 App、管理端和事件详情，确认同一事件不新增通知、不重复资产、不出现旧事件残影；App 与管理端的证据画廊完全一致。
- 真机门禁通过前，相关台账保持“待实机验收”；不得把自动测试、云端健康或 TestFlight 可安装写成跌倒准确率和通知闭环已达标。

## 15.71 2026-08-12 初始化与重连隐私状态解耦

状态：代码和完整自动回归通过，进入盒子与生产云部署验收。

1. [x] 将媒体可播放状态与纯骨架背景就绪状态拆开，兼容字段只反映真实媒体发布，不再由校准状态强制置为离线。
2. [x] 原画和人物模糊在家中有人且无空房背景时继续使用当前唯一采集帧，不等待纯骨架校准。
3. [x] 纯骨架未就绪时保持原 H.264 发布器，输出无摄像头像素的 `privacy_hold`，不泄露、不降级、不返回播放协商失败。
4. [x] 从唯一共享帧链路自动观察空房；人物证据清零候选，稳定空房自动提交，流代变化重置未完成候选，手动校准可接管。
5. [x] 盒子 `63/63`、云端、iOS 单元 `196/196` 和 UI `28/28` 回归通过。
6. [ ] 部署盒子和生产云，确认两路媒体、Hailo、自动校准状态与服务日志正常。
7. [ ] 实机在有人时验证原画/模糊立即可播，纯骨架只显示隐私准备；无人后不退出 App、不重绑摄像头，自动恢复纯骨架。
