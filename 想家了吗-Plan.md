...
本轮确认此前盒子并非“算法运行但没有命中”，而是由于 Python 3.13 环境缺少 YOLO 依赖，运行配置曾临时降级为 `basic`。basic 只支持画面质量、运动、活动和烟火等基础信号，不能输出真实人数和人形框；页面算法开关只代表规则配置，不能代替模型运行状态。
...
3. 跌倒目前仍是 YOLO 人框比例、低位区域和时间窗形成的候选判断，不是姿态模型结论。
4. 下一项算法工作应单独完成轻量姿态模型 POC、误报测试和 Pi 性能验证，再决定是否启用 `GOHOME_POSE_ENABLED`；不能仅因为 YOLO 已恢复就宣称跌倒算法完成。
