...
#### X. 阶段 23：EACP 事件自适应连续姿态感知（方案已确认，未实施）
...
1. **P0 只读基线与契约测试**：记录每路帧龄、推理耗时、模型锚点 FPS、调度模式、CPU、温度和丢弃帧；先证明旧 worker 的固定休眠和双摄串行问题，不改变报警行为。
2. **P1 最新帧独立调度**：把整轮双摄加固定 sleep 改为每摄像头 deadline 和统一优先队列；空闲约 1 FPS、活跃约 2 FPS、风险目标 3-5 FPS，吞吐不足时自然降级但不得处理历史帧。
3. **P2 推理链去重**：A/B 对比每摄像头 PoseTracker 与 YOLO 人框直送 RTMPose ROI，优先消除 RTMLib 内部重复人体检测；冻结原姿态分类和跌倒回归集后再切换。
4. **P3 连续跟踪**：接入 OC-SORT 人框和 KLT 关键点短时传播，增加前后向误差、骨骼几何、置信度衰减和约 600ms 过期门；管理页分别绘制模型锚点和跟踪补帧。
5. **P4 风险升频与事件整合**：由运动突变、人体中心快速下降、人体框高度变化和姿态因子触发短时升频；边缘临时候选先进入 App，云端三图复核继续异步更新同一 SafetyIncident。
6. **P5 数据驱动增强**：申请数据集到位后训练轻量 TCN/Continual ST-GCN，输出下降、倒地和恢复趋势，与 PoseFactorGraph 并行；不直接部署 SkateFormer、CoTracker 或 TAPIR。
7. **P6 加速后端评估**：只有 CPU 路线未达到双摄延迟和温度目标时，才对 RTMO、Hailo YOLO 或兼容姿态模型做同帧 A/B；业务规则不得绑定 AI HAT+。
...
阶段 23 当前状态：仅完成论文、现有代码和实机负载审计，方案已由用户确认；尚未修改 worker、推理链、跟踪算法或树莓派部署。下一步先完成 P0 基线与契约测试，经回归后再进入 P1，不允许通过直接把 `capture_interval_seconds` 从 5 改为 1 冒充 EACP。
