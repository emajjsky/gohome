...
3. App/H5 生成设备绑定码或绑定凭证。
4. 树莓派 `.env` 配置云端 `GOHOME_APP_SERVER_BASE_URL` 和 `GOHOME_DEVICE_API_TOKEN`。
5. 重启 `gohome-edge-agent` 后，云端能看到设备心跳。
6. App/H5 提交摄像头配置后，云端 `config_version` 变化。
7. 树莓派拉取新配置，应用到本地摄像头配置，并回传同步状态。
8. 触发一条真实事件，确认树莓派本地 `upload_jobs` 从 `pending` 变成 `completed`。
9. 手机离开老人家局域网后，App/H5 仍能通过云端看到事件列表、事件详情和证据图。
