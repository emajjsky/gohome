...
2. 更新三份文档后提交并推送 `feature/native-ios-app`。腾讯云只部署本批服务端最小差异，不修改盒子视觉、EACP、跌倒、火灾或隐私流代码。
3. 云端验收必须新建一次性临时账号和临时家庭，依次验证导出、阻断计划、普通成员注销、独立创建者注销、旧 token 401 和盒子解绑；禁止删除用户现有真实账号或真实家庭。
4. 家庭创建者转移 API 与原生入口是多人家庭注销的下一项依赖。本批只提供正确阻断，不把“请先转交”描述成已经可完成的闭环。

后续顺序更新为：部署账号数据接口并用临时账号验收 -> 真机覆盖安装验证系统分享和注销回登录 -> 家庭创建者转移闭环 -> 付费 Apple Team 下的 APNs、Archive 和 TestFlight。视觉算法继续由原主线维护。
