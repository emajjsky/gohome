...
9. [ ] 进入 GH-030：建立唯一共享 Pose 调度所有者，把高频当前帧 Pose 锚点与低频完整产品分析拆开；每种 HEF 仍只有一个 runtime，显示锚点不得直接升级为正式风险证据。
10. [ ] 在 GH-030 结构完成后执行双摄有人、遮挡、快速移动和离开实测，目标活跃人物 Pose 每路约 `10-15 Hz`，并记录 Hailo 队列、阶段 P50/P95、CPU、温度和真实成品 FPS。
