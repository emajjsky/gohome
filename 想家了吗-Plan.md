...
#### X. 阶段 23：EACP 事件自适应连续姿态感知（P0/P1 已完成，P2-P6 待实施）
...
阶段 23 当前状态：P0/P1 已完成并部署真实树莓派。关闭算法页时双摄 idle 实测约 0.99-1.15 FPS/路，单次 YOLO 锚点约 0.14-0.17 秒，帧龄约 0.16-0.24 秒；两分钟处理 261 个锚点但只持久化 48 条，双路视频约 7.97-7.98 FPS。两分钟窗口温度最高 69.2 摄氏度，后续模型短时活跃时瞬时 73.6 摄氏度并回落至 65.9 摄氏度，当前无热降频，worker 无错误。下一步进入 P2 推理链去重；当前尚无 OC-SORT/KLT，真实人物 active/risk 频率和 1.5-3 秒候选延迟仍需后续验收。
