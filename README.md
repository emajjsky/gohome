...
- 当前发布候选：`1.0.0 (17)`，事件处理、商品图片、回家记录/计划和今日关怀事实闭环已进入正式工程；App Store Connect 已接收上传并开始处理
...
空房背景只服务骨架模式。首次接入或重连时，即使家中有人，原画和人物模糊也必须立即可用；骨架背景未就绪时保持同一个 H.264/WHEP 会话并输出不含摄像头像素的隐私准备帧。盒子随后从唯一共享采集链路自动观察稳定空房，有人证据清零候选，无人达到确认帧数后原子提交并恢复纯骨架，不要求删除摄像头、重新绑定或重启 App。
