...
- 新增轻量 `video_privacy.py` 统一模式校验和隐私等级，不让普通配置同步间接依赖 NumPy/OpenCV；`privacy_stream.py` 只负责 JPEG 解码、人物区域强模糊、整帧安全回退和暗背景骨架渲染。
- 隐私处理只接在本地管理流和云端实时帧上传前。正式视觉 worker、连续姿态跟踪、跌倒/火灾规则、事件截图和多模态验证继续使用原始采集帧，本批未修改任何算法阈值或事件状态机。
- 人像模糊使用连续跟踪元数据扩大人物框；没有可信人物框时整帧模糊。骨架模式在强模糊暗背景上绘制跟踪关键点。非原画渲染异常时丢弃该帧，不能以原始 JPEG 兜底。
- 本地普通视频流和同步姿态流都受家庭模式最低等级约束；算法页在非原画模式下切到隐私流。管理端首页、摄像头、事件和算法页面统一使用 `20260727-privacy-2` 脚本版本，避免页面加载不同缓存。
...
- 腾讯云和盒子代码已启用；真机覆盖安装尚未执行。当前钥匙串开发证书属于 Team `F8KVW83HC2`，Xcode 未登录该团队，而现有描述文件属于 Team `CPRVX9XK47`，无法交叉签名。添加匹配 Apple ID 后再覆盖安装，并由用户在 App 与管理端各切换一次完成最终双向可见性验收；P5 在此之前保持“云端/盒子完成，真机点击待验收”。
