...
- `PrivacyBackgroundReconstructor` 在复用历史背景前比较当前镜头可见区域，允许全局亮度偏移，但拒绝结构差异过大的背景。拒绝后删除该路内存状态，仅用当前帧局部修复。
- `PrivacyFrameRenderer` 校验同步姿态包的 `camera_id`，并提供单路缓存重置；`LiveRelayAgent` 以流地址、用户名、密码和启用状态形成配置签名，签名变化时先停旧线程、清缓存，再启动新线程。
- 自动化主动将 B 路异色背景写入 A 路缓存，验证输出拒绝 B 路像素、统计一次不兼容拒绝并能完整重置。修复已部署盒子，重启未更改数据库、摄像头配置和用户数据。

### 宠物稳定

- 当前 Hailo-8 只有可用的 `yolov8s_h8.hef` 通用 COCO 检测模型；设备上的 H10/H8L HEF 不与当前 Hailo-8 硬件混用。
- 新增独立 `PetTemporalStabilizer`：`0.28` 以上猫狗作为候选，同位置至少两次命中才输出；猫/狗使用衰减加权投票，单帧错类不翻转已确认类别，短时漏检最多保留 2.2 秒。
- 轨迹以摄像头 ID 分区，不与人体检测或跌倒证据混用；新增独立回归验证单帧不显示、两帧确认、单帧错类不翻转、短时保持、过期删除和双摄隔离。
- 本批改善通用检测的稳定性，不等于已解决家庭猫狗的细粒度准确率。后续需以两路真实视角收集清晰的正例、漏检和猫狗互相误分样本，再决定是否训练宠物专项分类头或替换检测模型。
