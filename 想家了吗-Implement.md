...
- 新增 `app-state-store.js`，为首页、守护、事件、陪伴、我的保存最近一次真实 `<main>` 渲染和滚动位置。页面返回时先恢复真实内容，再后台刷新；首次无快照只显示一个页面级加载状态。快照会清除表单值、临时弹窗和带播放票据的视频/图片地址，账号切换或退出登录时整体清理。
...
- 本地浏览器验证五个主页面首次渲染后均为真实状态；二次进入约 36-101ms 显示快照，没有模块级“正在读取/同步中”，无横向溢出和控制台错误。守护双摄卡片返回后保留真实数据状态并重新连接视频。
- 腾讯云已同步全部版本化 HTML 和前端脚本；`gohome-app` 重启后 active，HTTPS 健康检查返回 PostgreSQL store，线上首次与二次进入首页均无模块级加载文案和控制台错误。
