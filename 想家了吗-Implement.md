...
这份文档记录 `回家` 从 V1 试点候选版继续收口时的真实实施进度、当前运行方式、已完成能力、未完成事项和下一步开发记录。

PRD 负责定义产品边界，Plan 负责定义实施顺序，Implement 负责记录“现在到底做到哪里了”。
根目录 `README.md` 负责开发者入口和当前基线索引，不替代本文件的部署证据和未关闭条件。
...
## 1.1.1 2026-07-21 原生 iOS 支线决策（历史基线，当前状态见第 123 节）

本节记录原生迁移开始时的决策。后文关于 H5、陪伴页和 SwiftUI + WKWebView 壳的内容属于已完成历史验证；导航和下一阶段产品口径已经由第 123 节更新。

已确认但尚未实现：

- 正式比赛交付切换为最低 iOS 16 的原生 SwiftUI App。
- 当时家属端五个主导航调整为首页、守护、事件、精选和我的；该历史导航已于 2026-07-23 被首页、守护、记忆、社区和我的替换。
...
- 近黑帧不进入实时输出；连续达到确认条件后主动释放并重开 RTSP。后续 GH-029 已删除旧画面保持，正式黑屏或遮挡由独立质量状态审计，不能依赖伪实时预览。
- 管理页在视频请求仍然失败时使用 `0.8s` 起步、最大 `8s` 的指数退避自动重连。
- 新增 `verify-camera-stream-resilience.py`，验证读取失败后重开、近黑帧抑制和后续有效帧恢复；GH-029 增加中央缓存写入与帧身份审计。
...
- 读取失败与持续近黑均由共享读取器统一重连；GH-029 已移除原有效帧保持逻辑，任何订阅者都不得把旧像素作为实时帧继续输出。
...
## 123. 2026-07-23 原生 App 初步跑通与下一阶段产品化记录

### 当前真实完成状态

- 原生 SwiftUI App 已安装到真机，账号、家庭、引导、双摄像头配置和云端 PostgreSQL 数据能够读取。
- 手机离开家庭局域网后仍通过腾讯云访问；两路盒子实时帧持续上传，云端可以创建播放票据并输出移动端 MJPEG 流。
- 移动端视频已增加最新帧优先、云端背压丢弃旧帧、页面离开释放流、4 秒无帧看门狗和自动重连。30 秒 150 KB/s 限速验证中，画面时间跨度约 29.55 秒，不再持续积累旧帧。
- 2026-07-23 发现云端主 `server.js` 曾被旧部署文件覆盖，导致原生 `/api/v2/app/bootstrap`、`/api/v2/home` 和 `/api/v2/products` 返回 404。已重新部署完整服务器文件并验证首页返回天气、17 条资讯、2 路在线摄像头和真实产品目录；PostgreSQL 事件和媒体数据未清理。
- 当前导航和页面代码仍是上一版“首页 / 守护 / 事件 / 精选 / 我的”，尚未按新产品决策改造成“首页 / 守护 / 记忆 / 社区 / 我的”。

### 尚未完成

- 消息仍未完整形成“异常或生活信号 -> 关怀话题 -> 编辑 -> 系统分享/微信 -> 已联系/稍后提醒”的用户闭环。
- 守护、今日活动轨迹和事件尚未合并；普通活动区间、夜间活动和个人基线还没有形成正式日报/周报。
- 记忆模块尚未建立数据表、媒体上传、家庭权限和原生页面。
- 原画、人像模糊、骨架隐私三种模式尚未进入正式 App 设置；本地人脸特征方案尚未实施。
- APNs、TestFlight 发布验收和真实社区服务接口尚未完成。
- 26 TOPS Hailo-8 AI HAT+ 尚未到货和接入；当前视觉运行仍使用 CPU 路线。ESP32 未接入，仅保留备选。

### 下一执行批次

1. 先完成首页关怀行动闭环的数据契约、服务端动作记录和原生交互。
2. 再合并守护、轨迹和事件，避免继续维护两套安全信息结构。
3. 建立普通活动日志的盒子聚合、云端定时分析、家庭开关和数据留存策略。
4. 完成记忆 MVP，再进入隐私画面、社区服务和 Hailo 加速适配。

本节只确认产品已“初步跑通”，不宣称上述产品化能力已经完成。

## 124. 2026-07-23 原生家庭记忆 MVP 与五栏产品导航

产品结构：

- 原生底栏调整为“首页 / 守护 / 记忆 / 社区 / 我的”；事件不再独占底栏，通过守护页的“守护记录”进入。
- 记忆定位为家庭私有时间流，不是公开社交广场。页面采用缓存优先和静默刷新，切换 Tab 时不清空已有内容，也不展示阻塞式整页读取态。

记忆闭环：

- 新增 `family_memories / family_memory_media / family_memory_comments / family_memory_favorites` 四张家庭归属表和 `007_family_memories.sql` 幂等迁移。
- API 支持家庭时间流读取、发布、编辑、删除、评论、收藏和私有图片上传；所有读写先验证家庭成员身份，编辑和删除仅允许作者或家庭创建者执行。
- 原生编辑器支持正文、最多 9 张图片、发生时间、地点和人物；编辑时可保留或移除已有图片。删除记忆或替换图片时同步删除不再引用的家庭记忆文件与媒体记录。
- 时间流展示作者、时间、地点、人物、多图、评论和收藏；“这一天”只使用真实历史记忆，不生成虚构内容。私有图片通过带登录令牌的请求加载，并使用 80 张、约 48 MB 上限的内存缓存减少重复读取。
- 创建、编辑、评论、收藏和删除成功后直接更新本地时间流与磁盘缓存，不为每次动作重新加载整页。

验证与部署：

- Node 定向测试 9 项通过，覆盖迁移约束、家庭隔离、跨家庭图片拒绝、完整记忆生命周期和替换图片清理。
- iOS 原生工程编译通过；新增记忆解码、缓存优先和完整写操作生命周期测试。全量 59 个单元测试为 0 失败，其中 1 项因模拟器测试宿主缺少 Keychain entitlement（`-34018`）明确跳过；真机仍保留完整 Keychain 读写断言，缺少权限会继续判定为失败。
- 守护记录改为由主 Tab 的统一 `NavigationStack` 驱动，守护组件只发送打开事件的命令，避免子页面维护第二套路由状态。事件动作与五栏持久化 UI 回归共 4 项全部通过，覆盖事件详情、确认安全、社区原生页和 Tab 往返不卸载首页内容。
- 腾讯云已应用 `007_family_memories.sql`，`gohome-app.service` 恢复 active，`/health` 确认仍为 PostgreSQL store。
- 使用家庭 `2` 完成真实云端冒烟：图片上传、记忆创建、更新、列表读取、删除均成功，删除后私有媒体返回 404；冒烟记录已删除，没有改动现有家庭、摄像头、事件和历史媒体。

后续边界：记忆生成关怀话题、已有图片拖拽排序、服务端缩略图与 EXIF 清理、TestFlight 真机照片选择验收仍属于下一批次。本节不宣称这些能力已完成。

## 125. 2026-07-23 原生守护信息结构收口

页面结构：

- 守护页按产品文档合并为“实时 / 轨迹 / 事件”三个分段，不再从实时画面跳转到第二个重复的事件页面。
- 事件列表抽成同一份可复用原生内容，继续支持待处理、已处理、误报分段、证据详情、确认安全和误报反馈，没有建立第二套事件状态。
- 轨迹分段当前只提供诚实空态，明确只记录房间、时间和可验证活动区间；在盒子普通活动聚合和云端轨迹接口完成前，不生成吃饭、睡眠或健康状态等推测内容。

资源与体验：

- 只有“实时”分段挂载摄像头舞台并维持 MJPEG 会话；切换到轨迹或事件会立即停止视频流，返回实时后按当前选中摄像头重新建立会话。
- 摄像头切换、App 进入后台、页面离开和恢复前台继续使用同一个 `GuardViewModel` 生命周期，不增加并行拉流或重复播放器。
- 待处理事件数量保留在守护页标题区，事件成为守护内部的待办证据，而不是独立主导航。

验证：

- iOS 工程编译通过。
- 全量 59 个单元测试为 0 失败、1 个模拟器 Keychain entitlement 跳过。
- 守护与五栏 UI 回归 4 项通过，覆盖实时、轨迹、事件切换，确认非实时分段不再挂载摄像头舞台。
- 事件详情动作回归通过，确认事件嵌入调整没有破坏证据详情和“确认安全”乐观更新。

未完成边界：P2 的页面信息结构与视频资源边界已完成；真实“今日轨迹”仍依赖 P3 的盒子普通活动区间聚合、断网幂等补传、云端小时聚合、权限开关和留存删除策略，当前不能表述为活动轨迹产品闭环已经完成。

## 126. 2026-07-23 活动区间云端契约与原生今日轨迹

数据契约：

- 普通活动采用 `ActivityInterval` 区间上传，不上传逐帧检测日志。字段只包含设备、摄像头、房间、开始/结束时间、最高人数、观察姿态、置信度和受控 metadata。
- 每条区间使用 `device_id + source_interval_id` 唯一约束，盒子断网重试不会制造重复轨迹；单批限制 1 到 100 条，单区间最长 6 小时，拒绝倒置时间、远期时间和非法置信度。
- 轨迹只陈述“某房间在某时段有人活动”和观察到的姿态，不根据人物出现推断吃饭、睡眠、精神状态或健康结论；普通活动区间不包含截图。

云端：

- 新增 `008_activity_intervals.sql` 和 `activity_intervals` 表，按家庭与时间、设备与时间建立索引。
- 新增设备认证接口 `POST /api/v1/device/activity-intervals`，家庭归属只从已绑定设备 token 或服务端设备记录解析，盒子请求不能自行指定其他家庭。
- 新增家属接口 `GET /api/v2/activity-timeline?family_id=...&date=YYYY-MM-DD`，继续使用家庭成员权限、ETag 和稳定视图字段。
- JSON 测试仓储与 PostgreSQL 正式仓储同时实现，避免本地和上云行为分叉。

原生 App：

- 守护“轨迹”分段接入类型化今日轨迹接口，并按账号、家庭和日期使用磁盘缓存；缓存先显示，网络在后台静默刷新。
- 有真实区间时使用时间轴展示房间、时段、人数和标准姿态；无数据时保留真实空态。切到轨迹仍停止实时视频流。
- 轨迹加载失败但有缓存时继续展示上次数据，并以非阻塞状态提示，不出现整页读取态。

验证与部署：

- Node 语法检查、完整项目校验和活动接口定向测试通过；定向测试覆盖设备幂等、家庭查询、事实字段、ETag 和非法时间拒绝。
- iOS 工程编译通过；单元测试增至 61 项，0 失败、1 个模拟器 Keychain entitlement 跳过；守护分段和事件详情 UI 回归通过。
- 腾讯云已应用 `008_activity_intervals.sql`，首次迁移因错误引用 `devices(id)` 被事务回滚，核对真实 schema 后改为 `devices(device_id)` 并成功提交。`gohome-app.service` 为 active，`/health` 返回 PostgreSQL，新增表当前 0 行，没有写入测试轨迹。

未完成边界：本批完成云端接收、持久化、家庭查询和原生展示。树莓派将 `presence_sessions / posture_episodes` 聚合为普通活动区间、5 到 15 分钟批量发送、断网有界队列、家庭开关、留存删除、小时/日/周摘要仍需在视觉主线按该契约实现，不能宣称 P3 全链路完成。

## 127. 2026-07-23 原生记忆图片体验收口

保存性能：

- 照片选择后在后台并行读取，并使用 ImageIO 按最长边 1280 px 降采样、统一输出 0.7 质量 JPEG；每个图片槽位先以稳定 UUID 建立，再按完成顺序渐进显示，删除待处理图片不会让异步结果写入错误位置。发布按钮在全部照片处理完成前保持禁用，避免快速提交遗漏图片。
- 新增 `POST /api/v2/memory-media-batch`，一次请求接收 1 至 9 张私有图片，保持用户选择顺序，所有文件写入后只执行一次 PostgreSQL 差量持久化；不再让每张图片分别触发一次完整 `store.save()`。旧单图上传接口保留兼容回退。
- 批次原始图片总量限制为 12 MB，使 Base64 JSON 在现网 Nginx 20 MB 请求体限制内稳定通过；单图仍限制 4 MB。失败时同时回滚文件和内存媒体记录，不留下半批数据。
- 发布成功后，原生端直接把已压缩的本地图片写入原图与网格图内存缓存，再关闭编辑器；时间流不需要重新下载刚发布的图片。相同图片的并发读取由缓存合并为一个在途任务，避免 SwiftUI 视图重建制造重复 GET 和 499 取消请求。

图片布局：

- 编辑预览和已发布时间流共用同一套布局规则：单图 4:3，2 张双列，3 张三列，4 张 2x2，5 至 9 张三列，最多显示九宫格。
- 每个格子先由固定宽高比容器确定尺寸，图片只在容器内部裁切；草稿预览图只解码一次，不再在每次 SwiftUI 刷新时重复解码，从结构上消除异步加载导致的重叠、跳高和滚动卡顿。
- 已发布图片支持点击进入全屏预览，多图可左右翻页；私有图片仍通过原有登录令牌和内存缓存加载，没有增加公开图片地址。
- 时间流请求 `variant=grid`；旧的大图由服务端按需生成最大 1280 px WebP 并使用 ETag 与 7 天私有缓存，新原生上传的 1280 px JPEG 直接返回，不再额外同步转码。全屏预览仍请求原图。

验证：

- 新增网格规则、9 张上限、批量上传顺序和单次持久化回归；记忆定向测试 6 项、Node 全量 27 项全部通过，其中 PostgreSQL 集成项因本地未配置测试数据库跳过 1 项。
...
- 家属读取图片时仍先请求受保护的 `/api/v1/video/assets/:id`。视频播放先请求 `/api/v2/memory-media-playback/:assetId`，服务端验证登录和家庭权限后签发五分钟私有 GET 地址，`AVPlayer` 直接使用 COS Range 请求流式播放；数据库和 App 时间流中不保存永久公开链接。
...
- 一分钟内视频已经完成：原生端使用 AVFoundation 导出最高 720p MP4，限制 60 秒和 24 MB；一条记忆只能是最多 9 张图片或 1 个视频，不能混传。视频复用同一套签名直传、完成校验、私有读取和删除生命周期。
...
## 129. 2026-07-24 家庭记忆视频与私有 COS 流式播放

### 原生视频处理

- 记忆编辑器支持从相册选择一个视频，与图片选择互斥；图片仍最多 9 张。选择后先读取真实时长，超过 60 秒立即拒绝。
- 视频通过 AVFoundation 导出为最高 1280x720 MP4，使用 H.264/AAC 兼容路径并启用网络优化；压缩后超过 24 MB 不上传。首帧作为本地预览，发布后无需重新下载视频生成封面。
- 上传申请把内容类型、字节数、像素尺寸和时长写入独立 HMAC 令牌。服务端只接受 JPEG/PNG/WebP 图片或 MP4 视频，并在入口与 JSON/PostgreSQL 仓储层统一拒绝图视频混合和多视频记忆。
- COS 直传或完成确认失败时，App 调用 `/api/v2/memory-media-upload-abort` 清理未登记对象；旧 Base64 接口只作为图片兼容回退，视频不会静默走旧路径。

### 播放与持久化

- `/api/v2/memory-media-playback/:assetId` 复用家庭资产鉴权，只为私有 COS 资产签发五分钟播放地址。原生端不再把整段视频下载到内存和临时文件，而是让 `AVPlayer` 直接执行 Range 请求。
- 时间流媒体视图明确返回 `media_type / content_type / duration_seconds / media_url`，旧图片缓存 JSON 中缺少这些可选字段时继续兼容。
- 真实云测首次发现 PostgreSQL 通用导出层只保留媒体 metadata 的旧字段，导致视频时长回读为 0。根因已在 `buildCloudSeedBundle` 和 `createDbFromCloudRows` 的统一序列化边界修复，完整保留尺寸、时长、上传者和后续媒体元数据，并增加 PostgreSQL 写出再加载的往返回归；没有在视频接口旁增加第二套旁路字段。

### 验证与边界

- Node 全量 29 项中 28 项通过、1 项因本机未提供 PostgreSQL 集成 URL 跳过；iOS 69 个单元测试和 12 个 UI 测试全部通过，代码格式检查和 Node 语法检查通过。
- 腾讯云使用真实 6 秒、1280x720、H.264/AAC、fast-start MP4 完成隔离闭环：2,105,187 字节直传成功，COS `bytes=0-1023` 返回 206 和有效 MP4 头，记忆回读为 `video / 6 秒`，删除后原签名地址返回对象不存在。测试用户、家庭、数据库记录和 COS 对象均已清理，清理接口复查为 0 条残留。
- `gohome-app.service` 和公网健康检查保持正常，现有家庭数据未清空。仍需 iPhone 真机覆盖安装后验收相册授权、压缩耗时、发布进度、前后台恢复、连续播放、删除回显和弱网体验；在完成前不宣称 TestFlight 视频体验已验收。
- App 被系统强杀且已完成 PUT、尚未调用完成或中止接口时仍可能留下未登记对象。正式扩大视频使用前，需要在 COS 为未登记临时对象配置生命周期，或增加服务端持久化上传意图回收任务。`event-evidence/*` 仅完成权限和前缀预留，盒子事件证据迁移继续留在视觉主线。

## 130. 2026-07-24 主线视觉闭环合并基线

本原生 App 支线已合并主线截至 `ec5dc9a` 的已提交视觉闭环，不重复修改盒子算法。主线完成的是事件状态语义和恢复证据，不是针对现场样本追加临时阈值：

- `posture_semantics.py` 集中定义物理恢复姿态与过渡低位姿态；`PoseFactorGraphEngine` 输出 `gohome-physical-recovery-v1`，只有同一 `track_id` 连续稳定为 `standing/sitting` 才形成 `confirmed / sample_count / required_samples / identity_match=same_track` 恢复证据。
- `RuleEngine` 将跌倒生命周期拆为 `confirmed -> candidate_cleared -> recovered`。候选消失、下蹲、弯腰、遮挡或旁人站立都不能关闭事故；`EdgeWorker` 只消费权威 `fall_recovery`，不再维护第二套原始姿态恢复判断。
- 云端设备状态接口同步执行同轨迹稳定恢复契约；恢复只记录 `recovery_status/recovered_at`，不代替家属确认，也不提前把事故标成已处理。
- 研发管理页使用同 `frame_id` 的连续姿态流；KLT 只做短时展示和风险唤醒，不能成为人物、姿态、跌倒或恢复证据。云端多图复核按 `before -> transition -> current` 顺序比较动作过程，成功排除时不先发送错误高优先级告警。
- 现场三分钟动作中，`2098/2099` 弯腰和 `2100` 下蹲被云端以 0.95 置信度排除，`2101` 快速倒地被确认；树莓派双路保持 8 FPS、温度约 67 摄氏度，盒子与腾讯云服务均正常。

合并后云端必须同时包含上述视觉事故契约和本支线的原生 App/COS 接口；以后部署 `server.js` 前先完成主支线同步，不得再用任一旧分支文件覆盖另一侧能力。

## 131. 2026-07-24 私有视频封面与播放授权收口

- 新增独立 `MemoryVideoPlaybackStore`，统一管理私有视频的短时播放授权、首帧生成和封面缓存；时间流封面与全屏 `AVPlayer` 不再各自请求签名。
- 播放授权按资产 ID 合并并发请求，并在距过期不足 30 秒时主动失效。封面先查内存和磁盘；本地没有时才通过家庭鉴权接口取得 COS 地址，从远程 MP4 的 0.1 秒位置生成首帧。
...
## 133. 2026-07-24 原生记忆发布器真机收口与代码清理

### 产品口径

- 记忆发布器按家庭私密时间流收口，只保留正文、媒体和可选地点。人物与手选发生时间不再作为首版输入；新记录使用服务端 `created_at` 展示真实发布时间，编辑旧记录时仅兼容保留历史隐藏字段。
- 媒体入口固定为“选择照片 / 选择视频 / 拍摄”。系统选择器在提交前限制最多 9 张照片或 1 个视频；图片与视频互斥，视频最长 60 秒。
- 时间流采用正文在媒体上方，喜欢、评论和管理动作同一行，地点位于动作下方；编辑继续显示已发布媒体，删除和预览使用独立点击层级。

### 媒体与资源生命周期

- 相册照片使用 data representation 原子写入临时目录，避免系统文件表示过早失效造成偶发漏选；拍摄结果和视频文件也先复制到 App 临时目录，再由发布器统一消费。
- 图片在后台任务中降采样为最长边 1280 的 JPEG。视频按时长选择 720p、medium、low 多级导出计划，逐级重试直到低于 24 MB；每次失败或超限输出都立即删除。
- 选择器生成的临时文件在处理完成、失败、移除媒体或发布器关闭时回收。自定义选择器启用后，旧 `PhotosPickerItem` 图片读取、旧 Transferable 视频导入和对应无用 import 已删除；图片与视频预处理统一归属 `MemoryMediaProcessor.swift`。
- COS 继续使用持久化上传意图、签名 PUT、完成核验和中止回收。迁移 `009_media_upload_intents.sql` 尚未应用，当前代码未部署；事件证据 COS 仍由视觉主线负责。
...
- 2026-07-25 用户已在覆盖安装后的真机冷启动状态完成首次媒体选择，发布页第一次即显示预览，不再需要退出重选。云端随后保持 `pending_media_uploads=0`，正常发布链路没有遗留上传意图。
- 首次选择问题已完成真机验收。下一门槛为上传中的受控强制退出或弱网中断，以及 App 重启后私有视频封面恢复；两项完成前不推送分支。

## 136. 2026-07-25 真机异常中断与视频重启恢复验收

### 上传中强制退出

- 验收前云端基线为 `family_memories=12 / media_assets=476 / media_upload_intents=0`。用户选择一个处理后为 4,862,690 字节的 MP4，点击发布后，服务端于 11:38:38 持久化上传意图；监听器在 11:38:40 检测到 `pending_media_uploads=1` 后强制终止 App 进程。
- 中断后没有生成文案为“中断测试”的记忆，正式记忆和媒体计数保持 `12 / 476`，目标资产没有进入 `media_assets`。COS `HeadObject` 返回 404，说明 PUT 中断没有形成可见半对象；数据库只保留可审计的待回收意图。
- 该意图于 11:48:38 正常过期，服务端周期任务在 11:50:08 自动将 `pending_media_uploads` 从 1 清为 0。最终上传意图、正式资产和测试记忆关联均为 0，COS 对象仍为 404；过程中未重启服务、未手工改库、未删除用户已有记忆。
...
## 140. 2026-07-25 原生设备与摄像头管理收口

### 服务端权限与数据一致性

- `GET /api/device-claims/available` 现在要求明确家庭上下文和家庭创建者权限；设备认领、兼容绑定、摄像头创建/编辑/删除/重新验证统一改为创建者权限，普通成员请求返回 403。
- 摄像头创建必须找到当前家庭的有效盒子绑定；指定的 `device_id` 不属于该家庭时返回 409。编辑接口拒绝 `family_id / device_id`，避免通过普通编辑把摄像头迁移到其他家庭或盒子。
- 摄像头公开结构继续删除 `stream_url / username / password`，只返回 `has_stream_config / password_set` 等非敏感状态。凭证只在认证保存和设备配置下发链路中使用。
- 解绑继续撤销盒子 token、移除该家庭下的摄像头配置并保留历史事件数据；绑定列表新增 revoked 过滤，修复解绑后刷新会重新出现失效盒子的根因。旧 Web 客户端的可认领设备方法同步支持家庭参数，没有新增第二套接口。

### 原生 iOS 交互与结构

- 新增唯一的 `CameraConfigurationForm`，由首次引导和后续设备管理共同使用。添加模式优先调用已绑定盒子的 `/api/cameras/discover`，展示可选候选和明确选中态；失败时展开设备地址、端口和视频路径，用户不填写完整 RTSP URL。
- 表单提供画面名称、可见安装位置选择、自定义位置、账号密码和编辑模式启停；主动作统一为“保存并同步”。删除了原生端先调用固定成功响应的 `testCamera` 路径，在线状态必须等待盒子真实同步回传。
- “我的 -> 家庭盒子与摄像头”展示全部有效盒子和摄像头。创建者拥有添加、编辑、删除和带二次确认的解绑入口；普通成员看到同一状态但没有 mutation 控件。待同步、待盒子验证和待设置统一显示“配置中”。
- `ProfileViewModel` 是设备页面唯一状态所有者。创建、编辑、删除和解绑成功后先更新当前 `ProfileData`，再等待家庭隔离缓存写入完成；不再启动无序的后台缓存任务。解绑同时从内存和缓存移除对应盒子与摄像头。
- 数据模型为历史响应补充 `enabled=true / password_set=false` 默认值，旧服务器或旧缓存缺少字段时仍可读取；创建与编辑使用独立请求类型，编辑编码只包含名称、位置和启用状态。

### 当前验证与边界

- 服务端设备专项通过，覆盖成员越权、创建者新增/编辑、敏感字段隐藏、跨盒迁移拒绝、解绑级联、解绑后有效列表为空和无盒新增拒绝。
...
## 141. 2026-07-25 删除最后一路摄像头后的引导状态修复

### 根因与产品规则

- 真机设备管理验收中，两路云端摄像头分别收到成功的 `DELETE /api/cameras/2` 和 `DELETE /api/cameras/3`。问题首次出现时盒子绑定 `1`、家庭 `2` 和设备 `edge-042714be475b91da` 仍为 active，因此“添加第一路画面”不是解绑结果。
- 原生启动接口按当前老人资料、盒子和摄像头数量动态计算首次引导。最后一路摄像头删除后，已运行家庭被错误改回 `camera`，App 因而进入设计上不可返回的“添加第一路画面”。问题属于服务端状态模型，不以增加返回按钮掩盖。
- 首次引导现改为一次完成、持久完成。`families.metadata.onboarding_completed_at` 存在时始终返回 `complete`；首次真正具备资料、有效盒子和摄像头时写入标记。对升级前已删除摄像头或已经解绑盒子、但仍保留家庭事件的成熟家庭，以事件历史作为一次性补全证据。
- PostgreSQL 组合存储此前只把家庭元数据中的成员数、创建者和存在状态水合到内存，后台保存会覆盖直接写入数据库的其他元数据。现改为完整保留并回写家庭 metadata，仓储写入完成标记时同时更新唯一服务内存；标记已跨服务重启和多个后台保存周期保持不变。
- 真正的新家庭即使已经填写资料并绑定盒子，只要从未配置第一路摄像头且没有历史证据，仍返回 `camera`。已完成家庭删除全部摄像头后进入五栏主界面，由守护与设备管理展示真实空状态和重新添加入口。

### 验证与部署

- 新增 JSON 回归三项：完成后删除最后一路仍完成、新家庭仍要求第一路摄像头、无当前盒子但保留事件可补全完成标记；新增 PostgreSQL 回归验证标记只持久化一次、解绑后可补全，并新增家庭任意 metadata 水合与序列化往返测试。
- 当前分支完整服务端结果为 52 项中 51 项通过、1 项因未配置 PostgreSQL 集成 URL 跳过、0 失败。生产基线隔离工作树结果为 32 项中 31 项通过、1 项同原因跳过、0 失败。
- 腾讯云继续基于稳定生产文件做最小部署，没有部署 APNs、iOS 文件、数据库迁移或盒子视觉代码。最终只修改当前线上 `server.js` 的 metadata 同步注入，以及两个 native repository、`postgres-store.js` 和导出序列化脚本；远端 SHA-256 依次为 `8dbabf1c2d019c2c05f6e0519a15eb865a3db7f5bba87ab57d0da7cad8aa1d3e`、`7f4604de4d275bb8bbd0cd87892cbd1d54529b26636448daa869384b99cfde51`、`81e19ed48340a05186953aaf513f85714918a4a4e899c3dc8f4ba09b6e8d01a5`、`24f84219af6eb186590bd292cc4b0b3f66cca9e96c88d9824c38793d04d23093` 和 `c45d70e79a70169256e90c07126e1d958deb7133c553f850731d9373d6817e00`。
- `20:45:56` 原生 App 明确发送 `DELETE /api/device-bindings/1` 并收到 200，因此最终真实状态为盒子绑定 revoked、云端摄像头 0、树莓派本地摄像头 0；系统不会自动重绑。家庭 `2` 已根据 1 条老人资料和 244 条保留事件写入完成标记，跨重启后仍存在，所以 App 保持五栏主界面并在设备管理展示未绑定空状态。
- 公网 `/health` 返回 PostgreSQL、244 条事件、479 个媒体资产和 `pending_media_uploads=0`，本次部署没有启动或语法错误。生产环境另有既存的关怀调度器 `family_id required for app message` 每分钟告警，与本次引导和解绑链路无关，后续需单独收口。

## 142. 2026-07-25 安全配对窗口与盒子归属展示

### 根因与结构调整

- 原实现只用 `edge-agent` 进程启动时间判断 15 分钟配对窗口，过期后提示用户重启盒子；管理端 `/api/device` 没有家庭归属信息，也没有可信的重新开窗入口。
- 新增独立 `PairingWindow`，统一负责初始窗口、管理端 10 分钟重开、剩余时间和绑定成功关闭；新增 `DeviceBindingState`，只持久化白名单内的脱敏家庭归属摘要。
- `POST /api/admin/pairing-window` 和 `/api/device` 纳入盒子管理会话保护。未登录开窗返回 401；已绑定盒子拒绝重新开窗，避免本地管理端绕过 App 的家庭创建者解绑权限。
- 云端 `/api/device/token/exchange` 和 `/api/v1/device/config` 返回 `binding_summary`。家庭创建者手机号只返回 `前三位****后四位`；完整手机号、邮箱、用户 ID 和 token 不进入摘要。配置同步收到旧 token 的 401 后清除本地旧归属。
- 管理端首页显示绑定状态、家庭、脱敏创建者、绑定时间、云端同步和配对倒计时；App 过期文案改为前往盒子管理端开启 10 分钟安全配对。

### 验证与部署

- 配对专项脚本通过窗口 15 分钟初始开放、过期、10 分钟重开、主动关闭、摘要字段白名单和 401 清理回归；服务端设备专项覆盖云端配对摘要、脱敏账号、设备配置同步和解绑后旧 token 失效。
- 完整原生服务端为 53 项中 52 项通过、1 项因未配置 PostgreSQL 集成 URL 跳过、0 失败；Python 编译、JavaScript 语法和 iOS 模拟器 Debug 构建通过。
- 腾讯云基于现有稳定生产文件只应用本次 `server.js` 配对摘要差异，未部署 APNs、数据库迁移或视觉算法；服务 active，PostgreSQL、244 条事件、479 个媒体资产和待回收上传 0 保持不变。
- Raspberry Pi 只部署配对状态、配置同步、管理端归属组件和相关 API 文件，未修改 EACP、姿态、跌倒、火灾或多模态复核代码。实机服务 active，局域网发现返回可配对；未认证开窗为 401，认证开窗返回 600 秒，管理页已包含归属栏。
- 当前盒子和云端均为未绑定状态，旧 token 同步返回 401 并已清空本地归属摘要。本次 10 分钟窗口已实机开启，用户可直接回 App 搜索绑定，无需重启盒子。

### 量产边界

- 当前 Raspberry Pi 管理端仍保留开发期默认密码，只适合受控演示网络。量产必须使用每机唯一初始管理凭证或首次强制改密，并增加盒身实体配对键；App 不获得远程强开窗口的能力。
- 追加修复摄像头添加 401：管理中间件只对私有局域网源地址放行 `GET /api/cameras/discover` 只读扫描，新增、编辑、暂停、删除和管理 API 未放宽。该例外保证绑定后 App 能发现摄像头，同时不改变云端家庭创建者权限边界。
- 复用历史 revoked 绑定重新认领时，云端现在同时刷新 `bound_at / updated_at` 并清除旧解绑标记；公开绑定和盒子脱敏摘要统一采用最近一次绑定时间，不再显示首次历史绑定日期。
- 修正摄像头配置后的 App 状态闭环：新增、编辑、暂停、删除或解绑成功后触发首页/守护摄像头数据后台重同步，并在进入守护页时静默刷新；保留已有缓存画面，不再要求退出并重启 App 才能看到新摄像头。设备配置变更仅执行立即、2 秒、5 秒三次有限对账，不做常驻轮询。
- 修正绑定时间与配置版本耦合：只有历史 `revoked -> active` 重新认领才刷新 `bound_at`；普通心跳、配置拉取和自动归属检查不再改写绑定时间，避免盒子配置版本持续变化和“配置中”假状态。
- 摄像头配置保存成功后，App 若仍能发现同一局域网内的已绑定盒子，会调用私网 `POST /api/lan/config-sync/wake` 唤醒盒子立即向云端拉取；接口不接受配置内容，并在盒子端做 2 秒合并防抖。局域网唤醒失败时继续使用 10 秒轮询兜底，不把同步频率永久提高。

## 143. 2026-07-26 家庭实时画面隐私模式

### 单一状态与权限

- 云端将家庭隐私模式保存到 `care_preferences.metadata.video_privacy`，提供家庭创建者可写、家庭成员可读的 `GET/PUT /api/v1/families/:id/video-privacy`，以及设备令牌保护的配置读取和修改入口。没有新增平行摄像头表或 App 直写盒子数据库。
- 播放会话把摄像头、家庭和隐私模式写入短时票据。视频出口取家庭状态和请求模式中更严格者，不能通过 URL 或旧票据请求更低隐私等级；云端实时帧按 `camera + privacy_mode` 隔离，非原画模式无新帧时不回退历史原画。
- 盒子管理端和原生 App 都使用同一家庭状态。实时页面每 1 秒静默查询；App 修改后，云端在下一次实时帧上传响应返回期望模式，`LiveRelayAgent` 立即更新共享策略。实机首次验收发现双摄帧回执受取帧和编码耗时影响，切换曾需 `4.2-7.8` 秒，因此补充设备令牌保护的轻量 `GET /api/v1/device/video-privacy`，盒子每 1 秒只读取该策略，完整摄像头配置仍保持 10 秒周期。

### 盒子视频出口

- 新增轻量 `video_privacy.py` 统一模式校验和隐私等级，不让普通配置同步间接依赖 NumPy/OpenCV；`privacy_stream.py` 只负责 JPEG 解码、可信人物区域遮蔽和同区域骨架渲染。
- 隐私处理只接在本地管理流和云端实时帧上传前。正式视觉 worker、连续姿态跟踪、跌倒/火灾规则、事件截图和多模态验证继续使用原始采集帧，本批未修改任何算法阈值或事件状态机。
- 人像模糊和骨架隐私合并同一连续感知锚点的 YOLO 人框与姿态跟踪框，只处理人物区域，房间背景保持原画。有人框但没有可信关键点时仍遮蔽人物但不伪造骨架；当前无人时显示正常环境画面。非原画渲染异常时丢弃该帧，不能以原始 JPEG 兜底。
- 本地普通视频流和同步姿态流都受家庭模式最低等级约束；算法页在非原画模式下切到隐私流。管理端首页、摄像头、事件和算法页面由共享脚本生成唯一一套模式控件，并统一使用 `20260727-privacy-3` 的 CSS/JS 缓存版本。

### 原生 iOS

- 守护实时页新增三段式模式控制。家庭创建者可修改，成员只读；模式变化重建当前摄像头播放会话但保留上一帧，新模式首帧到达前不显示黑屏或整页加载态。
- 播放请求和 MJPEG URL 同时携带票据约束后的模式。离开守护、切到轨迹/事件或 App 进入后台时停止视频及隐私同步，恢复时去重启动，避免重复流和重复轮询。
- 当前自动化覆盖家庭策略拉取、创建者更新、同一路流无黑屏切换、卡流 watchdog、摄像头切换和 MJPEG 分片解析。

### 当前验证与部署

- Python 编译、配置同步回归、安全配对回归和 JavaScript 语法检查通过。配置模式定义已从图像渲染模块拆出，非视觉脚本不再要求 NumPy。
- 原生服务端完整测试 54 项中 53 项通过、1 项因未配置 PostgreSQL 集成 URL跳过、0 失败；新增隐私专项覆盖创建者权限、成员拒绝、App 播放票据、设备配置、实时上传回执以及管理端修改后 App 查询一致。
- iOS 单元测试 94 项中 93 项通过、1 项按模拟器 Keychain entitlement 规则跳过、0 失败；其中守护与 MJPEG 相关 17 项全部通过。
- 本机工作区 Python 有 NumPy 但没有 OpenCV，系统 Python 同时缺少 NumPy，因此像素级渲染脚本必须在树莓派现有 `.venv-pi` 执行，不向系统 Python 安装重复视觉依赖。
- 2026-07-27 已将生产差异合并版部署到腾讯云 `gohome-app.service`，没有用开发分支整文件覆盖生产定制。服务和 PostgreSQL 均为 `active`，内外网 `/health` 正常，部署前后保持 `252` 个事件、`494` 个媒体资产和 `0` 个待处理上传。
- 树莓派 `.venv-pi` 像素级三模式脚本通过；真实双摄依次验证骨架、人像模糊和原画，两路均持续上传非空 JPEG、模式一致、无转发错误，验收后恢复原画。低延迟修正后策略状态和两路实际输出均低于 2 秒门槛；持续运行温度约 `59°C`、`throttled=0x0`，双摄保持在线和同步。
- 最新完整回归：服务端 `54` 项中 `53` 通过、`1` 项因未配置 PostgreSQL 集成 URL 跳过、`0` 失败；iOS 单元测试 `94` 项中 `93` 通过、`1` 项按模拟器 Keychain 规则跳过、`0` 失败；iOS UI 测试 `15/15` 通过；前端缓存、服务端闭环、发布元数据、安全配对和 Pi 配置同步回归通过。
- 腾讯云和盒子代码已启用；使用 Team `CPRVX9XK47` 的描述文件完成 iPhone 15 Pro Max 真机构建、覆盖安装和启动，原登录数据保留。App 与管理端双向模式同步已完成人工点击验收；本次人物区域修正只改盒子输出和管理端，不需要重新安装 App。

### 人物区域渲染修正（2026-07-27）

- 删除人像模糊在无人物框时的整帧模糊兜底，也删除骨架模式的整帧强模糊和暗色画布。两种模式现在都从原始环境帧开始，只在可信人体区域内做强模糊；骨架模式随后只对同一可信姿态绘制关键点和连线。
- 隐私区域同时读取连续跟踪的姿态框和锚点分析中的 YOLO 人框，不新增 YOLO/RTMPose 推理、不增加线程，也不改变 EACP、跌倒、火灾、事件和正式证据链。无人时背景保持正常；仅有人框无关键点时人物仍被遮蔽。
- `verify-video-privacy.py` 新增像素约束：人物区域变化必须显著高于框外背景、框外平均误差受限、骨架画面不得全局变暗、无人两种隐私模式必须保持原环境、仅 YOLO 人框时两种隐私模式仍处理人物区域。
- 树莓派 `.venv-pi` 隐私像素回归、连续画面管理台回归和配置同步回归通过。实机管理页确认四个页面均只有一套可见模式控件、无横向溢出，模式切换可写回同一家庭状态；双摄 8 FPS 均在线同步、实时转发无错误，温度约 `64.8°C`、`throttled=0x0`。
- 检测驱动的隐私无法保证漏检人物仍被遮挡，因此当前模式按“观看隐私”描述，不作为绝对匿名化或合规脱敏承诺。

## 144. 2026-07-27 原生个人数据导出与账号注销

### 服务端契约

- 新增 `GET /api/v2/account/export`、`GET /api/v2/account/deletion-plan` 和 `DELETE /api/v2/account`。删除请求必须携带固定确认值 `DELETE_ACCOUNT`，服务端路由现已为 `DELETE` 解析 JSON body。
- 导出由仓储在一致性快照中组装，返回账户、家庭、成员、照护资料、设备、摄像头脱敏配置、规则、日历、事件、证据资产 ID、媒体元数据、记忆、活动区间、关怀卡片和消息操作。序列化白名单不包含密码、RTSP、摄像头账号密码、绑定码、设备 token、会话 token、APNs 密文、COS storage key 或签名 URL。
- 注销计划统一返回 `can_delete / blockers / deletion_scope / retention_note`。家庭创建者仍有其他有效成员时返回 409 阻断；普通成员只退出家庭并删除本人受限内容；唯一创建者删除家庭并让设备回到未绑定状态。

### 删除一致性与媒体边界

- JSON 仓储通过 `applyAccountDeletionToDb` 统一清理用户、成员关系、会话、推送、本人记忆互动、被删除家庭配置和运行快照；服务端在保存 PostgreSQL 行差异前同步清除内存快照，避免已删除账户被旧快照重新写回。
- PostgreSQL 仓储在事务内锁定用户、成员关系和家庭，再计算最终删除计划；先处理 `family_memories / family_memory_comments` 的 restrict 外键，再删除用户。家庭删除依赖既有外键清理配置，`devices.family_id on delete set null` 释放实体盒子归属。
- 媒体清理在删除前收集资产 ID 和未完成上传对象键。本人记忆的资产只有在没有任何保留记忆继续引用时才删除；该规则在 JSON 和 PostgreSQL 两条路径一致。服务端调用 COS 或本地文件清理后删除媒体元数据，响应不向客户端暴露内部清理列表。
- 注销事务完成后旧 bearer token 立即返回 401。审计日志通过外键或内存清理解除用户关联；本批不新增独立长期审计归档系统，后续若有法定留存要求必须补充明确期限和清理任务。

### 原生 iOS

- 新增 `AccountDataModels` 和 `PrivacyDataViewModel`，`AppRepository` 注入导出、注销计划和注销动作。`ProfileView` 的“隐私与数据”页面显示数据导出、创建者阻断或可删除状态，不出现工程字段。
- JSON 写入系统临时目录并启用原子写入和完整文件保护，通过 `UIActivityViewController` 分享；分享关闭或下一次导出前删除临时文件。注销使用系统破坏性 Alert 二次确认，确认文案按服务端范围提示家庭和本人记忆影响。
- 服务端确认删除成功后，根视图撤销推送、清空 Keychain、全部账户缓存和会话上下文，然后回到登录注册状态。UI 测试只验证确认对话框，不使用测试账号执行真实删除。
...
## 149. 2026-07-31 Hailo 骨架隐私链路性能收口

### 根因与实现

- 骨架页面的 `1 FPS` 不是 Hailo 推理速度。边缘端 `GOHOME_LIVE_SCENE_RELAY_INTERVAL_SECONDS=1` 且代码强制最小 0.5 秒，云端安全背景 MJPEG 又固定 250ms 轮询；骨架背景还使用单连接同步上传，而普通/模糊有四个并发槽。
- 删除独立时间限速，骨架背景随真实摄像头源进入同一有界上传管线。人物区域按连续姿态和人框合并遮罩，用保留的干净背景替换；人物外直接保留当前环境，异常时不回传原始人物帧。
- 每个安全背景带 `stream_epoch_ms + sequence`，每个上传线程持有独立 keep-alive 连接。云端只接受最新序号，晚到旧帧返回 `stale_ignored`；边缘端分别统计普通帧和安全背景的提交、完成、有效、忙丢、p95 和最大延迟。
- 云端 `/health` 新增 `stream_metrics.scene_cameras`，避免骨架模式停止普通帧后被运维页误判为同步中断。安全背景 MJPEG 使用播放 profile 的真实输出节奏，不再固定 4 FPS。
- 部署门禁修复 `.venv-pi` 被硬编码 `.venv` 误判的问题；未显式配置时只允许项目内 `.venv/.venv-pi`，盒子部署同时校验 Hailo Python 库、设备节点和 Pose/Object HEF。

### 自动与实机验证

- 边缘端验证覆盖人物像素不泄漏、人物外动态环境保留、四槽有界背压、场景序号、姿态中继和三种隐私模式；云端原生服务端 `90/90` 通过，JavaScript/Python 语法和差异检查通过。
- 两路摄像头源约 15 FPS；骨架安全背景边缘连续样本约 11-14 FPS、云端完整窗口约 10-14 FPS，有人摄像头姿态包约 10.8 Hz，无人摄像头只发送低频空状态。Hailo Pose 中位约 14-18ms、p95 约 24-29ms、失败 0，摄像头重连 0。
- 盒子进程约 276MB，短时骨架双摄温度约 70.5-74.3 摄氏度，无服务警告。当前结果证明边缘到云端链路已解除 1 FPS 根因，但不能替代 TestFlight 端真人移动、前后台和切页返回 10-20 分钟验收。

## 150. 2026-07-31 双摄隐私隔离与宠物类别稳定

### 隐私隔离

- 真机发现客厅摄像头的人物替换区域嵌入冰箱摄像头视角，判定为高优先级跨摄像头背景污染，不是正常的骨架渲染。
- 根因不是单次缓存键碰撞，而是骨架隐私允许历史场景像素进入当前人物替换区。`PrivacyBackgroundReconstructor v3` 已删除历史背景、候选背景和跨帧像素缓存；人体区只对当前摄像头当前帧做局部 inpaint，无人帧直接复制当前画面，元数据不可用时返回不含可识别人像的中性安全画面。
- `CameraAgent` 为每个采集帧写入 `camera_id`、摄像头前缀 `frame_id` 和由实际流配置生成的 `source_key`。`EdgeWorker`、`ContinualPoseTracker`、`PrivacyFrameRenderer` 与 `LiveRelayAgent` 全链校验三者；流源变化使旧 KLT 锚点立即失效，任何不一致都拒绝同步帧。
- Hailo Pose 原有 IoU-NMS 无法删除“上半身框完整包含于全身框”的重复姿态。解码层已增加较小框包含率去重；回归固定验证包含框删除、空间分离的第二人保留。
- 自动化新增“底图 A、仅人物区替换为摄像头 B”的精确污染样本，旧实现稳定失败，新实现通过。同步帧、持续跟踪、共享采集、上传顺序和安全场景回归全部通过。
- 初版当前帧 inpaint 在电视柜实机 P50/P95 约 95/141ms，仍会拖慢画面，因此改为只在人体局部 192px 最长边工作区修复后回填原尺寸。树莓派稳定后有人路 P50/P95 约 7.6/11.8ms；状态为 `privacy-frame-renderer-v9 / privacy-background-reconstructor-v3 / retained_pixel_state=false / memory_bytes=0`。

### 宠物稳定

- 当前 Hailo-8 只有可用的 `yolov8s_h8.hef` 通用 COCO 检测模型；设备上的 H10/H8L HEF 不与当前 Hailo-8 硬件混用。
- 截图中的灰猫被通用模型以 0.31 置信度判成狗；旧 `v1` 又将衰减分数除以未衰减命中数，使界面置信度继续跌到 0.09，证明类别既不可靠，计算也不正确。
- `PetTemporalStabilizer v2` 让分数和有效权重同步衰减，以加权平均作为最终置信度。`0.28` 以上只进入内部候选；同位置至少两次命中、类别优势满足且最终平均置信度不低于 0.40 才允许输出明确猫或狗，低于门槛不进入用户可见宠物列表。
- 轨迹仍以摄像头 ID 分区，短时漏检最多保持 2.2 秒，单帧错类不能翻转已确认类别。新增连续五次 0.31 狗误分类仍不得输出的回归，并保留确认、翻转保护、过期和双摄隔离覆盖。
- 当前改动解决“低置信度错误类别被明确展示”，不等于通用 COCO 模型已经能稳定区分家庭猫狗。若真实猫在清晰近景下仍长期没有 0.40 以上猫类别，应收集带距离、遮挡和摄像头视角标签的困难样本，评估 Hailo 二阶段宠物分类器；不能用手工规则把狗强改成猫。

### 部署与运行结果

- 盒子视觉运行时检查通过：Hailo 设备、Pose/Object HEF、Python、Ultralytics、RTMPose 均可用；Hailo Pose 中位约 14ms、P95 约 23ms、失败 0，视觉管线中位约 17ms。
- 部署和两次服务重启均未覆盖 `.env`、SQLite、日志、摄像头配置、家庭绑定或用户数据。两路摄像头保持在线且重连数为 0。
- 云端安全场景完整帧当前约 11 FPS，仍受两路 JPEG 编码、网络上传和云端接收延迟影响；本批已去掉人物消隐的主要 CPU 峰值，但 10-20 分钟 TestFlight 真人移动和切页返回验收仍未完成，不能把该项写成最终验收通过。

## 151. 2026-07-31 空场景人物幻觉根因修复

### 根因

- 旧管线把 Hailo Pose 输出框转换为 `accelerated_people`，随后又用同一组 `raw_people` 验证姿态，形成“姿态框证明自身是人”的循环证据。未验证姿态还会先排除重叠的家具场景框，使后续家具抑制失去参照。
- 实机数据库进一步证明 Hailo Object 也会在固定家具位置以约 0.79-0.91 置信度持续误报人物。Pose/Object 都来自相近 YOLO/COCO 分布，简单双模型重叠仍不是独立真人证据。

### 实现

- `pipeline.py` 不再把 Pose 框写入目标人物列表；Hailo Object 人框、Pose 检测分数和关键点分数分别保留来源。场景建图只被独立目标人物框遮挡，未验证姿态不能删除沙发、电视等场景参照。
- 新增 `HumanEvidenceGate`：每路摄像头独立维护有界候选轨迹。新目标需伴随候选框局部画面运动，已存在的静态候选需发生足够空间位移后才可激活；电视、宠物或其它区域的运动不能替固定假人激活。确认轨迹在检测连续时允许静止，超过 2.5 秒间隙自动过期。
- 主管线在人物计数、连续骨架、活动、进食、跌倒和日志之前统一消费门控结果。姿态模型已运行但可信姿态为空时，孤立 Object 人框同步清零，避免 UI 空骨架但日志仍记录有人。
- Hailo Object 人物最低阈值从隐式 0.20 收口到独立配置的最低 0.30；Pose 原始检测分数写入 `detector_confidence`，拒绝原因和时序门控细节保留在诊断结果。

### 验证

- 新增回归覆盖：姿态自证家具必须输出人物/骨架 0，且不能产生跌倒或进食；目标人物与姿态匹配但静止启动仍等待运动；真实运动可建立轨迹并保留；电视、边缘、家具和低质量姿态拒绝；独立人物匹配的真实躺卧继续保留。
- 树莓派完整视觉回归通过，临时验证脚本执行后已删除。部署未覆盖 `.env`、SQLite、绑定、摄像头、日志或用户数据。
- 最终服务重启并运行 30 秒后，两路连续骨架均为 `empty / pose_count=0 / observed_count=0 / tracked_count=0`，确认人体轨迹为 0，连续骨架错误为空。Hailo Pose 中位/P95 约 15.0/23.9ms，Hailo Object 中位/P95 约 22.5/24.8ms，推理失败均为 0。
- 当前只完成双摄空场景实证。下一步仍需真人进入、停止、坐卧、离开和跌倒序列验收，重点确认门控降低误报后没有不可接受的召回损失。

## 152. 2026-08-01 骨架扭曲与连续追踪根因收口

### 根因与结构调整

- 复查 `20260801000026.mp4` 确认旧实现的人物区域存在家具纹理拉伸、旧人物矩形和低频跳动。录屏时间为 00:00:29 至 00:02:05；本节新进程在 00:47:20 启动，因此该视频只作为旧故障证据，不写成新实现验收结果。
- 删除骨架正常路径的 Telea inpaint。`PrivacyBackgroundReconstructor v4` 仅保留同摄像头、同实际流配置、同流代际和同分辨率的模型确认干净背景；连续三次无人后才可使用，场景不一致立即失效，无背景时输出明确中性隐私画面。
- `ContinualPoseTracker v2` 不再直接跟踪缺少纹理的关节点像素，改为在姿态轮廓内提取 Shi-Tomasi 特征，执行前后向 LK 校验，再用 RANSAC 估计局部仿射变换并传播整套骨架与人物框。传播结果继续只用于显示与风险升频提示。
- 新增 `untracked`：模型已确认有人但完整骨架或稳定特征不足时，保留当前人物框用于隐私替换，禁止作为无人样本污染背景，也禁止进入正式事件证据。摄像头 ID、`source_key`、流代际、分辨率和帧 ID 继续全链校验。
- 管理端运行指标拆分为骨架/模糊实际输出 FPS、摄像头源流 FPS 和 Hailo 模型锚点 Hz，避免把空场 2 Hz 巡检、源流帧率或模型标称吞吐混为同一指标。

### 自动验证与当前实机边界

- 连续追踪、视频隐私、同帧同步、worker 调度、姿态中继和云端原生隐私契约回归通过；Python 编译、JavaScript 语法和差异检查通过。合成移动样本验证 RANSAC/LK 正确传播约 `dx=5.01 / dy=3.0`，`untracked` 人物保持遮蔽且不能更新背景。
- 树莓派已运行 `eacp-continual-pose-v2 / privacy-frame-renderer-v12 / privacy-background-reconstructor-v4`。Hailo Pose 中位约 14-15ms、P95 约 24ms且失败为 0；电视柜源约 15 FPS、冰箱源约 12 FPS，无读取失败和重连。
- 00:59 至 01:00 的 90 秒实机观察中两路均无人，连续姿态保持 `empty`，空场模型巡检和显示状态约 2 Hz，帧龄未持续增长。旧页面曾显示 9.4-11.7 FPS，后确认包含同一缓存帧的重复读取，不能作为唯一画面速率；该问题已由 v12 指标修正。空场结果只证明新进程稳定，不证明真人骨架流畅度已经通过。
- `privacy-frame-renderer-v12` 只按变化后的非空同步 `frame_id` 统计实际骨架内容 FPS；浏览器重复读取同一缓存帧不再累计，空帧 ID 也不制造虚假速率。实机空场冰箱源约 11.9 FPS、骨架内容约 2.0 Hz，符合无人低频巡检语义；真人阶段仍待动态验收。
- 使用摄像头正式凭证和 `ffprobe` 直接探测两路 RTSP：当前 `/1/2` 子流均为 HEVC 640x360、声明平均 15 FPS；可用的 `/1/1` 主流均为 HEVC 2880x1620、平均仍为 15 FPS。切换主流只能提高清晰度，不能把真实输入变为 30 FPS，并会显著增加双路解码、隐私渲染和上行成本；当前不以插帧或请求次数伪造 30 FPS。后续高清播放应采用主辅双流和原生 H.265/WebRTC 链路，算法仍消费低延迟子流。
- 部署重启时发现 Uvicorn 长连接等待与六个后台服务串行停止叠加，最坏时限超过 systemd 15 秒。现将 ASGI 长连接关闭限制为 6 秒，所有独立后台服务在 7 秒共享截止时间内并行收敛，视频中继先关闭 HTTP 再等待线程。保持真实骨架 MJPEG 连接重启耗时约 9 秒，日志出现受控取消和 `Application shutdown complete`，无 timeout、无 SIGKILL，systemd 正常退出并拉起新进程。
- 待现场分别在两路完成真人进入、行走、静止、坐卧、快速下降和离开，实测人物阶段输出 FPS、追踪成功率、帧龄、背景替换、跨摄像头隔离和通知。该项通过前不关闭骨架隐私质量门槛。

## 153. 2026-08-01 单一骨架合成与新帧驱动调度

### 根因与架构收口

- 新录屏证明管理端同时存在两套骨架：`PrivacyFrameRenderer` 将粗黄色骨架烘焙进 MJPEG，浏览器又用异步 SVG 绘制细骨架。视频像素会保留旧服务端骨架，即使元数据已进入 `empty`，因此出现双骨架、黄蓝不一致和“跟踪已清除但旧骨架仍在”的残影。
- 管理端骨架模式改为明确的单一合成契约：MJPEG 只传人物已替换的 `safe-scene`，Pose 元数据单独传输，浏览器是唯一骨架绘制方；原画模式保留服务端同帧标注，模糊模式明确不绘制骨架。响应头声明 `X-GoHome-Composition` 和 `X-GoHome-Pose-Owner`，不再靠 UI 隐藏第二层。
- 管理端与 iOS 统一骨架样式为 `#21D1DB` 青色主线、黑色对比底线和白色关节点。浏览器元数据刷新从 500ms 调整为 80ms 轻量轮询；鉴权失效统一返回登录页，网络或接口异常自动退避，避免高频 401 污染日志。
- 摄像头缓存增加全局新帧条件通知。连续跟踪线程不再按固定 67ms 轮询两路缓存，而是由任一路真实新帧唤醒、按 `frame_id` 去重处理；KLT 最小处理间隔调整为 50ms，避免与 15 FPS 摄像头约 66ms 周期发生相位碰撞。
- 无人物锚点时仍将当前新帧写入同步显示通道，但状态保持 `empty / no_anchor`，不产生人物、姿态或正式证据。显示 FPS 只统计唯一 `frame_id`，模型锚点、同帧传播和空场刷新不再重复累计；`observed / tracked` 会清理历史失败原因，运行指标与当前状态一致。

### 验证与边界

- 树莓派视觉隐私、同帧接口、共享拉流新帧通知、连续追踪、worker 调度、姿态中继、安全场景和有界停机回归通过；视频隐私合成帧重建约 5.1ms，Hailo Pose 中位约 14.3ms、P95 约 24.3ms、失败 0。
- 稳定窗口内两路 RTSP 源均约 15.1 FPS、读取失败和重连均为 0；电视柜真人活动阶段连续姿态达到约 14.9 FPS，帧龄约 20-80ms，证明 Hailo 与 KLT 不是 1 FPS 根因。
- 云端安全场景 JPEG 上传仍受双路编码、网络往返和有界上传槽影响，当前有效窗口约 9-12 FPS；该指标与本地姿态跟踪 FPS 分开报告，不能写成 15 FPS 已完整贯穿到 TestFlight。
- 服务重启后管理端会话按设计失效并跳转登录页。必须重新登录、硬刷新，再分别完成两路真人行走、离开、模糊模式无骨架、单骨架无残影和 TestFlight 前后台切换验收；视觉验收前不宣称骨架隐私最终交付完成。

## 154. 2026-08-01 源帧时序、乱序回写与重复合成收口

### 根因与实现

- 显示 FPS 已按唯一 `frame_id` 去重，但仍可能短时高于摄像头源 FPS。根因是旧指标使用处理完成时间：连续跟踪线程积压后追帧会压缩处理间隔；正式模型线程还可能在较新视频帧之后完成旧帧推理，旧结果再次写回会造成骨架倒退、重复计数和视觉抖动。
- 摄像头缓存为每个真实解码帧增加同进程 `captured_monotonic`。连续姿态和隐私输出统一按源帧抵达时间计算 FPS 与帧龄，不再使用请求时间、处理完成时间或客户端刷新次数冒充源帧节奏。
- `ContinualPoseTracker` 增加严格展示单调性：同一路摄像头中，时间不晚于已展示帧的结果不得覆盖当前像素、姿态或 FPS。晚到的可信人物模型结果只作为下一真实新帧的 KLT 锚点，不能让画面倒退；晚到的空模型结果直接丢弃，不能清除较新的状态。
- `EdgeWorker` 除摄像头 ID、帧前缀和配置源哈希外，再校验当前活动 `stream_generation`。摄像头重连后，旧代次仍在执行的模型结果不得重新进入连续姿态和隐私合成。
- `safe-scene` 按 `camera_id + source_key + frame_id + resolution + quality` 复用现有有界渲染缓存。同一真实帧被管理端、云端中继或多个观看端同时消费时，只执行一次人物背景重建与 JPEG 编码，不重复改变背景状态。
- 运行进程只由 `gohome-edge-agent.service` 管理。部署和重启使用 systemd，不再手工启动第二个 Uvicorn；服务最终保持单一 PID、`NRestarts=0` 和健康状态。

### 验证与当前边界

- 树莓派连续追踪、乱序模型锚点、旧流代次拒绝、共享拉流、隐私像素、同帧接口、worker 调度和有界停机回归通过；合成位移约 `dx=5.01 / dy=3.0`，快速下落只触发风险升频提示，不进入正式证据。
- 稳定窗口中 camera 31 源流/连续显示/隐私输出约 `15.07 / 14.77 / 13.96 FPS`，camera 32 约 `15.10 / 15.10 / 14.65 FPS`；读取失败和重连均为 0，连续姿态、场景中继和姿态中继错误为空。
- Hailo Pose 中位约 `16.8ms`、P95 约 `27.4ms`、失败 0。当前端侧连续显示已受真实源流约束；云端 JPEG 输出仍低于源流，后续长时 TestFlight 验收不达标时应升级持久视频传输，不回退到重复 JPEG 或伪 FPS。
- 仍需分别在两路完成人物进入、行走、静止、坐卧、快速下降和离开视觉验收，确认晚到锚点策略没有降低真人召回，且只存在一套青色骨架、无扭曲、无旧矩形和无跨摄像头像素。

## 155. 2026-08-01 管理端视频重启恢复闭环

### 根因与实现

- 树莓派重启后，已打开页面中的 MJPEG 长连接可能直接终止但不触发浏览器 `img.onerror`。旧页面因此保留失效 URL，姿态元数据已经恢复而视频元素仍为 `naturalWidth=0`，只能依赖手动刷新。
- 管理端继续保留 `onerror` 指数退避，同时由轻量连续姿态元数据监督视频连接：同一路摄像头 `source_key` 变化、帧序号回退或流启动 2.5 秒后仍未解码图像，均只触发一次有冷却时间的流重建。
- 帧序号回退用于识别盒子进程重启；`source_key` 用于识别摄像头断线重连或换源。恢复只替换当前摄像头的 MJPEG 请求，不重载页面、不清空绑定、不重启服务，也不创建第二套视频或骨架管线。

### 验证

- 整机重启后确认 `gohome-edge-agent.service` 单一 MainPID、`NRestarts=0`，Hailo 固件正常，双摄源流约 15 FPS，Hailo Pose P95 约 26ms且失败为 0。
- 树莓派真实环境通过连续追踪、自适应 worker、共享拉流、同帧同步、视频隐私和有界停机回归；临时 `*-current.py` 与 `/tmp/verify-*.py` 已删除，只保留主线正式测试。
- 已复现旧页面黑场并确认刷新后恢复。新增静态回归锁定 `source_key + frame sequence + naturalWidth` 三类恢复信号；加载新资源后保持管理页面不刷新，执行 systemd 服务重启，视频请求时间戳自动更新并在 12 秒观察点恢复为 `640x360`、骨架 `14.6 FPS`、源流 `15.0 FPS`、帧龄 `50ms`，自动恢复实证通过。

## 156. 2026-08-01 骨架残影、漏检穿透与遮罩漂移收口

### 根因与实现

- 真人双摄录屏确认连续骨架已经明显变顺，但人物像素仍可能残留。根因不是第二套骨架，而是旧背景重建在连续“无人”判断后反复替换整张干净背景；人物短暂漏检时，人物像素会被写入背景，后续再作为所谓干净区域回填。
- `PrivacyBackgroundReconstructor v5` 将干净背景改为同一摄像头、同一 `source_key`、同一流代际和同一分辨率内一次确认后保持不变。确认门槛提高到连续 8 个稳定无人新帧；背景已建立后，短暂误判为空不再重写背景。
- 最近一次人物遮罩保留 1.4 秒。单帧或短时检测丢失继续按人物区域执行替换，不直接返回原画，也不把该帧当作干净背景样本。
- 姿态或光流框发生漂移时，仅在人物框扩展后的局部 ROI 内比较当前帧与干净背景，将真实前景差异并入隐私遮罩。该处理覆盖框后真实人物像素，同时避免全帧差分拖慢双路视频。
- 连续跟踪最小处理间隔从 50ms 收口到 20ms，适配摄像头平均 15 FPS 但到帧间隔不均匀的真实 HEVC 子流；只处理真实新帧，不重复旧帧，也不插值伪造端到端 FPS。

### 验证与运行结果

- 树莓派真实 OpenCV 环境通过视频隐私、连续骨架、自适应 worker、共享拉流、同帧同步和有界停机六项回归；固定覆盖背景污染阻断、单帧漏检保护、漂移遮罩扩展、双摄隔离和流代际隔离。
- 局部 ROI 合成回归耗时约 `7.1ms`。服务只由 `gohome-edge-agent.service` 重启，保持单一 MainPID、`NRestarts=0`，Hailo Pose 失败 0，摄像头读取失败和重连均为 0，服务 warning 为空。
- 稳定观察窗口中两路源流约 `15.0-15.6 FPS`，连续骨架显示约 `13.1 / 13.7 FPS`，隐私唯一内容约 `12.3-12.5 FPS`；云端安全场景有效约 `11.7-13.5 FPS`。RTSP 实测为 HEVC 640x360、标称 `20/1`、平均 `15/1`，8 秒内容各解码 121 帧，不以 UI 刷新或重复 JPEG 冒充 30 FPS。
- 电视柜摄像头重启后仍持续输出 3 个已确认人物候选，导致该路 8 帧干净背景尚未建立并使用中性隐私场景。该现象需要下一轮实机确认画面中是否确有人员、电视内容或固定误检；在确认前不降低人物门控来换取背景初始化，也不允许用可能含人的帧建立背景。
- 本批已解决已知背景污染、短时漏检穿透和遮罩漂移代码根因。最终验收仍需两路重新行走和离开，确认人物后方无原像素、遮罩贴合、离场 1.2-1.4 秒内清空、帧龄不增长；若 App 长时仍低于 12 FPS，按既定路线升级持久 H.264/H.265/WebRTC 传输，不继续扩展逐帧 JPEG 并发。

## 157. 2026-08-01 当前帧隐私渲染与分割时序加速

### 根因与正式结构

- 函数级剖析确认背景重建平均约 14ms，旧链路主要瓶颈不是背景本身，而是每个输出帧都执行完整 YOLOv11s 人体分割，并且隐私渲染读取姿态线程完成后的缓存画面。姿态推理或跟踪变慢时，直播会重复旧像素，导致人物残影、跳帧和骨架输出被姿态频率锁死。
- `PrivacyFrameRenderer v17` 改为始终渲染直播线程当前采集帧。`camera_id / frame_id / source_key / captured_monotonic` 从摄像头缓存贯穿隐私分割、背景合成、JPEG 编码和云端有序上传；姿态只作为有界时效的几何元数据，来源不一致或超过 240ms 时拒绝绘制，不能把旧姿态画面替换当前视频。
- `HailoPersonSegmentationBackend v2` 为每路摄像头维护独立时序状态。YOLOv11s-seg 约每 180ms生成一次当前帧锚点；中间真实新帧使用 320 像素宽灰度光流传播遮罩，并在每帧校验来源、分辨率、时间单调性、场景差异和最长 350ms 时效。上一锚点为空但当前帧出现至少 1% 显著变化像素时立即重新 Hailo 分割，不传播空遮罩。传播结果只用于观看隐私，不进入人物、跌倒、恢复或通知证据。
- 背景重建升级为 v8，21x21 学习排除膨胀只在实际像素学习周期执行，不在已冻结背景的每帧重复计算；人物遮罩扩张、背景兼容和合成边界保持不变。
- 云端 JPEG 上传继续使用实测更稳定的四槽有界窗口；队列满时丢弃当前过期输入，云端按流批次和序号拒绝晚到旧帧，不形成持续增长的延迟队列。六槽与动态窗口对照没有提升有效帧率，因此不保留额外调度复杂度。

### 自动验证与当前实测

- 树莓派通过视频隐私、连续姿态、worker 调度、共享拉流、同帧同步、管理端单一骨架、实时上传和有界停机八项回归；新增覆盖同帧分割去重、连续帧传播、空遮罩遇新前景强制锚定和来源切换强制重新 Hailo 锚定。Python 编译与差异检查通过。
- 双路稳定窗口源流约 `15.11 / 14.96 FPS`，骨架隐私唯一输出约 `14.20 / 13.91 FPS`，不再是 1 FPS。Hailo 分割中位约 `33.87ms`、P95 `58.09ms`；光流传播中位约 `15.93ms`、P95 `21.09ms`；锚点 743 次、传播 1460 帧、失败和传播拒绝均为 0。
- 背景覆盖约 `99.72% / 99.51%`，两路摄像头读取失败、重连和 Hailo 推理失败均为 0。云端四槽阶段有效约 `12.5-13.3 FPS`、忙丢帧约 8%；六槽对照窗口因 HTTPS 请求乱序使有效帧降至约 `12.0-12.3 FPS`，已按实测恢复四槽上限。需在 TestFlight 真人画面中继续复核最终有效 FPS、帧龄和热状态。
- 当前数据证明端侧架构已解除旧帧重复和逐帧完整分割瓶颈，不等于残影真人验收已经通过。仍需两路分别行走、静止、坐卧、快速下降和离开，持续 10-20 分钟核对人物像素无残留、遮罩贴合、骨架不倒退、无跨路像素、通知幂等和 App 前后台恢复。

## 158. 2026-08-01 App 骨架单流与模糊底图收口

### 根因与正式结构

- 真机反馈原画和人物模糊正常，但骨架在 App 中出现黑场、低流畅度和人物切割边缘。根因是骨架正常路径仍调用历史背景重建，同时 iOS 仍保留安全场景 MJPEG + Pose SSE 双流和客户端第二套绘制能力；同步源不可用时又会返回中性安全场景，视觉上表现为黑屏。
- `PrivacyFrameRenderer v19` 将骨架正式结构改为当前帧人物强模糊后叠加唯一青色骨架。人物外环境保持当前画面，背景重建不再参与骨架正常展示；姿态无效时只保留模糊视频。
- 同步帧、来源或流代次不满足时，不再返回中性黑场，改为当前帧全画面强模糊。该降级不泄露人物，也保持实时画面存在；独立 `safe_scene` 兼容能力仍保留，但不被正式 App 会话使用。
- iOS 删除 `PoseOverlayView`、Pose 时间线、SSE 解析、双流任务和 Pose Hz UI。三种隐私模式统一使用 `MJPEGStreamClient` 单路播放；骨架会话只接受 `server-composed-mjpeg-v1`，守护页 FPS 只统计成功解码的新 JPEG。
- 盒子端删除已无正式消费者的 `PoseRelayAgent`、相关设置和验证脚本，不再以 30 Hz 向云端发送显示专用姿态包。云端旧 Pose/Scene 接口暂时保留兼容已安装构建，待新 TestFlight 验收后删除。

### 验证与部署

- 视频隐私回归通过，固定验证人物模糊底图、唯一骨架、全帧模糊安全降级、双摄隔离、来源切换、遮罩传播和背景兼容能力。云端原生视频隐私测试通过，JavaScript/Python 语法和差异检查通过。
- iOS 工程重新生成后 Debug Simulator 编译通过；原生单元测试执行 125 项，1 项跳过，0 失败。已删除工程中的旧 Pose 文件引用，没有临时副本或第二套骨架代码。
- 树莓派部署未覆盖 `.env`、SQLite、摄像头、家庭绑定、日志或运行数据。服务重启后为单一 MainPID，`NRestarts=0`，旧 Pose Relay 文件已从盒子删除，启动日志无异常。
- 稳定窗口两路 RTSP 源约 `15.3 / 15.4 FPS`，骨架隐私输出约 `14.3 / 9.9 FPS`，云端有效接收约 `11.1 / 9.2 FPS`；Hailo 人体分割失败 0，摄像头读取失败和重连均为 0。第二路仍受公网逐帧 JPEG 延迟影响，不能宣称端到端 15 FPS 已完成。
- `GoHome 1.0.0 (8)` 已由主线唯一原生工程完成 Archive，并于 2026-08-01 22:39 上传成功；`xcodebuild` 返回 `Upload succeeded / EXPORT SUCCEEDED`，当前由 App Store Connect 处理。
- Apple 处理完成后必须从 TestFlight 安装 Build 8，完成两路真人移动、模式切换、切页返回、前后台和 10-20 分钟稳定性验收。通过标准为无黑屏、无人物切割块、无双骨架、无跨摄像头像素、无持续残影，App 显示 FPS 与实际解码一致。

## 159. 2026-08-01 骨架产品定义纠正与 Build 8 结论废止

### 审计结论

- Build 8 将骨架模式实现为“人物强模糊 + 骨架”，与产品要求的“保留当前场景、完全移除人物像素、只显示一套实时骨架”不一致。第 158 节记录的代码、自动测试和上传事实仍作为历史保留，但其“正式结构”和“完成”结论作废，Build 8 不作为最终交付版本。
- `PrivacyFrameRenderer._render_skeleton()` 直接调用人物模糊路径；历史背景重建会自动学习未显式确认的空帧，并在背景不足时生成中性色区域；这些结构分别导致模式语义错误、人物残影、画面块和扭曲。
- 云端仍存在 Pose SSE、safe-scene MJPEG 和服务端合成 MJPEG 三条骨架兼容路径；iOS 对 `server-composed-mjpeg-v1` 的强校验还可能与生产云端实际响应不一致。正式链路必须收敛为盒子合成完整帧、云端中继、管理端和 App 只显示。
- 当前双路源约 12-15 FPS，Hailo Pose 与分割中位耗时分别约 28.75ms 和 43.34ms；生产云端有效画面约 6.5-8.5 FPS，并累计大量过期帧拒绝。瓶颈包括 CPU 合成/JPEG、多份复制、每摄像头分割 runtime 和并发 HTTP 上传乱序，不能用 Hailo 单模型标称吞吐解释或掩盖端到端性能。

### 正式实施边界

- 三种模式以 PRD 和 `issue.md` 的定义为唯一依据。未完成空房校准时拒绝进入骨架模式并返回 `calibration_required`，保持切换前模式；禁止使用模糊、黑屏、中性色块或历史画面冒充骨架模式。
- 背景校准按 `camera_id + source_key + stream_generation + resolution` 隔离，必须由用户触发并连续确认空房；当前及近期人物掩码区域禁止学习，摄像头移动、重连、换源、分辨率或显著场景变化立即失效。
- 后续实施和验收按根目录 `issue.md` 逐项推进。任何条目只有完成根因修复、自动回归、运行指标、文档同步和真机验收后才能标记关闭。

## 160. 2026-08-02 盒子唯一合成与单帧协议本地实现

### 被废止的历史结构

- 第 153 节的“浏览器绘制唯一骨架”、第 157 节的四槽 JPEG 上传，以及第 158 节的“人物强模糊 + 骨架”均已废止，不是当前实现，也不能作为交付依据。
- `safe-scene` 上传、Pose 上传、Pose SSE、场景 MJPEG 和客户端 Pose 覆盖层已经从正式链路删除。管理端与 iOS 没有第二次合成权限，只显示盒子生成的完整成品帧。

### 当前唯一结构

- `PrivacyBackgroundReconstructor` 使用显式空房校准，不从普通运行帧自动学习背景。校准状态按 `camera_id + source_key + stream_generation + resolution` 隔离并持久化；流重连后必须完成规定观察次数的复验，复验期间返回 `stream_revalidation_required`，不会提前暴露当前帧。
- `PrivacyFrameRenderer` 的骨架路径只使用已确认的无人背景替换当前人物像素，再绘制一套与该精确帧同步的骨架。人物模糊路径与骨架路径相互独立；校准、分割或 Pose 不满足时停止骨架输出并返回明确状态，不通过模糊、黑场、中性色块或历史画面降级。
- 完成帧身份包含 `camera_id`、`source_key`、`stream_generation`、`frame_id` 和单调时间。Pose 必须与当前帧完全一致；相邻较新结果、晚到旧结果、旧流代次和跨摄像头状态全部拒绝。
- `LiveRelayAgent` 每路只允许一个 HTTP 上传在途，并保留一个可被更新帧覆盖的待发送槽。完成请求后立即发送当时最新帧，不补发中间旧直播帧；校准等待只暂停该模式输出，不重启摄像头采集循环。
- 云端唯一协议为 `edge-composed-mjpeg-v1`。会话 JSON 和实际 MJPEG HTTP 响应同时返回 `X-GoHome-Display-Transport: edge-composed-mjpeg-v1` 与 `X-GoHome-Composition-Owner: edge`。旧 Pose、scene 和 safe-scene 路由不存在，并由契约测试固定为 404。
- 正式 iOS 工程仅为 `ios-shell`，显示名 `GoHome`，Bundle ID `com.gohome.family`。`MJPEGStreamClient` 在接受视频帧前校验实际响应的传输协议和合成所有者；工程中不保留 Pose SSE 模型、解析器或覆盖视图。
- `HailoPersonSegmentationBackend` 按 HEF 路径共享一个 runtime，设备推理串行执行；每路摄像头的遮罩传播和时序状态继续隔离。重置单路只清除该路状态，不关闭共享设备 runtime。

### 本地验证结果与交付边界

- `verify-video-privacy.py` 通过，包含隐私像素、显式校准、流代次复验、精确帧 Pose 和相邻 Pose 拒绝。
- `verify-live-relay-upload-pipeline.py` 通过：20 帧快速输入时最大并发请求为 1、发送序号单调、最终第 20 帧送达，且校准等待不重启采集。
- `verify-hailo-segmentation-runtime.py` 通过：双摄只创建一个分割 runtime、最大设备推理并发为 1、单路重置不销毁 runtime。
- 云端 `native-video-privacy.test.js`、Python 编译、Node 语法和差异检查通过；正式 iOS Simulator 编译通过，`MJPEGStreamClientTests` 12 项 0 失败。
- 本节只证明本地结构和自动回归通过。树莓派、生产云端和 TestFlight 尚未部署本批实现；双摄真人无残影、端到端 FPS、断线恢复、通知幂等及 30 分钟长时测试未完成，因此 GH-002 至 GH-009 仍为“待实机验收”，不能标记关闭。

## 161. 2026-08-02 直播性能测量与 BGR 帧所有权

### 正式帧路径

- 盒子共享摄像头读取器只负责持续 RTSP 解码、源指标和新帧序号通知。解码帧由 `CameraAgent` 的中央最新帧缓存唯一持有，不再同时复制到读取器内部；订阅者等待序号推进后只从中央缓存取得一份私有帧。
- `LiveRelayAgent` 从该私有 BGR 帧按目标尺寸缩放，由 `PrivacyFrameRenderer` 完成唯一隐私合成并执行一次 JPEG 编码，然后交给单在途、单最新待发送槽上传。云端只缓存并转发 JPEG，iOS 只解码最新一帧，不做隐私重建、骨架绘制或视频重编码。
- 本节当时保留的“黑帧保持”已由 GH-029 作废。当前实现由共享读取器拒绝近黑帧，首帧清除中央实时缓存，订阅者不保存最近有效帧；缓存拥有者、消费者与无效帧边界因此保持唯一。

### 指标与验证

- 云端直播测量从 `server.js` 提取到 `stream-metrics.js`。`accepted_fps_10s` 按窗口内首尾真实接收时间和帧间隔计算，避免流刚启动时固定除以 10 秒造成虚低；同时输出 `accepted_sample_count_10s`、帧间隔 P95/最大值、传输延迟 P95/最大值、`last_frame_age_ms` 和 stale reject。
- 最后帧龄使用 60 秒留存窗口中的最后真实接收帧，即使 10 秒 FPS 窗口已经为空也不会错误归零，可直接识别流已停止但连接仍存在的情况。
- 本地通过云端指标、隐私协议、MJPEG 背压、盒子单在途上传、共享采集、黑帧恢复、纯骨架像素、显式校准和共享 Hailo 分割 runtime 回归。共享采集测试确认同一路只打开一次 RTSP，并且读取器不再持有 `_frame` 副本。云端 Node 测试 `94` 项中 `93` 通过、`1` 项因未配置 PostgreSQL 连接跳过；正式 `ios-shell` 单元测试 `127/127`、UI 测试 `25/25` 全部通过。
- 本节不代表端到端性能完成。GH-010/GH-011 保持“处理中”，必须在树莓派和生产云端采集解码、Pose 等待、分割、背景替换、绘制、JPEG、上传、云接收和 TestFlight 解码的真实 P50/P95/最大值，再决定持久 H.264/H.265/WebRTC 的具体实现和验收门槛。

## 162. 2026-08-02 边缘旧视频服务与路由所有权收口

### 审计结论

- 正式盒子只有 `app.main:app` 一个 systemd/Uvicorn 入口；仓库和部署脚本没有启动 `video_app:app` 的服务。盒子管理端实际使用 `/api/cameras/{id}/stream.mjpg` 与 `/api/cameras/{id}/continual-pose/stream.mjpg`。
- 正式 iOS 通过生产云端 `POST /api/v1/video/sessions` 和 `GET /api/v1/video/cameras/{id}/stream.mjpg` 播放。生产云端优先使用设备令牌代理盒子 `/api/v1/device/cameras/{localId}/stream.mjpg`，后者直接委托盒子管理流的同一 `PrivacyMjpegStream`。
- 边缘端旧 `/api/app/*`、边缘 `/api/v1/video/*`、独立 `video_app.py`、播放票据和节点分发没有真实消费者，而且 `VideoService.stream_response()` 直接调用原始 `camera_agent.mjpeg_frames()`，构成绕过隐私合成的结构性风险。

### 代码收口

- 删除 `video_app.py`、`video_service.py`、`video_distribution_service.py` 和 `video_profiles.py`；删除边缘旧 App 路由、播放会话、视频节点 API、对应 Pydantic Schema、设置项和 SQLite 方法。
- `ObjectStorageService` 接管快照证据提升和鉴权资产读取；受鉴权媒体路径改为 `/api/v1/media/assets/{id}`。对象存储不再依赖视频节点调度，也不会把盒子本地资产伪装为云端视频节点资产。
- `PublicPilotService` 只解析显式公网/生产 App 基址，不再从无消费者的视频节点表推导。`EdgeBootstrapService` 会从历史 `config.json` 中移除视频节点、旧 Pose relay 和旧上传并发环境变量。
- SQLite 初始化删除遗留 `video_service_nodes` 表；树莓派部署脚本显式删除四个旧模块，且不把新增验证脚本部署到量产目录。

### 契约与验证

- 新增 `verify-video-route-ownership.py`，通过 AST 固定边缘端不得拥有 `/api/app/*` 或 `/api/v1/video/*`，并验证管理流调用 `privacy_mjpeg_stream.mjpeg_frames()`、设备流只委托管理隐私流、生产云端声明 `composition-owner=edge`、正式 iOS 只使用云端播放会话。
- 临时数据库迁移回归确认旧节点表被删除；运行时 FastAPI 路由检查确认 130 条路由中不存在边缘 App/视频播放路由，且新媒体资产路由存在；历史 Bootstrap 配置迁移确认 10 个废弃环境变量被清除而其它配置保留。
- 共享采集、摄像头重连、单在途上传、纯骨架隐私、共享 Hailo 分割 runtime、管理端单一合成、帧节拍和有界关停回归全部通过。
- GH-012 本地根因已消除，状态转为“待实机验收”。尚未部署本批到树莓派、生产云端或 TestFlight，因此不能标记关闭；GH-010/GH-011 的持久视频协议工作也不因本次清理而完成。

## 163. 2026-08-02 统一媒体存储生命周期本地收口

### 根因

- 盒子旧清理先删除 SQLite 行，再尝试删除快照文件；物理删除失败后元数据已经消失，无法可靠重试或审计。
- 盒子容量只覆盖部分目录，SQLite sidecar、对象存储和运行日志可能在水位判断之外；数据库曾膨胀到约 1.10 GB。
- 云端只有未完成上传意图回收，没有正式媒体保留分类、删除状态、COS 全量分页清单、本地/COS 孤儿对账和生产启用前 dry-run。

### 边缘实现

- `Storage` 新增 `media_lifecycle_jobs`，快照、媒体资产和上传会话共用一条生命周期队列。任务带目标、提供方、对象路径、状态、尝试次数、租约、下次重试、错误和完成时间。
- 删除任务持有 `BEGIN IMMEDIATE`，先重新核对所有权威引用，再删除受管文件，最后删除快照/会话行或把资产写成删除墓碑。物理删除、数据库提交或外键约束失败都会保留可重试任务。
- 清理事务不再关闭 SQLite 外键。关闭的活动摘要和完成上传任务会先解除短期预览引用；开放摘要、未完成上传、正式事件和未删除资产继续保护快照。
- 常规清理按保留期执行；高/临界水位可以选择最老未保护数据，但每路摄像头最新快照不会进入候选。家庭记忆和用户上传不自动到期。
- 容量报告覆盖数据库、WAL/SHM、快照、对象目录和运行目录。只有在高/临界压力、已产生可回收页且磁盘具备完整临时空间时才 checkpoint/VACUUM；worker 显式传递实际目录。
...
- PostgreSQL 导出和水合保留全部生命周期字段。验证事件和验证资产不再从数据层静默丢弃；验证关联 App 消息继续过滤，避免测试证据触发用户通知。

### 验证结果与上线边界

- 云端完整 `npm run test:native-server`：`101` 项、`100` 通过、`1` 项因未配置 PostgreSQL 集成地址跳过、`0` 失败。新增覆盖失败重试、保护资产、删除墓碑、COS/本地孤儿、存储后端隔离、dry-run、删除开关门禁、COS 分页、运维鉴权和 PostgreSQL 字段往返。
- 边缘 `verify-media-lifecycle.py`、运行历史、环形留存、有界关停和视频路由所有权回归通过；Python 编译、Node 语法、部署脚本语法和 `git diff --check` 通过。
- 本轮没有修改或清空真实盒子数据，没有应用生产迁移、删除真实 COS 对象或上传 TestFlight。上线必须先关闭自动生命周期，应用迁移并 dry-run 核对；再启用真实删除。树莓派需要保留数据部署并验证高水位、压缩时长和服务不中断，因此 GH-013 只能标为“待实机验收”。

## 164. 2026-08-02 盒子真实数据库完整性修复与压缩

### 实机发现与根因

- 保留数据部署后，真实 `agent.db` 有 `10,476` 条外键违规，集中在 `upload_jobs/events/event_candidates/rule_evaluations/snapshots/detection_results -> cameras`。历史摄像头 1-30 已被物理删除，活动摄像头为 31、32。
- `Storage.connect()` 从未启用 `PRAGMA foreign_keys=ON`，旧 `delete_camera()` 可以删除仍被历史证据引用的主记录。开启外键后又暴露生命周期任务以 `NO ACTION` 引用快照，导致正常快照回收被约束拒绝。

### 根因修复

- 每个 SQLite 连接强制启用外键。摄像头删除改为软归档：关闭运行状态，清空账号密码，以 `archived:{id}` 替换流地址，禁用摄像头并写入 `deleted_at`；活动列表、详情、编辑、在线状态、宠物状态和首页统计统一排除归档行。
- 初始化时从所有引用摄像头的子表收集缺失主键，恢复无凭据的历史摄像头锚点。该修复先于事件同步状态迁移执行，避免在外键开启后继续更新非法子记录。
- `media_lifecycle_jobs.snapshot_id/asset_id` 改为 `ON DELETE SET NULL`。旧表使用幂等事务重建，失效历史引用置空，但保留任务、`target_id`、错误、重试与完成审计。

### 验证结果

- 本地旧约束迁移、生命周期、运行历史、环形留存和配置同步验证通过；Python 编译、shell 语法和 `git diff --check` 通过。
- 盒子部署后 `PRAGMA foreign_key_check` 返回 0 行，`PRAGMA integrity_check` 返回 `ok`；30 条历史摄像头没有凭据或真实流地址，产品读取仅返回 31、32 两路活动摄像头。
- 手动受控压缩把数据库从 `735,203,328` 字节缩至 `159,166,464` 字节，回收 `576,036,864` 字节。服务重启后 `ActiveState=active`、`SubState=running`、`NRestarts=0`。
- GH-013 的盒子完整性阶段通过；生产 PostgreSQL/COS dry-run、真实删除门禁、盒子长时高水位和断电恢复尚未验收，整体状态不关闭。

## 165. 2026-08-02 RTSP 单一所有者与 Pose 同帧历史

### 实机定位

- 真实健康指标确认 31 号摄像头为 `640x360`、声明并测得约 15 FPS；同一设备主码流 `/1/1` 为 `2880x1620@15`，不能通过切换主码流获得 30 FPS。32 号地址 `192.168.1.3` 可见于邻居表，但 RTSP 和常用管理端口均拒绝连接。
- 离线时存在两条建连路径：托管 `_SharedStreamReader` 每 0.35 秒重试，worker 的 `capture_frame()` 等待后又打开临时连接。第二路故障会刷日志、消耗 CPU，并阻塞第一路算法调度。
- Continual Pose 只暴露最新元数据。渲染帧 N 时 Tracker 若已推进到 N+1，正确的 N 元数据无法再读取，只能被标为 superseded，造成骨架间歇缺失。

### 实现

- `_SharedStreamReader` 统一拥有网络采集，失败按 0.5、1、2、4、8 秒退避并保持 8 秒上限；状态增加 `open_attempt_count / consecutive_failures / next_retry_in_seconds`。收到真实解码帧后才清零。
- `capture_frame()` 发现托管读取器已 retrying 时立即返回同一 `CameraError`，warming 时仅等待中央帧；禁止再调用独立 `_capture_frame_unlocked()`。抓图、算法、管理流和云中继由此共享唯一 RTSP 解码器。
- `ContinualPoseTracker` 按摄像头保存最近 64 帧不含像素的 Pose/上下文元数据；`PrivacyFrameRenderer` 优先按 `camera_id + source_key + frame_id` 获取精确记录，错误流代拒绝，摄像头重置时清空。

### 验证

- 本地 18 项边缘核心回归全部通过，覆盖 Hailo 分割 runtime、共享采集、断线恢复、节拍、adaptive worker、Pose/KLT、隐私、同帧同步、单在途上传、跌倒、推理调度、去重、配置同步、生命周期、环形留存、有界关停和路由所有权。
- 盒子部署后离线摄像头只剩一条重试序列并稳定为每 8 秒一次；在线一路解码约 `14.98 FPS`、隐私输出约 `14.92 FPS`、分割中位约 `28 ms`、逐帧公网上传约 `13 FPS`。服务 active、`NRestarts=0`，进程约 58% 单核总量、温度约 59.5C。
- 纯骨架仍需逐路空房校准及真人长时验收；公网逐帧 JPEG 仍是剩余传输瓶颈，GH-010/GH-011 保持处理中。

## 166. 2026-08-02 摄像头地址原位编辑与生产恢复

### 根因与实现

- `CameraConfigurationForm` 在已有摄像头时不渲染连接区，也没有从服务器状态初始化 host、port 和 path；`CameraManagementView/ProfileViewModel/CameraUpdateRequest` 只提交名称、位置和启用状态。
- 云端 PATCH 会合并连接字段并保留空缺密码，但沿用旧 `status/sync_status`，无法明确表达连接配置需要盒子重新应用。
- 云端 `publicCamera` 现返回不含流地址原文、用户名和密码的结构化连接摘要。正式 iOS 编辑页读取该摘要，使用 `URLComponents` 验证和生成 `rtsp/rtsps` 地址；用户名或新密码留空时从请求中省略。
- 云端再次校验 RTSP URL。`stream_url/username/password` 任一真实变化后，同一摄像头写入 `pending_edge_sync`、清除旧错误并由现有配置同步链路原位更新，不增加第二套接口或同步任务。

### 生产恢复与验证

- 生产 PostgreSQL 确认云端 `29 -> 本地 32 -> 冰箱上摄像头` 的旧地址为 `192.168.1.3`。停止组合服务后在单事务中把 secret 地址更新为 `rtsp://192.168.1.7:554/1/2`，并将同一配置置为待同步；服务恢复后没有改动 ID、密码、绑定或历史关系。
- 盒子自动同步后，本地摄像头 `32` 为 `online` 且使用新地址；生产云摄像头 `29` 为 `online / synced`。公网健康指标恢复约 `12.5 FPS`、传输延迟 P95 约 `161 ms`、stale reject 为 `0`。
- 生产服务器按线上 `server.js` 基线仅合并连接摘要与重同步状态机，未用开发分支整文件覆盖。语法检查、生产设备专项 `2/2` 和公网健康通过，服务保持 active。
- 正式 `GoHomeShell` 的模型测试 `6/6`、权限与设备状态测试 `11/11` 通过。下一 TestFlight 仍需实机点击编辑和空密码保留；通过前 GH-021 状态为“待实机验收”。

## 167. 2026-08-02 源生命周期、唯一成品流与生产协议部署

### 根因收口

- 运行中修改摄像头 URL 或凭据原来只更新 SQLite，旧托管读取器仍连接旧地址；同一 URL 发生读失败或像素冻结时也没有统一失效 Pose、分割、背景和中继状态。
- 生产云会话仍向骨架模式返回 `safe-scene-pose-v1`，实际 MJPEG 缺少正式 iOS 强制校验的传输和组合所有者响应头，因此 TestFlight 在解码前直接拒绝响应。
- 算法诊断页曾根据隐私模式选择不同视频来源，并可能使用 Tracker 保留的旧像素，造成诊断骨架与当前摄像头帧职责混淆。

### 实现

- `CameraAgent` 统一拥有摄像头源生命周期。配置变化原子退休旧读取器；读失败、主动重连或持续重复像素会清除中央缓存、提升流代次并发布统一失效事件。Worker、隐私渲染器与直播中继只响应这一事件清空各自状态。
- 重复像素单独计入 decoded 指标，不创建有效帧号，不刷新有效帧年龄，也不进入 Tracker。管理端显示真实 effective FPS，不能以重复帧伪造实时性。
- 用户流只有盒子合成的一条 `edge-composed-mjpeg-v1` 成品流；管理首页与 App 显示同一流。算法页固定使用当前摄像头帧和完全匹配 `camera_id + source_key + frame_id` 的诊断 Pose 元数据。iOS 与云端均不再绘制第二套骨架。
- 生产云在现有线上源码基线上最小合并会话和响应头契约，没有整文件覆盖。正式播放会话不再返回 Pose/scene 路径；缓存 MJPEG、空流、静态回退和设备代理统一声明 `composition-owner=edge`。
- 部署脚本改为模式化清理：盒子只保留 `verify-vision-runtime.py`，其余 QA 脚本、字节码、历史备份和旧视频模块均不进入正式运行目录。

### 验证与剩余项

- 边缘端完整回归 `48/48` 通过。生产真实鉴权播放会话与 MJPEG 响应头通过，临时会话已删除；云端和盒子服务均 `active / NRestarts=0`。
- 盒子两路有效输入约 `14.6 / 13.6 FPS`，云端完成接收约 `14.3 / 13.6 FPS`，上传失败为 `0`。摄像头 32 使用 `192.168.1.7`，新进程未再访问旧 `192.168.1.3`。
- 原画和人物模糊成品流已取得持续 JPEG。新的源身份契约使历史空房校准失效，纯骨架保持拒绝输出而不回退为模糊或伪造画面；必须在两路画面无人时重新显式校准，再做管理首页、算法页、TestFlight 和 30 分钟长时验收。

## 168. 2026-08-02 双摄空房校准与六路短测

- 先通过受鉴权管理端原画流确认两路现场无人，再分别校准本地摄像头 31、32。两路均以当前配置源身份和 `640x360` 分辨率完成 8 帧稳定空房采集、0 次人物拒绝及 3 帧当前流代确认，最终状态均为 `ready`。
- 校准没有修改摄像头 ID、绑定、地址或凭据。背景基线按摄像头、配置源身份和分辨率持久化；连接配置或场景变化时继续按隐私安全契约失效，不会把旧背景用于新源。
- 逐路验证 `original / person_blur / skeleton` 共六条盒子成品流。所有流均持续返回 JPEG，并统一携带 `X-GoHome-Display-Transport: edge-composed-mjpeg-v1`、`X-GoHome-Composition-Owner: edge` 和正确的解析后隐私模式。
- 12 帧短测实际出帧约 `8.4-12.1 FPS`；该结果只证明校准后的空房链路恢复，不代表纯骨架真人效果或端到端性能达标。下一门槛是两路真人动作、管理首页与 TestFlight 同帧视觉核对、算法诊断页独立骨架核对，以及 30 分钟双摄长时测试。

## 169. 2026-08-02 追踪丢失时旧骨架显示根因修复

### 录像结论与根因

- 复查 `20260802213732.mp4`：视觉算法诊断页中的站立、坐下和躺下大部分时间能够跟随，但约 32.6 至 33.4 秒人物向右移动时，旧骨架在沙发附近滞留约 0.9 秒；页面同时报告 `短暂补偿`，最大帧龄约 `906 ms`。
- 该页保留人物像素并叠加黄色骨架符合诊断页定义，问题不是隐私合成，而是两套渲染器错误接受 Tracker 的 `coasting`。该状态保存最近可信几何以触发恢复，本身不代表骨架已在当前帧被观测或跟踪。

### 单一状态契约

- 管理诊断骨架和纯骨架成品流只允许绘制 `observed / tracked`；`coasting`、`display_only_stale`、来源不匹配和帧身份不一致全部不可显示。
- 纯骨架合成不再使用旧 coasting Pose 扩张人物遮罩。当前 Hailo 分割继续从当前帧移除人物；Pose 暂时失效时不画错误骨架，也不回退为人物模糊、黑场或历史人物画面。
- 管理元数据对旧状态保留恢复诊断信息，但 `available / frame_available` 为 false，人物和姿态列表为空，前端没有第二条路径可重画旧骨架。
- `AdaptiveInferenceScheduler` 提供统一的 `request_refresh()`。Worker 在追踪进入 coasting 或 stale 时立即请求该摄像头刷新 Hailo 锚点；请求在推理进行期间到达也会保留到下一轮，不通过风险状态冒充调度原因。

### 本地验证与部署边界

- 同步诊断流、视频隐私、连续姿态 API、连续追踪和自适应 Worker 专项回归通过；边缘完整回归 `48/48`，Python 编译与 `git diff --check` 通过。
- 本节代码尚需部署到树莓派。部署后必须确认服务单进程、`NRestarts=0`、Hailo runtime 就绪、两路校准仍为 `ready`，再重复相同动作。两路均无滞留骨架后才能把 GH-025 标为通过，并继续 30 分钟双摄稳定性验收。

## 170. 2026-08-02 管理端静态入口收敛

- 正式管理端唯一页面集合固定为登录、首页、摄像头配置、视觉算法和事件日志；唯一 JavaScript 运行时为 `login.js` 与 `console.js`。
- 删除旧 `app.js / detection.js / vision.js`。三者共 2,009 行，重复拥有摄像头、抓帧轮询、检测框、规则和盒子测试通知职责，且旧 `app.js` 已引用不存在的页面元素。
- 删除六个纯跳转 HTML 和 `redirect.js`，共 106 行。生产静态目录不再保留已合并页面或历史 URL 兼容实现。
- 新增 `verify-admin-static-contract.py`，使用 HTML 解析器核对精确页面/脚本清单、逐页依赖和依赖文件存在性，防止旧入口重新进入生产包。
- 专项契约、`console.js / login.js` Node 语法、Python 编译、差异检查及边缘完整回归 `48/48` 通过。GH-044 本地根因已消除，部署后只需验收五个正式页面加载和操作，不恢复任何旧入口。

## 171. 2026-08-02 管理端算法展示去演示化

- 正式视觉算法页没有模式选择器和算法示意容器，运行时固定为统一感知；删除与实际 DOM 不一致的八模式切换、演示档案、视频加载和错误处理。
- 删除 8 个算法演示 WebM。静态资源契约现在要求管理端 `assets` 为空，生产部署不能重新引入示意视频。
- 删除由亮度、对比度、运动、人物存在等规则量人工组合的“通过率、健康度、可信度”和火灾分数 `* 2800` 换算。算法页只展示后端当前帧的目标计数、模型名称、状态机结果和可测延迟/帧龄。
- `console.js` 本项净删除 643 行。管理端静态契约、Node 语法、差异检查与边缘完整回归 `48/48` 通过；GH-045 本地根因已消除，待盒子部署后核对算法页真实字段。

## 172. 2026-08-02 移除未经验证的火灾暖色启发式

### 根因

- `FireAnalyzer` 只依据暖色像素、纹理、连通域和帧间变化计算分数，却在产品中被包装成火灾应急能力。
- 该分析每帧占用 CPU，连续五次命中后会进入严重安全事件、证据上传、云端复核和通知链路，电视、暖灯和红色物体均缺少可证明的误报边界。
- 正式规则 Schema、SQLite、管理端开关和评估脚本继续暴露这项未经验证的能力，单纯关闭通知会留下死代码和错误产品承诺。

### 实现

- 删除 `FireAnalyzer` 及视觉管线调用，正式分析结果和证据包不再包含 fire 算法、分数、阈值、标签或候选字段。
- 删除规则引擎火灾计数状态、严重事件生成、云端复核资格和通知行为；管理端移除火灾开关、实时状态和活动候选范围。
- 删除 `RulesUpdate` 与 SQLite `rules` 表的五项火灾配置；启动迁移幂等删列并保留全部非火灾设置及更新时间。
- 删除失效火灾评估入口与合成测试。保留历史 `events/event_candidates` 通用读取和事件页标签，旧记录仍可审计但没有新生产入口。

### 验证

- 旧规则表迁移后五个废弃列全部消失，保留配置值和 `updated_at` 不变；旧配置补丁不会重新写入。
- 连续十帧注入旧 fire 字段不会产生候选或规则状态；历史火灾事件仍能按原类型和载荷读取。
- 视觉管线输出算法集合固定为 `quality/person/pose/fall/activity`；聚焦视觉、持久化、事件去重、管理静态契约和完整边缘回归均通过。

## 173. 2026-08-03 签名升级包与可信运行

### 根因

- 旧发布记录只复制资产元数据中的摘要，安装阶段不重新计算；升级包没有受信任发布者身份。
- ZIP/TAR 直接 `extractall()` 到最终版本目录，允许目录穿越、链接和资源耗尽；版本号、入口路径和已安装路径均可越界。
- 当前清单在运行健康检查前被改写，失败时再尝试恢复多份清单；运行守护根据后缀和执行位直接启动未验证文件。

### 实现

- 新增独立 `PackageTrustStore`。Ed25519 清单版本 1 对发布作用域、包体身份、入口类型和安装策略统一签名；服务端记录发布和盒子执行时均重新验签、计算大小与 SHA-256。
- 包体只进入私有暂存目录。ZIP/TAR 逐成员检查并复制，拒绝绝对路径、父目录、反斜杠路径、重复成员、符号链接、硬链接、设备节点、加密 ZIP、过多成员和过大展开体积。
- 校验后的版本原子移动到 `releases/{type}/versions/{version}`，同版本不同内容永久拒绝；入口必须是已验证负载根内的普通文件，安装文件索引和摘要在每次启动前复验。
- 运行命令由签名 `entry_type` 明确决定。新进程通过启动宽限期后，在同一激活事务中原子切换 `current.json`；失败或清单写入失败时只恢复经过同样验证的上一版本。
- 增加包类型级 `flock`，避免手动升级、配置同步和多进程同时切换。清单和运行状态使用临时文件、`fsync` 与 `os.replace` 原子写入。
- 私钥只用于发布环境的 `tools/sign-package.py`，部署脚本排除整个 `tools/`；盒子只配置 PEM 公钥。`cryptography` 作为独立安全依赖进入正式部署。

### 验证

- 确定性回归覆盖合法 ZIP/单文件包、包体篡改、错误签名、错误大小/摘要、不可变版本冲突、ZIP/TAR 穿越、绝对路径、符号/硬链接、入口逃逸、设备范围、成员/展开体积限制、安装后篡改、启动失败回滚和激活失败回滚。
- Python 编译、部署 shell 语法、签名工具实测和边缘完整回归 `51/51` 通过。GH-038 进入待实机验收，不能在公钥配置、真实升级、断电和重启恢复完成前关闭。

## 174. 2026-08-03 边缘媒体职责删除与事件证据流式上行

### 根因

- 边缘 `ObjectStorageService` 同时承担家庭上传、公开下载、设备媒体接收、事件快照提升和升级包资产，和生产云/COS形成双数据源；上传会话还引用未定义的 `event_id`。
- `UploadAgent` 使用 `Path.read_bytes()`，边缘接收和生产云接收使用完整请求缓冲，文件大小直接放大进程内存。

### 实现

- 删除 `object_storage_service.py` 及边缘 `/api/v1/media/*`、`/api/public/media/*`、`/api/v1/device/media-assets*` 路由；本地事件只保留原快照引用，不再复制对象资产。
- `UploadAgent` 使用 `HTTPConnection/HTTPSConnection` 直接发送文件对象和明确 `Content-Length`；现有持久队列、租约、指数退避和事件前置媒体完成约束保持不变。
- 生产云接收端先校验请求长度和 16 MB 默认上限。COS 模式把原始请求流直接交给 SDK；本地模式通过同目录临时文件、字节数复核和原子重命名提交。
- 升级包拆为 `PackageArtifactService`，路由、令牌、对象键和留存类别全部使用 package-artifact 语义；上传使用异步流和临时文件，声明大小不一致立即失败并清理。
- 初始化迁移将历史 `package_releases.asset_id` 对应资产标记为 `package_artifact`，保证已签名历史发布不被生命周期误删或因新域检查失效。

### 验证

- 新增 `verify-edge-media-ownership.py` 与 `verify-package-artifact-service.py`；更新上传代理和 COS 证据测试，锁定旧路由不得恢复、文件不得整包入内存、超限不得残留临时字节。
...
## 176. 2026-08-03 骨架显示与跌倒证据解耦

### 根因

`RtmposeAnalyzer._result_from_raw_poses()` 原来将 `_fall_evidence_quality()` 同时作为显示、人物和跌倒入口。少于 8 个可见点、核心肩髋不足或总体置信度低于跌倒门槛时，Pose 被直接移入 `rejected_poses`，导致正常半身、进餐和遮挡场景无骨架；同时下游无法区分“当前能显示”和“足以确认风险”。

### 单一数据契约

- `poses`：当前帧、几何有效、达到最低显示质量并通过后续人物一致性的唯一骨架流。
- `display_evidence_eligible`：允许进入显示和诊断。
- `person_evidence_eligible`：允许进入后续人物一致性判断；不能绕过独立目标与运动门禁。
- `fall_evidence_eligible`：允许进入正式跌倒分数、场景聚合和 Pose 因子图。
- `display_quality / fall_quality`：分别保存阈值、计数和拒绝原因。
- `raw_fall_score`：仅供诊断；风险不合格时 `fall_score=0` 且移除 `fall_candidate`。

### 代码调整

- `pose_rtmpose.py` 新增显示质量检查，最低要求 4 个当前可见关键点、有效有限几何和独立显示置信度；严格跌倒质量不再删除显示合格 Pose。
- `pipeline.py` 的场景跌倒聚合只读取风险合格 Pose；短时缓存 Pose 明确设为 `fall_evidence_eligible=false`；输出三类骨架计数。
- `pose_factor_graph.py` 只以明确声明风险合格的 Pose 建图，人物框仅补充同一合格轨迹的场景字段，不再单独创建 Pose 风险目标。
- 新增 `verify-pose-evidence-boundaries.py`，并更新视觉管线和因子图永久回归。

### 本地验证

- 半身 Pose：可显示，正式跌倒资格数为 0，正式跌倒分数为 0。
- 完整 Pose：显示和跌倒资格均保留，可形成跌倒候选。
- 无效几何：进入显示质量拒绝，不进入当前 Pose 流。
- display-only Pose：场景聚合和因子图均不能重建风险。
- 既有因子图、跌倒规则、视觉管线、连续跟踪、同步骨架流和纯骨架隐私流专项回归通过；边缘完整回归 `55/55` 通过。

### 待实机验收

树莓派两路摄像头分别执行进餐、桌面遮挡、侧身、半身进入、站立静止、快速倒地和离开。管理算法页与 App 骨架模式应保留真实可见骨架，无风险质量的 Pose 不生成事件；完整倒地序列仍进入原有云端复核链路。连续 30 分钟记录显示 FPS、模型锚点 Hz、帧龄、显示骨架数和风险合格骨架数。

## 177. 2026-08-03 EACP 可复现评估与质量声明门禁

### 根因

- 帧评估只输出 Precision、Recall 和 FPR；跌倒序列脚本另写一套公式，两个结果无法严格比较。
- v1 报告不绑定算法、模型、阈值、数据内容和 split，复制或修改样本后仍可能沿用旧结论。
- 视频级标签可被逐帧重复使用，单次 EACP 实机验收又可能被误写成准确率证明；现有公开样本没有独立测试集、受试者隔离和真实家庭覆盖。
- 已退役火灾能力仍留在数据目录门禁和 EACP 安全事件集合；公开样本注入脚本还能制造正式事件，污染运行验收语义。

### 唯一评估实现

- `eval_vision_common.py` 统一混淆矩阵和二分类指标，增加 Specificity、F1、Accuracy、Balanced Accuracy、FNR 及 Precision/Recall/Specificity/Accuracy 的 95% Wilson 区间。
- 每行结果保留数据集、类别、受试者、视角、split 和 sequence ID；报告自动生成按四个维度的分层指标及完整 FP/FN/读取错误清单。
- 可复现元数据包括 Git revision/dirty、核心算法文件 SHA-256、模型 SHA-256、规范化配置 SHA-256、清单与全部引用媒体内容指纹、命令、Python 和平台。
- `eval-fall-sequences.py` 删除独立指标公式，复用公共实现；序列标签缺失不再默认为负例，跨 split 的同一序列直接记为错误。

### 数据审计与门禁

- `audit-vision-dataset-readiness.py` v2 检查文件、标签、序列 ID、帧顺序、序列元数据、split 完整性、序列/受试者/内容泄漏和重复内容。
- 质量层级拆为规则回归、真实家庭误报 pilot、时序模型实验和产品质量声明。产品声明要求独立 test、三 split 齐全、数据完整且无泄漏，测试集至少 30 正序列、50 负序列、3 个来源。
- 当前 239 条清单、240 张图片中，去除同源稀疏/密集重复计数后只有 25 个独立跌倒/ADL 时序来源，12 正、13 负、2 个公开数据集和 6 张真实家庭负样本；无 split 和完整姿态序列。因此仅 `fall_rule_regression` ready，其余门禁全部 insufficient。
- 删除 D-Fire/火烟视频目录项、EACP 的 `fire_candidate` 安全事件资格及 `emit-public-fall-validation.py`；部署脚本同步删除无意义排除项。历史火灾事件只读审计策略不受影响。

### 验证

- `verify-evaluation-integrity.py` 覆盖统一公式、置信区间、分层指标、稳定数据指纹、内容变化、序列/受试者泄漏和质量门禁。
- `verify-dataset-readiness-audit.py` 覆盖 v2 Schema、退役火灾门禁消失和四层 readiness 结果。
...
## 180. 2026-08-03 近黑帧身份与预览旧画面删除

### 根因

- 唯一共享读取器曾把每张不同内容的近黑帧当作有效帧写入中央缓存并推进身份。
- `CameraAgent.raw_frames()` 又为每个订阅者保存 `last_good_frame`。读取近黑缓存时，输出像素来自上一有效帧，`frame_id`、采集时间、源键和流代却来自当前近黑帧，形成旧人物像素伪装成实时新帧。
- 黑帧状态由订阅者按请求 FPS 独立计算，同一摄像头的管理端、云中继和算法可能对“当前有效帧”得出不同结论。

### 单一状态机

- 近黑判定进入 `_SharedStreamReader`，发生在内容指纹、中央缓存写入和有效序号推进之前。近黑帧只更新 decoded/read-latency 与独立 near-black 计数。
- 首个近黑帧清除该摄像头中央实时缓存、unique/effective FPS 窗口、最后有效时间和内容指纹，状态立即进入 stale，避免抓图、Worker、隐私合成或新订阅者继续取得旧画面或看到假 FPS。连续至少 5 帧且持续 0.75 秒后，读取器释放 capture、提升 `stream_generation`、发布统一 source transition 并使用既有有界退避重连。
- 近黑恢复会清空连续状态，第一张真实有效帧按新流代进入唯一中央缓存。`raw_frames()` 只负责从中央缓存按目标 FPS 取最新有效帧和缩放，不再拥有黑帧判定、旧帧、重连或第二套身份状态。
...
## 183. 2026-08-03 算法诊断与纯骨架成品流分层

### 现场事实

- `algorithms.html` 的唯一视频元素固定连接 `/continual-pose/stream.mjpg`，由盒子绘制黄色诊断骨架；查询参数中的隐私模式不会改变该诊断端点。
- 同一脚本曾在所有页面插入“原画 / 模糊 / 骨架”控制，因此算法页选择状态与中央诊断画面无对应关系，造成产品语义错误。
- `/health` 显示云端期望和逐路模式均为 `skeleton`，但摄像头 31、32 都是 `calibration_required`；`privacy-calibrations` 目录无 `.npz`，LiveRelay 两路 submitted/completed 均为 0，不能宣称纯骨架已可用。

### 单一页面契约

- 算法页动态侧栏只展示“算法诊断画面 / 原画 + 黄色骨架”和用户画面同步状态，不生成隐私模式按钮。
- 管理首页和摄像头页生成唯一一套“App / 首页画面”控制；事件页不显示无关隐私控制。纯骨架名称明确区别于诊断骨架。
- 算法诊断流继续由盒子单次合成，客户端不绘制第二层骨架；首页/App 继续消费盒子隐私成品流，不复用诊断端点。

### 未校准边界

- 管理首页在所选摄像头纯骨架未就绪时撤下 `img.src`，显示逐路校准或场景复核状态，并将显示 FPS 置为未知，不创建无限重连。
- `camera_mjpeg_stream()` 在创建 `StreamingResponse` 前调用逐路就绪门禁；纯骨架未就绪返回 HTTP 409 和 `code/message/camera_id/calibration`，原画与人物模糊不受影响。
- 管理端 API 错误解析不再把对象转成 `[object Object]`，优先显示结构化中文 `message`，非 JSON 错误使用确定的 HTTP 文案。
- 自动契约新增诊断/隐私页面所有权、未校准前端阻断、未校准 409 与校准后允许流建立。

### 部署与运行验证

- Node 语法、Python 编译、诊断/隐私控制、校准 API、隐私渲染和路由所有权专项均通过，完整边缘回归 `58/58` 通过；源码测试字节码已清理。
...
## 185. 2026-08-03 双摄校准现场与管理流会话隔离

### 现场证据

- 直连 RTSP 确认摄像头 31（`192.168.1.11/1/2`）是电视柜/沙发视角，摄像头 32（`192.168.1.7/1/2`）是冰箱/电视墙视角；两路源配置和帧内容均不同，未发现后端配置串源。
- 摄像头 31 在无人画面完成正式校准，状态 `ready`、`baseline_retained=true`、`baseline_revision=1`；持久文件为 `camera-31-9050304a85ab40a5-640x360.npz`，大小 `427376` 字节，文件 SHA-256 为 `291639114ffe4064d3e11e503a150ccb7b8b4f5dbd56bc992bf070a970256bd8`，基线像素 SHA-256 为 `cc8ff649c3611f51ae063ba1c089758140415c48c56d12d4566750ffcab5f969`。
- 摄像头 32 的真实电视墙画面在校准时出现人物，后端累计拒绝并返回 `person_present`，没有提交背景。这证明人物安全门禁生效，也说明该路必须等真实空房后再操作。

### 根因与实现

- 管理端摄像头选择、标题和新 `src` 会立即更新，但浏览器在同一 `<img>` 节点上继续保留上一条 MJPEG 的最后解码像素；旧实现还在 900 毫秒后无条件隐藏加载层，因此前端把旧画面展示在新摄像头标题下。
- `renderStream()` 现在每次建立或恢复流都替换为全新图像节点，先移除 `src` 并隐藏节点；旧节点被销毁，不允许其像素继续占据当前视图。
- 全局 `streamGeneration` 与所选 `camera_id` 共同约束 `load/error`，只有当前会话真实首帧才能揭示画面；过期事件不能改变新会话状态。移除 `streamMaskTimer`，加载层不再按时间猜测成功。
- 空房校准请求按摄像头进入唯一 pending 集合；页面轮询重绘只呈现该摄像头的处理中状态，重复点击不会产生第二次 POST。提示消失时同步清空 ARIA 文本，避免旧成功文案干扰后续判断。

### 验证边界

- Node 语法、管理静态契约和完整边缘回归 `57/57` 已通过；契约锁定新图像节点、首帧前隐藏、会话代次拒绝、校准请求所有权，并禁止重新引入定时揭示。首次回归还发现 CSS/JS 缓存版本不一致，现已统一为同一原子版本后重新全量通过。
- 后续必须部署到 Pi 后反复切换两路、断开一路并等待重连，确认新首帧之前只显示加载状态且从不出现旧摄像头像素。
- 摄像头 32 需在真实画面无人时完成持久校准。两路 `ready` 后才能进入纯骨架真人与 TestFlight 验收；当前不得把单路成功写成双路完成。

### 首轮实机反证与连接生命周期补全

- 首轮版本部署后，新标签页能正确加载绑定与两路摄像头；从 32 切到 31 的同步观察证明旧画面立即隐藏，`naturalWidth=0` 且加载层持续显示，31 路真实首帧到达后才变为可见。
- 继续快速双路循环时，新流最终超过 3 秒仍无首帧。Pi 指标显示共享源订阅者从正常每路约 2 个增加到摄像头 31 的 4 个、摄像头 32 的 5 个，证明脱离 DOM 的旧 `<img>` 仍持有 MJPEG HTTP 请求。
- 连接根因修复不再只做节点替换：`renderStream()` 先提升代次，使任何同步旧事件立即失效；`replaceLiveStreamElement()` 随后隐藏旧节点、清空旧 `load/error`、移除旧 `src` 触发请求取消，再替换成无源隐藏节点。
- 静态契约新增旧事件解除、旧请求取消和“代次先于替换”顺序检查；四个管理页面的 CSS/JS 原子缓存版本统一升级为 `stream-lifecycle-2`，保证部署后浏览器实际获取第二轮脚本。
- 第二轮 Node 语法、静态契约和完整边缘回归再次 `57/57` 通过。下一轮必须用同样 20 次切换复验订阅数不增长，不能只看画面不串就关闭问题。

### 第二轮 Pi 验收结果

- 提交 `a59e1ef` 已推送，Pi 只同步五份管理端静态文件，不重启后端；四页实际加载 `stream-lifecycle-2`，服务继续 `active`、`NRestarts=0`。
- 真实管理首页执行 31、32 两路交替切换 20 次。每次同步状态均为唯一新 `<img>`、目标摄像头 URL、`visibility=hidden`、`naturalWidth=0` 和加载层可见；真实首帧后才变为 `visible`，宽度 640，摄像头名称始终匹配。
- 20 次首帧耗时最小 `494 ms`、最大 `1312 ms`、中位 `751 ms`，没有 3 秒超时。测试前后订阅均为摄像头 31 的 2 个、摄像头 32 的 4 个，证明旧 HTTP 会话已终止且没有累积。
- 验收结束时两路源流约 `14.6-15.0 FPS`、重连数为 0、Hailo Pose/Object 失败为 0。GH-050 只剩“主动断开一路时显示不可用且无旧像素”场景，尚未关闭。
- 同时发现摄像头 31 的持久校准文件在磁盘上仍存在，但服务重启后管理页显示“未校准”；当前运行模式为模糊，背景重建器尚未加载状态。该问题回到 GH-003 的持久资产发现/复验语义处理，不能误写成校准文件丢失，也不能要求用户重复校准。

## 186. 2026-08-03 持久基线启动复验与 Hailo 锚点限频

### 启动状态根因

- 持久空房基线原先只在骨架重建路径按精确尺寸惰性加载。服务重启后若当前为原画或人物模糊，背景状态尚未实例化，管理端就把磁盘上真实存在的基线误报为 `calibration_required`。
- 启动装配现在按当前摄像头配置发现对应 `.npz`，加载后明确区分 `calibrated / baseline_retained / ready`：已有基线但当前流代尚未确认时为 `revalidating`，不是未校准，也不要求用户重复采集空房。
- 复验观察从显示模式中解耦，原画和人物模糊期间同样可以完成；背景状态用 `last_revalidation_token` 保证同一源帧不能被多个消费者重复计数。

### 人物安全门禁反证

- 首个版本依赖同步 Pose 和背景残差判断。真实有人画面在 Pose 未稳定覆盖全部人物时，人物占用区域可能被判作允许的局部场景变化，导致错误进入 ready。
- 复验现在调用人物分割后端的 `segment_anchor()`，强制对当前 `camera_id + source_key + frame_id` 执行真实 Hailo 锚点，拒绝直接使用时序传播掩码。
- Pose 或当前分割掩码任一包含人物都将复验进度重置为 0、状态保持 `revalidation_required`、错误为 `person_present`；Pose 身份不同步或分割不可用则返回 `person_evidence_unavailable`，不能用“未检测到”替代证据缺失。

### 有界复验调度

- 强制锚点首版在每个渲染帧入口执行。有人持续停留时摄像头 31 会以实时帧率重复分割，既不会推进复验，又会持续争用 Hailo 和 CPU。
- `PrivacyFrameRenderer` 新增按 `camera_id + 完整 source_key + width + height` 的线程安全调度状态。每路待复验流首次立即检查，之后每 `1.0 s` 最多一次；槽位在昂贵元数据等待和 Hailo 推理之前原子领取，避免管理端与云中继并发重复执行。
- 每次允许检查仍是新的源帧和独立 Hailo 锚点，三次空房确认规则不变。切换源、完成复验、开始校准或重置摄像头时清理对应调度状态，旧源不会积累后台任务。
- `/health` 的 `privacy_renderer.revalidation_scheduler` 公开间隔、活动流、最后帧身份和下一次检查剩余时间，便于现场区分“人物阻断”和“推理失控”。

### 自动与 Pi 验证

- `verify-video-privacy.py` 使用注入单调时钟，证明连续 12 张不同实时帧只执行首个锚点、`0.99 s` 前不重试、到 `1.0 s` 才执行下一锚点；有人时进度始终为 0，空房三次检查必须分别推进后才 ready。
- 隐私专项、Hailo 强制锚点专项、Python 编译和完整边缘回归 `58/58` 通过。提交 `5642c9d` 已推送并只同步受影响实现到 Pi，未覆盖 `data/`、模型、环境或摄像头配置。
- 服务重启后 `active`、`NRestarts=0`；两路源流约 `14.75-15.05 FPS`，重连 0，Pose 与人物分割失败 0，云配置同步连续失败 0。摄像头 31 调度器保持单一活动流、间隔 `1.0 s`。
- 摄像头 31 的持久 `.npz` 文件 SHA-256 部署前后均为 `291639114ffe4064d3e11e503a150ccb7b8b4f5dbd56bc992bf070a970256bd8`，基线像素 SHA-256 为 `cc8ff649c3611f51ae063ba1c089758140415c48c56d12d4566750ffcab5f969`，证明重启复验未修改基线。
- 20:51 的最新证据帧中可见两个人，Hailo 输出两个人框，背景状态正确保持 `revalidating / person_present / 0/3`。摄像头 32 仍为 `calibration_required` 且无持久基线；真正双路空房重启、纯骨架和 TestFlight 验收仍未完成。

## 187. 2026-08-03 摄像头 32 真实空房校准

### 现场操作与结果

- 通过当前直连画面确认摄像头 32 的冰箱/电视墙视角真实无人后，从已登录管理首页只对该路发起一次正式空房校准，没有删除摄像头、修改连接配置或绕过人物门禁。
- 校准完成 `8/8` 稳定空房观察，人物拒绝数为 0；提交后继续完成当前流代 `3/3` 独立 Hailo 人物复验。最终状态为 `status=ready`、`ready=true`、`calibrated=true`、`baseline_retained=true`、`baseline_revision=1`、`last_error=""`。
- 持久文件为 `/home/gohome/gohome/edge-agent/data/privacy-calibrations/camera-32-08e18ef779e5437d-640x360.npz`，大小 `369281` 字节，文件 SHA-256 为 `8a2cda89ea9412841142193ef7ea9114c4ccb8c9ca51d7ec271ddd2a4fd63344`；解码后的基线像素 SHA-256 为 `05eae00d1c0faa0fc818d05e68a70a707130d91e9e3647e1e63e456918a49aab`。两种哈希口径不混用。

### 同时段运行状态与剩余边界

- 管理首页显示冰箱上摄像头“已校准”、电视柜上摄像头“复验中”；盒子 `/health` 与页面一致。服务为 `active`、`NRestarts=0`，两路源流约 15 FPS、重连为 0、Hailo Pose/分割失败为 0、云配置同步连续失败为 0。
- 摄像头 31 的既有持久基线保持不变；当前画面仍有人，强制 Hailo 复验正确返回 `person_present / 0/3`。不得为追求双路 ready 重新校准、复用摄像头 32 的状态或绕过安全门禁。
- 当前只证明摄像头 32 的校准事务、持久化和流代复验闭环。摄像头 31 自动复验、双路纯骨架、生产云与 TestFlight 同帧显示、重启恢复和 30 分钟稳定性仍未完成，不能标记 GH-002 至 GH-008 已关闭。

## 188. 2026-08-03 自动复验与单路纯骨架实帧

### 摄像头 31 自动复验

- 电视柜摄像头画面真实无人后，已有持久基线自动完成 `3/3` 独立 Hailo 人物复验；状态进入 `ready`，`revalidation_scheduler.active_streams=0`，没有人工重校准。
- 摄像头 31 文件 SHA-256 保持 `291639114ffe4064d3e11e503a150ccb7b8b4f5dbd56bc992bf070a970256bd8`，摄像头 32 文件 SHA-256 保持 `8a2cda89ea9412841142193ef7ea9114c4ccb8c9ca51d7ec271ddd2a4fd63344`。服务 `active`、`NRestarts=0`，两路源流约 15 FPS、重连为 0。

### 纯骨架切换与视觉证据

- 两路同时 `ready` 时，通过已登录管理首页将唯一用户隐私模式由 `person_blur` 切换为 `skeleton`；云端配置、逐路运行模式和管理页选择状态一致，没有客户端二次绘制。
- 摄像头 32 当前管理首页成品帧只包含保留房间背景和一套蓝色骨架；人物原始或模糊像素不可见，没有黑底、中性色块、双骨架、跨摄像头像素或旧人物残留。端侧显示约 `12.8-13.6 FPS`，云端接收约 `10.5-11.4 FPS`，源流约 15 FPS，上传失败为 0。
- 切换后人物重新进入摄像头 31，独立 Hailo 锚点把该路重置为 `revalidating / person_present / 0/3`。该路隐私成品和云端接收 FPS 均为 0，页面显示复验中；系统没有回退原画、重复旧帧或影响仍为 `ready` 的摄像头 32。
...
## 2026-08-03 边缘持久 H.264 发布器

- 删除正式中继中的 `_LatestFrameUploader`、逐帧 JPEG 编码、HTTP keep-alive POST 和逐帧 JSON 响应处理。`LiveRelayAgent` 每路只创建一个 `H264StreamPublisher`，直接消费 `PrivacyFrameRenderer.render_image()` 的 BGR 成品；LAN 管理 MJPEG 仍由独立诊断接口按需编码。
- 每路发布器由单一工作线程拥有 FFmpeg 进程。外部暂停、源切换和关闭只提升控制代次；非阻塞管道写入每 50 ms 检查控制变化并具有总超时，避免写入线程与停止线程同时销毁进程。发布器关闭幂等，异常退出、无 stdin 和阻塞写入均清理进程并采用 0.5 至 8 秒有界退避。
- 输入队列容量固定为一张，编码繁忙时原子替换为最新成品，不补发旧帧。模式或完整源代次改变时重启编码器，确保新隐私模式从新编码上下文和关键帧开始；输入使用源单调时间，FFmpeg 以 wall-clock 时间戳和 passthrough FPS 模式保留真实不规则帧节奏。
- 编码固定为 `libx264 / ultrafast / zerolatency / yuv420p / baseline / B-frame=0 / refs=1 / 约 1 秒 GOP`，奇数尺寸在编码滤镜内补齐到偶数，RTSPS 使用 TCP。状态字段明确区分提交 FPS 和编码器输入 FPS，不能把 stdin 写入表述成云端送达；源代次只公开 SHA-256 短摘要，发布 URL、错误和 stderr 均脱敏。
...
## 194. 2026-08-05 H.264 发布竞态、状态写入与 DIS 长时验收

### 摄像头映射竞态根因

- 长时监控曾出现仅摄像头 32 的 H.264 发布器被重建，但摄像头源未重连、`stream_generation=1`、隐私模式和云端配置版本均未变化。
- 旧直播签名每三秒调用 `remote_camera_id_for_local_camera()`，并直接读取 `config-sync-state.json`。配置同步和隐私更新同时使用截断重写；一路可能在空文件或半写 JSON 窗口得到本地 ID 回退值，签名因此变化并销毁发布器，另一路在写完后读取则保持正常。
- 文件系统状态被错误用作线程间运行时通信，既没有单一所有者，也没有原子读取视图。这是单路无业务变化重启的确定根因，不是 FFmpeg、摄像头或云端随机故障。

### 唯一状态所有权

- `ConfigSyncAgent` 在初始化时加载一次持久状态，并以 `RLock` 保护内存摄像头映射。上传队列和直播签名都调用同一 `remote_camera_id_for_local_camera()`，运行时不再反复读磁盘。
- 配置应用与隐私状态更新共用同一重入锁。持久化先在目标目录创建临时文件，写完后执行文件 `fsync`，再用 `os.replace` 原子替换并对目录 `fsync`；异常时只清理临时文件，不留下部分正式 JSON。
- 状态变化比较使用持久事实快照。相同摄像头映射、配置版本、隐私模式、维护结果和规则版本不再写文件或 SQLite；同步心跳只更新内存 `last_sync_at`。
- Relay 在发布器对象之外保存逐路摄像头线程生命周期，公开累计启动/停止次数、最后原因和时间。新线程创建前清除旧停止原因，尚无线程或发布器时的过期停止通知直接忽略。

### DIS 分割传播

- 人物分割非锚点帧不再创建 Farneback 光流和重复网格。每路复用一个 DIS UltraFast estimator，按分辨率缓存只读坐标网格，并复用形态学核。
- Pi 旧 Farneback 中位约 `15.78 ms`、P95 `20.96 ms`；DIS 版本中位约 `2.26 ms`、P95 `4.63 ms`、最终最大值 `9.36 ms`。`flow_width=320`、分割锚点频率、传播时限、场景拒绝和人物隐私门槛均未降低。

### 精确部署验证

- 当前主线提交为 `ffd67e0`，包含 `78e24c9`、`b830d80`、`30bb662` 和 `43df10d`。Pi 上 `config_sync_agent.py`、`live_relay_agent.py`、`h264_publisher.py`、`hailo_segmentation.py` 的 SHA-256 与主线逐项一致。
- 服务单次重启后为 `active/running`、`NRestarts=0`。状态文件在配置同步持续推进超过 30 分钟时保持 `mtime=1785898150`、大小 `29047`、SHA-256 `e4eaf64df1004e2b2c083e0201117a569b79bd5b5227044f79e99f4735868970`；没有临时状态文件，`last_sync_at` 从 `02:52:53Z` 推进到 `03:23:48Z`。
- 本地完整边缘回归再次 `58/58`。精确最新版运行 30 分钟、采样 61 次，采样错误为 0、服务状态始终 `ok`、系统重启值始终 0。
- 摄像头 31 与 32 的 Relay 线程均 `thread_starts=1 / thread_stops=0`，FFmpeg 均 `process_starts=1`；`publish_ready=true`，进程失败、半帧中止、连续失败和 stderr 均为 0。自重启后的 systemd journal 无 warning。
- 摄像头 31 编码输入范围为 `12.44-14.78 FPS`，摄像头 32 为 `9.75-14.97 FPS`。后者低点与摄像头源唯一帧节拍下降一致，正式发布器只发送最新真实帧，不允许复制重复像素伪造 15 FPS。
- 服务主进程现场约 `63.7% CPU`、`4.2%` 内存、`349472 KiB RSS`，温度监控范围 `59.5-63.4°C`。GH-059 达到根因、回归、部署、可观测性和长时运行门槛，已关闭。
...
- `/health` 新增 `pose_inference_coordinator`，公开队列深度、最大深度、在途身份、每路提交/替换/延期/合并/缓存命中、等待、推理失败和真实完成 Hz。
...
### 首轮 Pi 反证与 cadence 修复

- 首轮精确部署后服务 `active`、`NRestarts=0`、journal 无 warning；共享 Pose owner 仍为 1，Hailo Pose ready、失败为 0，两路源流约 `13.6-13.7 FPS`，两个 H.264 发布器均单次启动、publish ready 且无失败。
- 协调器最大队列深度为 1、正式请求无超时、推理无失败；摄像头 31 处于活跃调度但协调器输出约 `8.62 Hz`，摄像头 32 idle 输出约 `1.97 Hz`。Hailo 单次中位约 `23.48 ms`，因此活跃路降频不是设备吞吐不足。
- 根因是 submit 阶段在距离上次开始不足 `67 ms` 时直接丢弃当前帧。RTSP 唯一帧会出现短间隔成对到达，提前丢弃后下一帧可能直到约 `130 ms` 才到，形成约半速输出。
- 修复为给显示请求记录 `not_before_at`：每路首帧立即执行；后续早到帧进入唯一待处理槽，继续到达的早帧只替换其像素和身份；协调器等待截止时间后处理当时最新帧。formal 合并会把同帧请求立即提升为到期，不延迟正式分析。
- 确定性专项用 `500 ms` 首帧间隔验证首帧仍须在 `100 ms` 内开始；随后验证截止前的 `6` 被 `7` 替换，30 ms 内不得提前运行，80 ms 到期后调用序列必须为 `5, 7`。完整边缘回归再次 `59/59`，等待 Pi 二次部署。

### cadence 二次 Pi 验证

- 提交 `7f29827` 只替换 `app/vision/pose_coordinator.py`，远端文件 SHA-256 为 `8bce32693fece5b1c051453322e783b7ba351f175bbc016e0b009f53d89a343c`，与本地主线一致；未覆盖环境、模型、数据库、校准和设备状态。
- 稳定采样时共享 Pose runtime 仍为 1。摄像头 31 协调器输出 `10.04 Hz`、Tracker 模型锚点 `9.85 Hz`；摄像头 32 空闲输出 `2.26 Hz`、模型锚点 `1.94 Hz`。两路均取得进展，符合 active 与 idle 的职责区分。
...
- 候选请求后保持 `awaiting_formal`，即使中间高频帧为空也不能忘记或重复申请；只有开始时间不早于请求时间的正式分析可以裁决。正式拒绝按同一源连续拒绝次数执行 `10/20/40/60 秒` 上限指数退避，正式确认立即清零拒绝连击和冷却。
...
- `5d294d3` 部署后，持续电视假候选不再写 active hold：Scheduler 全程 `idle`、旧 `refresh_request_count=0`，两路 Coordinator 均保持约 `2 Hz`，证明候选验证与 Tracker 快速重锚已经分离。
- 第一版固定 3 秒冷却仍产生约 12 次正式拒绝/分钟，因此继续以 `0228751` 改为 `10/20/40/60 秒` 上限退避。正常 idle 正式分析仍为 2 Hz，真人进入不依赖退避中的候选快捷唤醒。
- 二次一分钟采样中，摄像头 31 累计 131 个原始 Pose 样本但正式确认始终为 0；验证请求和正式拒绝均仅 3 次，冷却依次约 10、20、40 秒。摄像头 32 无候选请求；两路正式超时和推理失败均为 0。
- 该结果关闭“电视假候选导致空场 active 升频和验证风暴”，但不替代真人进入、遮挡、快速移动、离开与宠物实测。

## 194. 2026-08-05 真人动作首轮证据与候选裁决时间

### 首轮结构化结果

- 用户完成双路可见范围内的进入、停留、遮挡、快速移动和离开。正式检测记录显示摄像头 32 于 `23:18:58`、摄像头 31 于 `23:19:03` 首次确认真人；短暂清空后两路均可再次确认，最终在约 `23:19:54` 清空。
- 动作期间摄像头 31 单帧姿态风险分最高为 `0.90`，但时序规则没有形成跌倒候选、正式事件或通知。单帧异常指标被保留用于分析，没有越过正式风险证据边界。
- 测试后两路 Tracker 均为 `empty / pose_count=0`；源有效帧约 `14.95 / 17.47 FPS`，H.264 发布约 `14.53 / 14.27 FPS`。Hailo 超时、推理失败、发布失败、服务重启和 journal warning 均为 0。
- 健康快照取得时人物已经离开，活跃调度的滚动 FPS 和进入延迟已经消失，因此本轮不能宣称活跃 Pose 达到 `10-15 Hz`，也不能关闭 GH-030。

### 可观测性修复

- `PoseCandidateValidationGate` 在唯一状态内记录最近一次验证请求、正式确认、正式拒绝和正式错误的单调时间；`/health` 同时输出对应距今秒数，不新增数据库或第二套计数。
- 正式确认、拒绝和错误分别写入自己的时间字段。候选源重置仍保留累计诊断，摄像头生命周期重置则与原有计数一起清空，语义与现有状态机一致。
- 专项覆盖请求、确认、拒绝和错误四条时间路径；完整边缘回归 `59/59` 通过。下一步部署后先启动 0.5-1 秒同步采样，再让用户重复一次短动作，直接记录进入确认延迟、active Pose Hz、离开清空和各计数增量。

## 195. 2026-08-05 宠物类别证据可观测性

- 现场 `/health` 只能看到 `track_count=2`，看不到两路轨迹分别被判成猫还是狗，也看不到原始类别证据。仅凭历史 `presence.pet_types` 不能区分当前模型输出、时序缓存和旧观察记录。
- `PetTemporalStabilizer` 现在用同一重入锁保护更新、摄像头重置、全量重置和健康读取，避免健康请求读取到类别分数已衰减但命中数尚未更新的中间状态。
- 健康状态新增逐路轨迹摘要：确认类别、确认置信度、轨迹命中次数、最近命中年龄，以及猫/狗各自的证据命中数和时序平均置信度。摘要不包含图片、人体或宠物框坐标。
- 专项固定验证猫证据经两帧确认、同框低分狗证据不抢占类别，并核对逐路状态字段；完整边缘回归 `59/59` 通过。
- 本项不改变类别阈值，也不根据当前家庭情况硬编码“只有猫”。部署后要让真实猫分别经过两路摄像头，再依据逐路证据决定是否需要专用宠物分类器、类别不确定态或模型数据校准。
- 提交 `a109eef` 已按单文件哈希精确部署，Pi 与主线 `pet_temporal.py` SHA-256 均为 `7adb19ef...c2730c0`。重启稳定后两路当前 `track_count=0 / confirmed_count=0`，说明历史 `presence.pet_types=cat/dog` 没有被错误当成当前轨迹；Hailo Pose/Object ready、失败为 0，两路源流与 H.264 发布正常、发布器单次启动且无失败。

## 196. 2026-08-05 空房五分钟稳定性采样

- 候选裁决时间和宠物逐路诊断部署后，在 Pi 本机每 10 秒读取一次唯一 `/health`，持续 `300.8` 秒，共 30 个有效样本、读取错误 0。
- 温度范围 `59.5-62.8°C`。摄像头 31 源有效帧 `13.56-14.86 FPS`、H.264 编码输入 `11.91-14.75 FPS`；摄像头 32 分别为 `11.51-14.48 / 11.23-14.41 FPS`。
- 两路 Scheduler 全程只有 `idle`，Pose 与 Object 推理失败增量均为 0；监控结束时两路宠物轨迹均为空。
- 电视柜路 5 分钟内新增 5 次正式拒绝、0 次正式确认、0 次正式错误；冰箱路三项增量均为 0。持续电视内容没有进入 active，也没有被正式认成人。
- 本轮只验证空场、短时温控和持续假候选隔离，不替代同步真人 active Hz、真实猫分类、30 分钟混合负载和 TestFlight 画面验收。

## 197. 2026-08-05 同步真人 active cadence

- 用户在采样运行期间完成进入、停留、遮挡、快速横移和离开。90.3 秒内取得 173 个健康样本、读取错误 0。
- 摄像头 31 新增 1 次正式确认，active 模型锚点峰值 `11.11 Hz`、协调器 Pose `14.43 Hz`、显示 `14.99 FPS`；源有效帧 `13.47-15.07 FPS`、H.264 输入 `13.23-15.14 FPS`。最后非空骨架后 `0.54` 秒回到 empty。
- 摄像头 32 在采样开始前约 3.4 秒已确认真人；active 模型锚点峰值 `10.46 Hz`、协调器 Pose `12.70 Hz`、显示 `14.94 FPS`；源有效帧 `12.63-15.06 FPS`、H.264 输入 `12.51-15.01 FPS`。
- 两路最终均为 `empty / pose_count=0 / 约 2 Hz idle`。正式分析超时、Pose 推理失败、Object 推理失败、发布失败、事件候选和正式事件均为 0，没有通知正常动作。
- 温度峰值 `72.15°C` 短暂进入 warm，随后回落 `62.25°C / normal`；Hailo Pose 中位 `22.77 ms`、P95 `44.75 ms`，服务零重启。
- 这组证据通过真人 cadence、遮挡恢复、快速移动、离开清空和正常动作不误报。由于用户在采样进程建立前已经入画，不能从本轮精确计算进入确认延迟；GH-030 还需真实猫类别证据和 30 分钟混合负载。

## 198. 2026-08-05 宠物类别现场反证

### 结构化证据

- 本轮没有读取、复制或新增家庭画面，只查询现有 `detection_results` 和 `/health` 的非图像字段。
- 摄像头 31 在 `15:19:50-15:19:53Z` 对家庭宠物连续输出 4 个 `cat`，置信度覆盖 `0.4628-0.5827`；摄像头 32 在 `15:59:15-15:59:22Z` 连续输出 8 个 `dog`，置信度覆盖 `0.5277-0.6564`。
- 两组结果进入 SQLite 前已经是 Hailo COCO Object 类别。`PetTemporalStabilizer` 按摄像头隔离，只在同一路内累计原始类别证据；App 只消费最终 `category`。因此错误不在 App 翻译、跨路状态或历史 Presence 回灌。

### 同期运行状态

- 连续 6 分钟观察中，Object 成功推理约增加 600 次，失败增量为 0。
- 温度约 `61-64°C`；摄像头 31 有效源流约 `13.6-14.9 FPS`，摄像头 32 约 `11.4-14.2 FPS`；两个 H.264 发布器全程 ready、重连和进程失败均为 0。
- 该窗口没有新的宠物轨迹。它证明服务和推理没有中断，但不能证明宠物召回率。

### 实现决策

- 不提高统一宠物阈值。错误 `dog` 与正确 `cat` 置信区间重叠，阈值只能同时减少真阳性和错误类别，不能恢复类别语义。
- 不按当前家庭硬编码猫，也不把多帧一致错误当成正确。现有时序层继续负责候选稳定和短时保持，不能承担基础模型无法区分的物种分类。
- 下一实现单元先建立经过用户授权的宠物框标注与版本清单，再用独立验证集比较专用轻量分类器和类别不确定态。产品在类别模型通过混淆矩阵前只输出“宠物活动”，原始 `cat/dog` 仅保留在受控诊断证据中。

## 199. 2026-08-06 宠物产品隔离与 30 分钟混合负载

### 产品与诊断数据边界

- `PersonDetector.pet_box()` 成为 CPU YOLO 与 Hailo Object 共用的宠物身份规范化入口。正式字段固定为 `type=pet`、`label=pet`、`label_zh=宠物`、`species_status=unverified`。
- 原始 COCO `class_id`、猫狗英文/中文标签和模型置信度继续保存在 `raw_model_*` 字段，供受控诊断、后续标注与混淆矩阵使用。未删除证据，也没有通过提高阈值隐藏误分类。
- `PetTemporalStabilizer` 仍按原始类别 ID 做逐路时序诊断，但确认后的产品对象保持通用宠物身份。家庭首页对当前与历史 `pet_types` 只显示“宠物”；盒子管理端不再用 `pet.type` 或旧中文标签推断猫狗。
- 视觉管线专项新增 CPU `cat`、Hailo `dog` 两条原始证据，均必须映射为同一产品身份；时序专项固定确认、保持和跨摄像头隔离后产品字段不变；前端静态契约禁止重新引入猫狗翻译。完整边缘回归 `59/59`、前端状态契约、管理端静态契约和差异检查通过。

### 长时运行结果

- Pi 本机按 30 秒间隔完成样本 `0-60`，共 61 次、约 30 分钟。服务全程运行；双路源读取失败与重连、Pose/Object/分割失败、正式分析超时、H.264 发布失败均为 0。
- 温度约 `55.1-66.1°C`，始终为 normal。人物活动样本中两路正式模型锚点达到约 `15.67 / 16.89 Hz`，协调器与发布输入接近各自有效源流；空房后回到约 `2 Hz` idle。
- 从人物模糊切换原画时，每路发布器只发生一次受控重建，累计 `starts=2`、失败为 0。窗口中宠物原始证据曾在同一路发生 `dog -> cat` 变化，再次证明物种类别不能对外使用。
- 本轮后半程所有样本仍报告 `privacy_mode=original`，用户侧切换没有形成骨架模式事务。因此不能把该窗口用于证明纯骨架无残影、人物清除或双路视觉质量；GH-008、GH-020 和计划第 15 项继续保持未完成。

### 下一步

- 提交 `f965f7f` 已推送主线。Pi 只精确替换 `person_yolo.py` 与 `admin/console.js`，SHA-256 分别为 `25f0250a...77dd064` 与 `6eccc0b3...e1b4ea`，和本地主线一致；重启后双路 streaming、两个发布器均 ready 且只启动一次，Pose/Object/读取/重连/发布失败为 0，温度 `61.15°C`。
- 生产云通过正式构建器和事务安装器发布 `/opt/gohome/releases/20260805165225-f965f7f66a18`，迁移校验均为 skipped，`current` 和 systemd 入口指向新版本，健康接口使用 PostgreSQL 且 WHEP 鉴权正常。公网 `monitor-live.js` SHA-256 为 `a42e624f...f71e2c`，与本地主线一致；云端与 Pi 均 `NRestarts=0`、部署后 warning 为 0。
...
- `596dcff` 精确部署后，运行源码哈希与主线一致，两份基线文件 SHA-256 仍为 `291639...bd8` 和 `8a2cda...344`，服务 active、`NRestarts=0`、无 warning。
- 32 路连续得到强单应移动，中位/最大角点位移 `0.04770 / 0.05512`；31 路为 `0.05813 / 0.08850`。原三态状态机正确保留了基线，但单应矩阵的局部匹配外推仍不足以单独证明整台摄像头移动，因此本轮不通过、不得重校准。
- 几何 v3 对同一组 ORB 匹配同时求单应和部分仿射 RANSAC，分别计算内点率与角点位移，并计算当前帧和基线内点的最小空间包围覆盖及 `4x3` 网格覆盖。
- 正式机位变化要求两个模型均具备强证据、均判断位移超限且空间覆盖充分。局部特征簇、模型冲突、仿射不可用或覆盖不足统一为 unverifiable；强仿射同视角且单应证据集中时允许以中等置信恢复同视角。
- 新增局部 `100x100` 纹理簇平移的确定性反例：单应会产生大角点位移，但空间覆盖不足，最终必须为 `geometry_models_inconclusive`，不能成为 `camera_view_changed`。原有全场平移仍需连续三次独立强证据后阻断。

### 第二轮 Pi 结果与低纹理路径

- `4f9c402` 精确部署后，32 路自动完成 `3/3` 复验并恢复 ready。单应中位/最大位移 `0.05127 / 0.06293`，仿射仅 `0.00518 / 0.00581`，内点覆盖 `0.1076`，模型状态为 conflict；该结果验证单模型局部外推已不会再触发重校准。
- 31 路的移动候选和场景复核次数均为 0，但连续复验只有约 6 个 ORB 匹配，无法建立单应或仿射，状态保持 `revalidation_uncertain / insufficient_geometry_matches`。两份基线哈希仍未变化，服务零重启、无 warning。
- 几何 v4 在 ORB 无法估计时比较基线和当前帧的全局梯度幅值，通过 Hanning 窗和 phase correlation 计算响应及归一化平移。仅当边缘能量足够、响应至少 `0.08`、平移不超过画面对角线 `0.012` 时返回中等同视角。
- 相位路径只提供 same_view 证据，不具备 camera_view_changed 权限。确定性回归强制低特征亮度变化恢复同视角，而 50 像素全局平移保持 unverifiable；这不是降低 ORB 匹配阈值，也不会形成第二套背景合成。

### 真实重启恢复完成

- `57d7578` 精确部署后，运行文件 SHA-256 为 `5e64eccf...ceec4` 与 `90524a2e...ea34`，和主线一致。31 路首次可靠相位结果为响应 `0.21231`、位移比 `0.00111`，随后稳定为 `0.21288 / 0.00106` 并完成 `3/3`。
...
## 207. 2026-08-06 内容帧与直播投递帧首轮 Pi 实机证据

- 使用主线 `c0539a0`，只精确传输并原子安装两个运行文件；未运行会删除盒子历史文件的全量同步/清理脚本。
- 重启 `gohome-edge-agent.service` 后，盒子健康状态为 `ok`，两路 `streaming`，两份持久基线均完成 `3/3` 复验并为 `ready`。
- 两路 `decoded_fps/live_delivery_fps` 为 `15.01/15.01` 与 `15.05/15.05`；`content_fps` 为 `13.61/14.75`。这证明重复但尚未冻结的解码帧被送入直播槽，却没有推进内容分析身份。
- H.264 `encoder_input_fps_10s` 为 `14.48/14.23`，两路 `publish_ready=true`；读取失败、重连、发布失败和连续失败均为 `0`。纯骨架端侧成品输出约 `13.1/13.9 FPS`。
- 本证据只关闭“代码已部署、指标方向正确”的阶段门槛，不关闭产品验收：管理端登录画面、正式 App 播放、三模式、人物进入/离开、遮挡、冻结恢复、断网恢复和 30 分钟长测仍必须由真实用户路径验证。

### 现场用户路径补充

- 用户在正式 GoHome App 守护页以 `skeleton` 模式经过两路摄像头并离开；确认手机端能看到骨架，主观流畅度为“比较流畅”。
- 盒子同步指标显示两路人物帧均进入纯骨架合成，离开后 `track_count=0`，无读取/发布失败、跨路状态或旧骨架生命周期残留。
- 该结果只通过纯骨架可见性和基本跟随子门槛；下一步仍需同一 App 会话完成三模式切换、遮挡、断流恢复和长时指标验收。
...
## 210. 2026-08-06 可转动摄像头多机位基线根因修复

### 根因

- 旧实现以 `camera_id + configured_source_key + resolution` 为状态键，但每个状态只有一个 `background`。摄像头重连提升流代或云台转动后，几何复验只能拿当前画面和这一份基线比较；即使当前角度以前被确认过，也会进入 `scene_review_required`，导致纯骨架发布暂停。
- 不能通过自动学习当前画面解决：有人时会把人物像素写进隐私基线，产生残影或隐私泄漏；也不能删除摄像头、重新绑定或把源流在线直接当成隐私成品可用。

### 实现

- `PrivacyBackgroundReconstructor` 现在每路最多保留 4 份明确确认的机位基线，活动机位与流代/复验状态分开管理。重连或画面变化时，只在确认的其他机位中执行一次有界匹配；命中后切换活动基线并从 `stream_revalidation_required` 开始复验。
- 新空房校准先生成候选平均帧，再以 `SHA-256` 内容身份选择已有机位或创建新机位。新文件使用 `-view-<16 hex>` 命名；初始旧单文件继续使用无后缀路径，启动时同时发现两种格式。写入仍为临时文件、`fsync`、`os.replace`；失败时不改变旧基线。达到上限只淘汰最旧非活动机位。
- 管理状态新增 `active_view_id`、`known_view_count` 和不含像素的 `known_views` 摘要，保持 `baseline_sha256` 指向当前活动机位；不向 App 或云端返回图片。
...
## 211. 2026-08-07 流断续与摄像头配置变化分离

### 根因

- `CameraAgent` 在 RTSP 读失败、近黑、冻结或主动重连时会提升 `stream_generation` 并清理旧帧，这是必要的帧身份隔离。
- 旧的源变更回调没有携带事件类型，`LiveRelayAgent` 将这些流断续事件与修改地址、账号、启停状态混为一类，停止整路发布线程并等待下一次同步重建。重连期间因此额外丢失编码器上下文和直播窗口。

### 实现

- `CameraAgent` 为配置变化发送 `transition_type=configuration`，为流断续发送 `transition_type=stream_discontinuity`。
- `LiveRelayAgent` 对流断续只暂停现有发布器、标记隐私成品进入 `revalidating`，不停止采集线程、不关闭编码器、不创建第二个发布器；恢复帧通过原有隐私机位复核和发布状态机重新放行。
- 摄像头真实配置变化或删除仍走原有停止、重建和生命周期记录，避免把两个摄像头或旧源混入新流。

### 验证

- `edge-agent/.venv/bin/python edge-agent/scripts/verify-live-relay-upload-pipeline.py` 通过。
- 新增断流回归确认：同一路发布器数量保持 `1`、原发布器未关闭、线程 `thread_starts=1/thread_stops=0`，并记录 `stream_read_failed` 暂停原因；配置变化回归仍确认旧发布器关闭并创建新发布器。
...
- 真实家庭长期误报/漏报、APNs 端到端到达、猫狗类别混淆矩阵和跨品牌摄像头兼容。
- 双摄纯骨架在真实摄像头转动、拔插恢复、断网、前后台切换和 30 分钟长测下的无残影、帧龄、重连与温度证据。
- 20-50 户家庭试点中的有效联系次数、回家计划完成率、安装成本、售后频率和持续运行稳定性。
...
- 事件证据统一为 `before`、`transition`、`evidence`、`current` 四个角色。盒子只从有界动作窗口中选择真实帧；少于四个完整角色时不复制图片、不启动模型复核、不触发通知。
- 云端把四帧放入一次百炼兼容接口请求的单个有序 `type=video` 序列，按严格 `gohome-vision-verification-v2` 解析 `confirmed`、`suspected`、`rejected`、`uncertain`，四级之外拒绝入库。
- `confirmed`/`suspected` 进入 App 待处理，幂等键保证同一事件最多一条通知；`rejected`/`uncertain` 进入系统记录，不要求用户确认、不提供处理按钮、不发送通知。
- App 和盒子管理端使用同一事件证据关系和角色标签展示四帧；事件详情不复制资源、不跨摄像头拼接、不由客户端补画骨架或改变事件等级。

### 自动验证与部署

- iOS 单元测试 `196` 项通过，模拟器 Keychain entitlement 专项按环境跳过 `1` 项；UI 回归 `22` 项通过，首次整套运行被 Runner 终止的 `2` 项独立重跑通过。
- `npm test`、Node 服务契约、云媒体门禁、`git diff --check` 和发布元数据检查通过。
- 盒子和生产云已部署四帧事件代码；服务保持运行，生产 `/health` 的 PostgreSQL、COS、WHEP/H.264 和 APNs 配置检查通过，推送队列为 `0`。
- 唯一原生工程 `/ios-shell/GoHomeShell.xcodeproj` 归档为 `1.0.0 (16)`，App Store Connect 已完成处理并将构建加入内部组“比赛内测”；用户已确认手机显示 Build 16。二进制 WebRTC 依赖仍缺少匹配 dSYM，只影响第三方崩溃符号化，不影响安装和运行。

### 未关闭的产品验收

- 真实快速跌倒、正常坐下/弯腰、宠物活动和空场景仍需在同一家庭环境产生新事件，核对四帧、四级结果和通知一次性。
- `rejected`/`uncertain` 不通知且只进系统记录，`suspected`/`confirmed` 只通知一次并进入待处理，必须由 App 和管理端同时核对。
- 事件真机验收完成前，GH-118、GH-123 及双摄长时/断线/隐私链路相关台账保持“待实机验收”；本节不把自动回归或 TestFlight 可安装写成准确率达标。

## 215. 2026-08-12 摄像头初始化不再等待空房

### 根因与实现

- 旧链路把纯骨架校准状态和 H.264 媒体发布状态合并；缺少背景时暂停发布，云端返回不可播放，原画和人物模糊也被错误阻断。
- 新链路分别报告 `media_ready`、`privacy_ready` 和 `delivered_mode`。原画与人物模糊不依赖空房背景；纯骨架准备期间使用同一个持久发布器输出无摄像头像素的 `privacy_hold`。
- 自动空房校准复用每路唯一共享帧与现有 Hailo 人物分割，不创建旁路读取器。有人证据清零候选，稳定空房自动原子提交；流代变化重置候选，手动校准可接管，已有多机位背景继续保留。

### 自动验证

- 边缘端全部 `63/63` 专项通过，包含隐私背景、配置同步、直播发布、媒体所有权、视频路由和同帧分析。
- 云端 `npm test` 通过；原生服务 `120/120` 通过，PostgreSQL 实库专项因本机未配置连接地址跳过 `1` 项。
- 唯一正式 iOS 工程单元测试 `196/196`、UI 测试 `28/28` 通过。

### 未关闭条件

- 当前仅完成代码和自动验证，尚未把本项部署到盒子与生产云。
- 部署后必须现场验证有人初始化、原画/模糊立即播放、纯骨架隐私准备、无人自动完成、同会话切换和双路隔离；未取得这些证据前不得写成实机完成。
