...
- API 客户端从 sessionStorage 升级为按账号隔离的 localStorage 持久缓存。用户、家庭、老人资料和关怀设置缓存 5 分钟，天气和热点缓存 10 分钟，设备和摄像头缓存 10 秒，事件和消息缓存 3 秒；过期缓存立即返回并由单个后台请求静默更新，实时快照和视频流不缓存。
- 新增 `app-state-store.js`，为首页、守护、事件、陪伴、我的保存最近一次真实 `<main>` 渲染和滚动位置。页面返回时先恢复真实内容，再后台刷新；首次无快照直接展示中性页面结构，不再渲染整页白屏转圈。短期关怀图片播放票据随快照保留，超过有效窗口则显示卡片真实标题和正文，后台取得新票据后再无缝替换；视频流、access token 地址、表单值和临时弹窗不进入快照，账号切换或退出登录时整体清理。
- 视频播放票据 POST 标记为只读技术请求，不再误清 API 缓存；缓存增加代次保护，写操作发生前启动的旧后台请求不能覆盖新状态。守护页复用快照中的摄像头卡片，只重连视频和更新状态，不再先重建为“同步中”。
...
- 新增 `verify-frontend-state-cache.js`，验证过期数据立即返回、并发后台请求去重、更新事件、只读播放票据不失效缓存和写操作清缓存；该探针已纳入 `npm test`。
- `npm test` 和本地完整闭环均通过，闭环结果为 37 passed / 0 warnings / 0 failed。
- 本地浏览器验证五个主页面首次进入约 35-69ms 可见，整页遮罩、转圈和阻塞式加载文案均为 0；守护双摄卡片不再出现“同步中/识别中”。首页与陪伴页切走再返回时，1024px 关怀原图仍可见，fallback 保持隐藏。
- 腾讯云已同步全部版本化 HTML 和前端脚本；`gohome-app` 重启后 active，HTTPS 健康检查返回 PostgreSQL store。线上五个主页面实际渲染无整页 overlay、无阻塞式加载文案、无横向溢出和控制台错误；静态主内容扫描对应加载词数量全部为 0。
- 树莓派 vision preflight 全部通过，服务、配置同步、双摄像头和 live relay 正常。

当前边界：本轮只修改网页与缓存层，不需要重新安装 iOS App；真机需完全关闭后重新打开一次，加载线上新版本并复测五个主页面和视频重连。`app-shell.html`、`detection.html` 与纪念模式页面仍有真实引用，必须先迁移入口再删除。APNs 仍未开启，本轮不修改原生推送 entitlement。
