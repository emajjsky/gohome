...
- 照片选择后在后台并行读取，并使用 ImageIO 按最长边 1920 px 降采样、统一输出 0.8 质量 JPEG；发布按钮在照片处理完成前保持禁用，避免快速提交遗漏图片。
- 多张新照片由原来的串行上传改为并行上传，上传完成后仍按用户选择顺序组合 `asset_ids`；业务层再次限制保留图片与新图片合计最多 9 张。
...
- 新增网格规则、9 张上限和并行上传顺序回归；记忆定向测试 6 项全部通过。
