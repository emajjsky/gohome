...
function buildCloudSeedBundle(db, options = {}) {
    ...
    const exportedMessageIds = new Set(appMessages.map((message) => String(message.message_id || "")).filter(Boolean));
    ...
    const notificationDeliveries = toArray(db.notification_deliveries)
        .filter((delivery) => !delivery.message_id || exportedMessageIds.has(String(delivery.message_id)))
        .map((delivery) => ({
